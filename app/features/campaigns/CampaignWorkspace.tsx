...
export function CampaignWorkspace({ accessToken }: { accessToken: string }) {
  ...
  function selectApproach(value: string) {
    setApproach(value);
    const selected = data.approaches.find((item) => String(item.id) === value);
    if (selected) setMessage(approachText(selected.mensagens));
  }
  ...
  async function schedule() {
    if (!valid.length || !message.trim()) { setNotice("Selecione um público válido e escreva a mensagem."); return; }
    if (!window.confirm(`Agendar ${valid.length} mensagens? Elas serão processadas pelo provedor conectado.`)) return;
    setBusy(true); setNotice("");
    const response = await fetch("/api/campaigns", { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ leadIds: valid.filter((lead) => lead.id < 10_000_000).map((lead) => lead.id), message, rate: Number(rate), start: `${startDate}T${startTime}:00`, sourceStageId: Number(stage), destinationStageId: Number(destinationStage) }) });

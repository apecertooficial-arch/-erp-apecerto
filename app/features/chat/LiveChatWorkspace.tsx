...
export function LiveChatWorkspace({ accessToken, initialLeadId = null, onInitialLeadHandled }: { accessToken: string; initialLeadId?: number | null; onInitialLeadHandled?: () => void }) {
  ...
  return <div className="live-chat">
    ...
    {productOpen && data && <ProductSendModal data={data} canSend={Boolean(contact && instanceId)} onClose={() => setProductOpen(false)} onSend={async (content, mediaId) => { await send(content, mediaId); setProductOpen(false); }} onSendPack={async (empId) => { if (!contact || !instanceId) throw new Error("Selecione uma conversa e uma instância conectada."); const { error: fnErr } = await getBrowserSupabaseClient().functions.invoke("enviar-produto", { body: { empreendimento_id: empId, telefone: contact.telefone, instancia_id: instanceId } }); if (fnErr) throw new Error(fnErr.message); setProductOpen(false); }} />}
...
export function ProductSendModal({ data, canSend, onClose, onSend, onSendPack }: { data: ChatData; canSend: boolean; onClose: () => void; onSend: (content: string, mediaId?: string) => Promise<void>; onSendPack?: (empreendimentoId: string) => Promise<void> }) {
  const [productId, setProductId] = useState(""); const [mediaId, setMediaId] = useState(""); const [pack, setPack] = useState(false); const product = data.products.find((item) => item.id === productId); const items = data.media.filter((item) => item.empreendimento_id === productId); const fotos = items.filter((item) => item.tipo === "foto");
  const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  return <div className="crm-center-modal"><form onSubmit={(event) => { event.preventDefault(); if (!product || !canSend) return; setBusy(true); setError(""); const fail = (reason: unknown) => setError(reason instanceof Error ? reason.message : "Não foi possível enviar o produto."); const done = () => setBusy(false); if (pack && onSendPack) { void onSendPack(productId).catch(fail).finally(done); } else { void onSend(`${product.nome} · ${product.bairro || ""}, ${product.cidade || ""} · ${product.preco ? money.format(product.preco) : "valor sob consulta"}`, mediaId || undefined).catch(fail).finally(done); } }}><header><div><span>ENVIO PELO CHAT</span><h2>Enviar produto</h2><p>Escolha o imóvel e envie o pack de fotos ou um material específico.</p></div><button type="button" onClick={onClose}>×</button></header>{!canSend && <div className="modal-error">Selecione uma conversa com contato e uma instância conectada antes de enviar.</div>}<label>Produto<select required value={productId} onChange={(event) => { setProductId(event.target.value); setMediaId(""); setPack(false); }}><option value="">Selecione</option>{data.products.map((item) => <option value={item.id} key={item.id}>{item.nome}</option>)}</select></label>{onSendPack && <label className="pack-toggle" style={{ display: "flex", gap: 8, alignItems: "center" }}><input type="checkbox" checked={pack} disabled={!productId || fotos.length === 0} onChange={(event) => setPack(event.target.checked)} /> Enviar TODAS as fotos como pack{productId ? ` (${fotos.length})` : ""}</label>}{!pack && <label>Material<select value={mediaId} onChange={(event) => setMediaId(event.target.value)}><option value="">Somente resumo do produto</option>{items.map((item) => <option value={item.id} key={item.id}>{item.tipo} · {item.categoria || item.nome || "material"}</option>)}</select></label>}{pack && <div className="modal-hint">Serão enviadas as {fotos.length} fotos em sequência (o book entra junto quando disponível).</div>}{error && <div className="modal-error">{error}</div>}<footer><button type="button" onClick={onClose}>Cancelar</button><button className="crm-primary" disabled={busy || !productId || !canSend || (pack && fotos.length === 0)} type="submit">{busy ? "Enviando…" : pack ? `Enviar pack (${fotos.length} fotos)` : "Enviar no WhatsApp"}</button></footer></form></div>;
}

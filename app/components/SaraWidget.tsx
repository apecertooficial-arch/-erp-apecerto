...
type SaraMessage = { role: "user" | "assistant"; content: string };
type SaraRouterResponse = {
  ok?: boolean;
  reason?: string;
  resposta?: string;
  saida?: unknown;
...
declare global {
  interface Window {
    __saraWidget?: boolean;
    saraAsk?: (pergunta?: string) => void;
  }
}
  ...
  useEffect(() => {
    ...
    function md(s: string): string {
      ...
      for (let i = 0; i < lines.length; i++) { const ln = lines[i];
        const mn = ln.match(/^\s*\d+\.\s+(.*)$/), mb = ln.match(/^\s*[-•]\s+(.*)$/);
    ...
    const st = document.createElement("style"); st.id = "sara-style";
    ...
    "body:has(.modal-layer[aria-modal='true']) #sara-fab,body:has(.modal-layer[aria-modal='true']) #sara-panel{display:none!important;}",
    "#sara-fab .pulse{position:absolute;inset:0;border-radius:50%;animation:saraP 2.4s infinite;}",
    "@keyframes saraP{0%{box-shadow:0 0 0 0 rgba(224,30,99,.45);}70%{box-shadow:0 0 0 16px rgba(224,30,99,0);}100%{box-shadow:0 0 0 0 rgba(224,30,99,0);}}",
    ...
    "#sara-head{background:linear-gradient(115deg,#e21d74 0%,#a726b3 50%,#7c3aed 100%);color:#fff;padding:16px 18px;display:flex;align-items:center;gap:12px;}",
    "#sara-head .av{width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#ff8a2b,#ff5e1f);box-shadow:0 4px 12px rgba(255,94,31,.35);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:17px;flex-shrink:0;}",
    ...
    "#sara-msgs{flex:1;overflow-y:auto;padding:16px 14px;background:linear-gradient(180deg,#f7f2fc,#f1eafa);display:flex;flex-direction:column;gap:10px;}",
    ...
    ".sara-line .mini{width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#ff8a2b,#ff5e1f);color:#fff;font-size:12px;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-bottom:2px;}",
    ...
    const panel = document.createElement("div"); panel.id = "sara-panel";
    panel.innerHTML = '<div id="sara-head"><div class="av">S</div><div><b>Sara</b><span class="stat"><i></i> assistente apêcerto · online</span></div><button id="sara-x">×</button></div><div id="sara-msgs"></div><div id="sara-foot"><textarea id="sara-in" rows="1" placeholder="Pergunte algo ou peca uma direcao..."></textarea><button id="sara-send"><svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M22 2L11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/></svg></button></div>';
    document.body.appendChild(fab); document.body.appendChild(panel);
    const msgsEl = panel.querySelector<HTMLDivElement>("#sara-msgs")!;
    const inEl = panel.querySelector<HTMLTextAreaElement>("#sara-in")!;
    function bubble(role: SaraMessage["role"], content: string): HTMLDivElement {
      const line = document.createElement("div"); line.className = "sara-line " + (role === "user" ? "u" : "a");
      const mini = document.createElement("div"); mini.className = "mini"; mini.textContent = role === "user" ? "" : "S";
      const b = document.createElement("div"); b.className = "sara-b " + (role === "user" ? "u" : "a");
    ...
    function greet() {
      ...
      const chips = document.createElement("div"); chips.className = "sara-chips";
      sugestoes.forEach(function (s) { const c = document.createElement("button"); c.className = "sara-chip"; c.textContent = s; c.onclick = function () { chips.remove(); void enviar(s); }; chips.appendChild(c); });
    ...
    panel.querySelector<HTMLButtonElement>("#sara-x")!.onclick = close;
    ...
    panel.querySelector<HTMLButtonElement>("#sara-send")!.onclick = function () { void enviar(); };
    ...
    window.saraAsk = function (q) { open(); if (q != null && String(q).trim()) { setTimeout(function () { void enviar(String(q)); }, 120); } };
  }, []);
  return null;
}

...
test("copiloto, colaboração, board e métricas têm contratos honestos e RLS", () => {
  ...
  assert.match(board, /Visão do gestor/);
  assert.match(metricsDashboard, /conecte Meta/);
  assert.match(board, /Sem responsável/);
  assert.match(calendar, /moveSchedule/);
  assert.match(calendar, /mode === "mes"/);

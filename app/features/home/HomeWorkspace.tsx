...
export function HomeWorkspace({ accessToken, sessionName = "", onNavigate }: { accessToken: string; sessionName?: string; onNavigate?: (module: string) => void }) {
  ...
  const [chartMonths, setChartMonths] = useState(6); /* Doc §4 — período dos gráficos */
  ...
  return <div className="home-workspace">
    <header className="home-header"><div><h1>Bem-vindo(a), {sessionName ? sessionName.split(/\s+/)[0] : "corretor(a)"}! 👋</h1><p>Visão geral da operação em tempo real</p></div><span>● Dados reais · sessão protegida</span></header>
    ...
    <section className="home-charts">
      <article className="home-panel"><header className="home-chart-head"><h2>VGV por mês</h2><nav>{[[3, "3m"], [6, "6m"], [12, "12m"]].map(([months, label]) => <button className={chartMonths === months ? "active" : ""} type="button" onClick={() => setChartMonths(months as number)} key={String(months)}>{label}</button>)}</nav></header>{(() => { const nowDate = new Date(); const buckets = Array.from({ length: chartMonths }, (_, index) => { const ref = new Date(nowDate.getFullYear(), nowDate.getMonth() - (chartMonths - 1 - index), 1); const key = `${ref.getFullYear()}-${String(ref.getMonth() + 1).padStart(2, "0")}`; const total = data.finance.sales.filter((sale) => (sale.data_venda || "").startsWith(key)).reduce((sum, sale) => sum + Number(sale.vgv || 0), 0); return { key, label: ref.toLocaleDateString("pt-BR", { month: "short" }), total }; }); const maxTotal = Math.max(1, ...buckets.map((bucket) => bucket.total)); return <div className="home-bars">{buckets.map((bucket) => <div key={bucket.key}><b>{bucket.total > 0 ? compact.format(bucket.total) : ""}</b><i style={{ height: `${Math.max(3, bucket.total / maxTotal * 100)}%` }} /><span>{bucket.label}</span></div>)}</div>; })()}<footer className="home-legend"><span><i className="orange" /> VGV vendido no mês</span></footer></article>
      <article className="home-panel"><header className="home-chart-head"><h2>Leads por origem</h2></header>{(() => { const origins = [...new Set(data.crm.leads.map((lead) => lead.origem || "Sem origem"))].map((origin) => ({ origin, count: data.crm.leads.filter((lead) => (lead.origem || "Sem origem") === origin).length })).sort((a, b) => b.count - a.count).slice(0, 6); const maxOrigin = Math.max(1, ...origins.map((entry) => entry.count)); return origins.map((entry) => <button className="home-funnel-row drill" type="button" onClick={() => onNavigate?.("CRM")} key={entry.origin}><span><i style={{ background: "#8d2bd1" }} />{entry.origin}<b>{entry.count} leads</b></span><div><u style={{ width: `${entry.count / maxOrigin * 100}%`, background: "#8d2bd1" }} /></div></button>); })()}<footer className="home-legend"><span><i className="purple" /> Base completa do CRM · clique para abrir</span></footer></article>
    </section>
    <section className="home-two-columns">
      <article className="home-panel"><h2>Funil por etapa</h2>{stageRows.map((stage) => <button className="home-funnel-row drill" type="button" onClick={() => onNavigate?.("CRM")} key={stage.id}><span><i style={{ background: stage.cor || "#ff6500" }} />{stage.nome}<b>{stage.count} · leads</b></span><div><u style={{ width: `${stage.count / maxStage * 100}%`, background: stage.cor || "#ff6500" }} /></div></button>)}{stageRows.length === 0 && <p>Nenhum negócio aberto no funil.</p>}</article>
    ...
    <section className="home-three-columns">
      ...
      <article className="home-panel"><h2>Atalhos operacionais</h2><p className="home-panel-note">Notificações operacionais moram no CRM e na página de Notificações.</p><button className="home-alert drill" type="button" onClick={() => onNavigate?.("CRM")}><i>△</i><span><b>{data.crm.alerts.length} alertas de atendimento</b><small>Abrir no CRM</small></span></button><button className="home-alert drill" type="button" onClick={() => onNavigate?.("Financeiro")}><i className="yellow">$</i><span><b>{data.finance.receipts.filter((item) => item.status !== "recebido").length} recebimentos pendentes</b><small>Abrir no Financeiro</small></span></button><button className="home-alert drill" type="button" onClick={() => onNavigate?.("Produtos")}><i className="orange">▥</i><span><b>{data.catalog.catalog.filter((item) => item.available <= 1).length} produtos com estoque baixo</b><small>Abrir Produtos</small></span></button></article>
    </section>
  </div>;
}

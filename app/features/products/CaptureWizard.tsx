      ...
      <section className="capture-panel">
        ...
        <div className="capture-body">
          ...
          {step === 3 && <div className="form-section">
            ...
            <MoneyInput label="Preço de venda" value={property.price} onChange={(value) => setProperty({ ...property, price: value === null ? "" : String(value) })} />
            ...
            {propertyType === "construtora" && <div className="unit-editor"><div className="section-row"><div><strong>Unidades do empreendimento</strong><small>Preço em milhares por padrão, com confirmação do valor final.</small></div><button className="secondary-action" onClick={() => setUnits([...units, newUnit()])} type="button">＋ Adicionar unidade</button></div>{units.map((unit, index) => <div className="unit-row professional" key={unit.id}><span>{index + 1}</span><input aria-label="Número da unidade" value={unit.number} onChange={(event) => setUnits(units.map((item) => item.id === unit.id ? { ...item, number: event.target.value } : item))} placeholder="Unidade" /><select aria-label="Tipologia" value={unit.type} onChange={(event) => setUnits(units.map((item) => item.id === unit.id ? { ...item, type: event.target.value } : item))}><option value="">Tipologia</option>{unitTypologies.map((t) => <option key={t} value={t}>{t}</option>)}</select><input aria-label="Área" type="number" min="0" value={unit.area} onChange={(event) => setUnits(units.map((item) => item.id === unit.id ? { ...item, area: event.target.value } : item))} placeholder="m²" /><input aria-label="Vagas" type="number" min="0" value={unit.parking} onChange={(event) => setUnits(units.map((item) => item.id === unit.id ? { ...item, parking: event.target.value } : item))} placeholder="Vagas" /><MoneyInput compact label={`Preço da unidade ${unit.number || index + 1}`} value={unit.price} onChange={(value) => setUnits(units.map((item) => item.id === unit.id ? { ...item, price: value === null ? "" : String(value) } : item))} /><MoneyInput compact label={`Preço promocional da unidade ${unit.number || index + 1}`} value={unit.promotionalPrice} onChange={(value) => setUnits(units.map((item) => item.id === unit.id ? { ...item, promotionalPrice: value === null ? "" : String(value) } : item))} /><button aria-label="Remover unidade" disabled={units.length === 1} onClick={() => setUnits(units.filter((item) => item.id !== unit.id))} type="button">×</button></div>)}</div>}
          </div>}

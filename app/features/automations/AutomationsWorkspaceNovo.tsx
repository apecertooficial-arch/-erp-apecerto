...
/* AUTOMAÇÕES — casca nova (rota /automacoes-novo).
 *
 * ESCOPO, dito na cara: esta tela NÃO reimplementa o construtor. O
 * automationBuilderRuntime.js (159 KB) continua sendo montado exatamente como em
 * AutomationsWorkspace.tsx — mesmo import dinâmico, mesmo mount(host, ctx), mesmo
 * unmount na saída. Nós, portas, arraste, zoom, gaveta, paleta, publicar, versões
 * e monitor são os dele. Fluxos, blocos e versões continuam no Supabase; nada aqui
 * migra, reescreve ou recria dado.
 *
 * O que esta casca acrescenta: uma BIBLIOTECA antes do construtor — cada automação
 * lida como sequência ("Quando entra lead → Espera → Envia abordagem"), com os
 * contadores derivados da MESMA consulta que preenche a lista, para nenhum número
 * da tela divergir de outro.
 *
 * Rota separada de propósito: /automacoes segue no ar, sem uma linha alterada.
 * Comparar as duas lado a lado é o teste; trocar a rota principal é decisão do
 * Romulo, depois de aprovar.
 */
...
function lerPassos(mapa: unknown): { passos: Passo[]; blocos: number } {
  const m = (mapa ?? {}) as { editor?: { blocks?: Record<string, { x?: number; y?: number }> }; automation?: { blocks?: Array<{ id?: string; type?: string; presentation?: { x?: number; y?: number } }> } };
  const blocos = m.automation?.blocks ?? [];
  const editor = m.editor?.blocks ?? {};
  const ordenados = blocos.slice().sort((a, b) => {
    const ya = editor[String(a.id)]?.y ?? a.presentation?.y ?? 0;
    const yb = editor[String(b.id)]?.y ?? b.presentation?.y ?? 0;
    return ya - yb;
  });
  const passos = ordenados.map((b) => TIPOS[String(b.type)] ?? { rotulo: String(b.type ?? "Bloco"), tint: "#F2EFEC", tinta: "#6E6760" });
  return { passos, blocos: blocos.length };
}

export function AutomationsWorkspaceNovo({ accessToken }: { accessToken: string }) {
  ...
  /* Construtor original. Igual ao AutomationsWorkspace: import dinâmico, mount com
     o mesmo contexto, unmount ao sair. Quando o gestor chegou aqui clicando em uma
     automação, o único gancho possível é o próprio item da coluna do runtime
     (.sb-item[data-id]) — ele não expõe openAutomacao. Tentamos por alguns frames
     e desistimos em siлêncio: sem o clique, ele fica na tela inicial dele, que é um
     estado legítimo. */
  useEffect(() => {
    ...
    return (
      <div className="automations-v2-shell apn-shell">
        <header className="apn-topo-construtor">
          ...
          <div className="apn-topo-txt"><span>CONSTRUTOR</span><h1>Fluxo aberto no construtor original</h1></div>
      ...
      <section className="apn-secao">
        ...
        <div className="apn-kpis">
          {[
            ...
            { ico: "▤", n: contas.blocos, r: "Blocos no total", p: "somando todos os fluxos ativos", tint: "#F7ECFC", tinta: "#66009A" },

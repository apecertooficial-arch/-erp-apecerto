...
export function CrmWorkspace({ accessToken, initialDealId = null, onInitialDealHandled }: { accessToken: string; initialDealId?: number | null; onInitialDealHandled?: () => void }) {
  ...
  return <div className="crm-v2">
    ...
    {createOpen && data && <CreateLeadModal pipelines={data.pipelines} brokers={data.brokers} initialPipelineId={pipelineId} onClose={() => { setCreateOpen(false); setMessage(null); }} onCreate={async (payload) => { await mutate({ action: "createLead", ...payload }); setCreateOpen(false); setMessage("Novo lead criado e inserido na primeira etapa."); }} />}
  </div>;
}
...
function LeadChatDrawer({ accessToken, lead, deal, onClose, onResponse }: { accessToken: string; lead: Lead; deal: Deal; onClose: () => void; onResponse: () => Promise<void> }) {
  ...
  async function send() { if (!selected?.sendBig || !draft.trim()) return; setSending(true); setError(null); try { await request({ action: "send", telefone: lead.telefone, instanciaId: selected.sendBig, texto: draft }); setDraft(""); await onResponse(); await loadMessages(selected); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível enviar."); } finally { setSending(false); } }
...
function CreateLeadModal({ pipelines, brokers, initialPipelineId, onClose, onCreate }: { pipelines: Pipeline[]; brokers: Broker[]; initialPipelineId: number | null; onClose: () => void; onCreate: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  return <div className="crm-center-modal"><form onSubmit={(event) => { event.preventDefault(); setBusy(true); setError(null); void onCreate({ ...form, pipelineId: Number(form.pipelineId), corretorId: form.corretorId || null }).catch((reason) => setError(reason instanceof Error ? reason.message : "Não foi possível criar o lead.")).finally(() => setBusy(false)); }}><header><div><span>NOVO ATENDIMENTO</span><h2>Cadastrar lead</h2><p>O negócio será criado automaticamente na primeira etapa.</p></div><button type="button" onClick={onClose}>×</button></header>{error && <div className="modal-error">{error}</div>}<div className="create-grid"><label>Nome<input required autoFocus value={form.nome} onChange={(event) => setForm({ ...form, nome: event.target.value })} placeholder="Nome do cliente" /></label><label>Telefone<input required value={form.telefone} onChange={(event) => setForm({ ...form, telefone: event.target.value })} placeholder="(11) 99999-9999" /></label><label>E-mail<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label>Origem<input value={form.origem} onChange={(event) => setForm({ ...form, origem: event.target.value })} /></label><label>Funil<select required value={form.pipelineId} onChange={(event) => setForm({ ...form, pipelineId: event.target.value })}>{pipelines.map((pipeline) => <option value={pipeline.id} key={pipeline.id}>{pipeline.nome}</option>)}</select></label><label>Responsável<select value={form.corretorId} onChange={(event) => setForm({ ...form, corretorId: event.target.value })}><option value="">Distribuição automática</option>{brokers.map((broker) => <option value={broker.id} key={broker.id}>{broker.nome}</option>)}</select></label></div><footer><button type="button" onClick={onClose}>Cancelar</button><button className="crm-primary" disabled={busy} type="submit">{busy ? "Criando..." : "Criar lead"}</button></footer></form></div>;
}

...
export function CampaignWorkspace({ accessToken }: { accessToken: string }) {
  ...
  async function saveGenerated() {
    ...
    setGenBusy(true);
    try {
      const grupo = genForm.intention.trim() || "Variações comerciais";
      for (let i = 0; i < genResults.length; i++) {
        const response = await fetch("/api/approaches", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "createApproach", name: `${grupo} ${i + 1}`, empreendimentoId: genForm.productId, grupo, messages: [{ name: "send-text-message", options: { text: genResults[i] }, texto: genResults[i] }] }) });
        if (!response.ok) {
          const payload = await response.json().catch(() => ({})) as { error?: string };
          throw new Error(payload.error || `Falha ao salvar a variação ${i + 1}.`);
        }
      }
      await load();
      setNotice(`${genResults.length} abordagens salvas no grupo "${grupo}" do produto ${prod.nome}.`);
      setGenOpen(false); setGenResults([]);
    } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Não foi possível salvar as abordagens geradas."); }
  ...
  return <div className="campaign-workspace">
    <header className="workspace-top"><div><h1>Disparos</h1><p>Campanhas de mensagens por segmento, com cadência e abordagens reutilizáveis.</p></div></header>
    {loading ? <div className="workspace-loading">Carregando campanhas...</div> : error ? <div className="workspace-error">{error}<button type="button" onClick={() => void load()}>Tentar novamente</button></div> : <section className="campaign-main" aria-label="Campanhas">

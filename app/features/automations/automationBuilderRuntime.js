/* eslint-disable */
/* Runtime operacional do construtor, encapsulado como módulo da feature. */

export const AUTOMATION_EXPORT_SCHEMA = 'apecerto-automation/v1';

const SENSITIVE_EXPORT_KEY = /(?:authorization|api[_-]?key|token|password|passwd|senha|secret|bearer|credential)/i;

function cloneWithoutSecrets(value, key = '') {
 if (key && SENSITIVE_EXPORT_KEY.test(key)) return undefined;
 if (Array.isArray(value)) return value.map(item => cloneWithoutSecrets(item)).filter(item => item !== undefined);
 if (value && typeof value === 'object') {
  const clean = {};
  Object.entries(value).forEach(([childKey, childValue]) => {
   if (['__proto__','prototype','constructor'].includes(childKey)) return;
   const next = cloneWithoutSecrets(childValue, childKey);
   if (next !== undefined) clean[childKey] = next;
  });
  return clean;
 }
 return value;
...
export function createAutomationExport(map, metadata = {}, exportedAt = new Date().toISOString()) {
 const safeMap = cloneWithoutSecrets(map && typeof map === 'object' ? map : {});
 return {
  schema: AUTOMATION_EXPORT_SCHEMA,
  version: 1,
  exportedAt,
  automation: {
   name: String(metadata.name || (safeMap.automation && safeMap.automation.name) || 'Automação importada'),
   group: metadata.group == null ? null : String(metadata.group),
   map: safeMap,
  },
 };
}

export function parseAutomationImport(input) {
 let documentValue;
 try { documentValue = typeof input === 'string' ? JSON.parse(input) : input; }
 catch (_error) { throw new Error('O arquivo não contém um JSON válido.'); }
 if (!documentValue || typeof documentValue !== 'object' || Array.isArray(documentValue)) throw new Error('O JSON da automação precisa ser um objeto.');
 if (documentValue.schema && documentValue.schema !== AUTOMATION_EXPORT_SCHEMA) throw new Error('Versão de automação desconhecida: '+documentValue.schema+'.');
 const wrapped = documentValue.schema === AUTOMATION_EXPORT_SCHEMA;
 if (wrapped && Number(documentValue.version) !== 1) throw new Error('Versão de automação não suportada: '+documentValue.version+'.');
 const sourceMap = wrapped ? documentValue.automation && documentValue.automation.map : (documentValue.map || documentValue);
 if (!sourceMap || typeof sourceMap !== 'object' || Array.isArray(sourceMap)) throw new Error('O JSON não contém um mapa de automação.');
 const safeMap = cloneWithoutSecrets(sourceMap);
 const automation = safeMap.automation && typeof safeMap.automation === 'object' ? safeMap.automation : null;
 const blocks = automation && Array.isArray(automation.blocks) ? automation.blocks : [];
 if (!blocks.length) throw new Error('O JSON não contém nenhum bloco executável.');
 if (blocks.length > 500) throw new Error('O JSON ultrapassa o limite de 500 blocos.');
 const ids = new Set();
 blocks.forEach((block, index) => {
  if (!block || typeof block !== 'object' || Array.isArray(block)) throw new Error('Bloco inválido na posição '+(index+1)+'.');
  const id = String(block.id == null ? '' : block.id).trim();
  const type = String(block.type || '').trim();
  if (!id || id.length > 80) throw new Error('Bloco sem ID válido na posição '+(index+1)+'.');
  if (ids.has(id)) throw new Error('O JSON contém IDs repetidos: '+id+'.');
  if (!/^[a-z0-9][a-z0-9-]*$/i.test(type)) throw new Error('Tipo de bloco inválido na posição '+(index+1)+'.');
  if (block.options != null && (typeof block.options !== 'object' || Array.isArray(block.options))) throw new Error('Configuração inválida no bloco '+id+'.');
  ids.add(id);
 });
 const editor = safeMap.editor;
 if (editor != null && (typeof editor !== 'object' || Array.isArray(editor))) throw new Error('A camada visual do JSON é inválida.');
 const name = String((wrapped && documentValue.automation && documentValue.automation.name) || automation.name || 'Automação importada').trim() || 'Automação importada';
 const groupValue = wrapped && documentValue.automation ? documentValue.automation.group : null;
 return { map: safeMap, name, group: groupValue == null ? null : String(groupValue) };
}

export function findCollisionFreePosition(nodesInput, desired, nodeSize = { width: 340, height: 172 }, gap = 48) {
 const nodes = Object.values(nodesInput && typeof nodesInput === 'object' ? nodesInput : {});
 const start = { x: Math.round(Number(desired && desired.x) || 0), y: Math.round(Number(desired && desired.y) || 0) };
 const width = Math.max(120, Number(nodeSize.width) || 340), height = Math.max(80, Number(nodeSize.height) || 172);
 const overlaps = point => nodes.some(node => {
  const x = Number(node.x) || 0, y = Number(node.y) || 0;
  return point.x < x + width + gap && point.x + width + gap > x && point.y < y + height + gap && point.y + height + gap > y;
 });
 if (!overlaps(start)) return start;
 for (let row = 1; row <= 40; row += 1) {
  const below = { x: start.x, y: start.y + row * (height + gap) };
  if (!overlaps(below)) return below;
  const right = { x: start.x + row * (width + gap), y: start.y };
  if (!overlaps(right)) return right;
 }
 return { x: start.x + width + gap, y: start.y + height + gap };
}

export function reconcileAutomationMap(inputMap) {
 const mapa=inputMap&&typeof inputMap==='object'?inputMap:{};
 const editor=mapa.editor&&typeof mapa.editor==='object'?mapa.editor:{};
 const automation=mapa.automation&&typeof mapa.automation==='object'?mapa.automation:{};
 const editorBlocks=editor.blocks&&typeof editor.blocks==='object'&&!Array.isArray(editor.blocks)?editor.blocks:{};
 const automationBlocks=Array.isArray(automation.blocks)?automation.blocks:[];
 const automationById={};
 automationBlocks.forEach(block=>{if(block&&block.id!=null)automationById[String(block.id)]=block;});
 const editorIds=Object.keys(editorBlocks),automationIds=Object.keys(automationById);
 return {
  editor,
  automation,
  editorBlocks,
  automationBlocks,
  automationById,
  blockIds:[...automationIds,...editorIds.filter(id=>!automationById[id])],
  recoveredRuntimeIds:automationIds.filter(id=>!editorBlocks[id]),
  editorOnlyIds:editorIds.filter(id=>!automationById[id])
 };
}

export function reconcileAutomationWires(editorWiresInput, automationBlocksInput) {
 const editorWires=Array.isArray(editorWiresInput)?editorWiresInput.filter(Boolean):[];
 const automationBlocks=Array.isArray(automationBlocksInput)?automationBlocksInput:[];
 const runtimeWires=[];
 const push=(from,port,to)=>{if(from&&port&&to)runtimeWires.push({from:String(from),port:String(port),to:String(to)});};
 automationBlocks.forEach(block=>{
  if(!block||block.id==null)return;const id=String(block.id),o=block.options||{};
  push(id,'out',o.nextBlockId);push(id,'true',o.trueNextBlockId);push(id,'false',o.falseNextBlockId);push(id,'err',o.errorNextBlockId);push(id,'timeout',o.timeoutNextBlockId);push(id,'respondeu',o.respondeuNextBlockId);push(id,'naoRespondeu',o.naoRespondeuNextBlockId);
  (Array.isArray(o.randomizers)?o.randomizers:[]).forEach(route=>push(id,route&&route.id,route&&route.nextBlockId));
  (Array.isArray(o.conditions)?o.conditions:[]).forEach(route=>push(id,route&&route.id,route&&route.trueNextBlockId));
 });
 const routeKey=wire=>String(wire.from)+'::'+String(wire.port);
 const runtimeKeys=new Set(runtimeWires.map(routeKey)),editorByKey=new Map(editorWires.map(wire=>[routeKey(wire),wire]));
 const editorOnly=editorWires.filter(wire=>!runtimeKeys.has(routeKey(wire)));
 return {
  wires:[...runtimeWires,...editorOnly],
  recoveredWireKeys:runtimeWires.filter(wire=>{const visual=editorByKey.get(routeKey(wire));return !visual||String(visual.to)!==String(wire.to);}).map(routeKey),
  editorOnlyWireKeys:editorOnly.map(routeKey)
 };
}

const ApeCertoAutomationBuilder = (function(){
"use strict";
...
function ico(n,s=16,c='currentColor',sw=1.8){const p={
 ...
 plus:'<path d="M12 5v14M5 12h14"/>',search:'<circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/>',x:'<path d="m18 6-12 12M6 6l12 12"/>',trash:'<path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/>',copy:'<rect x="9" y="9" width="12" height="12" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/>',edit:'<path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/>',
...
canvasEl.tabIndex=0;canvasEl.setAttribute('role','application');canvasEl.setAttribute('aria-label','Canvas do construtor. Use a roda do mouse para zoom, arraste o fundo para navegar e as setas para mover o bloco selecionado.');
const zoomLevel=document.createElement('span');zoomLevel.className='zoom-level';zoomLevel.id='zoomLevel';zoomLevel.setAttribute('aria-live','polite');document.querySelector('.zoombar').insertBefore(zoomLevel,document.getElementById('zoomFit'));

/* status bar */
const sb=document.createElement('div');
sb.className='connection-status';
sb.innerHTML='<span id="dot" class="connection-dot"></span><span id="stTxt">Conectando…</span>';
MAIN.appendChild(sb);
...
/* ---------- transform ---------- */
function applyTransform(){worldEl.style.transform=`translate(${view.x}px,${view.y}px) scale(${view.scale})`;zoomLevel.textContent=Math.round(view.scale*100)+'%';zoomLevel.title='Zoom atual: '+Math.round(view.scale*100)+'%';scheduleEdges();}
canvasEl.addEventListener('wheel',e=>{e.preventDefault();const f=e.deltaY<0?1.1:.9;const ns=Math.min(2,Math.max(.35,view.scale*f));const r=canvasEl.getBoundingClientRect();const mx=e.clientX-r.left,my=e.clientY-r.top;view.x=mx-(mx-view.x)*(ns/view.scale);view.y=my-(my-view.y)*(ns/view.scale);view.scale=ns;applyTransform();},{passive:false});
...
document.getElementById('zoomOut').onclick=()=>{view.scale=Math.max(.25,view.scale*.87);applyTransform();};
document.getElementById('zoomFit').onclick=()=>fitView(false);
function fitView(readable){const ns=Object.values(cur?cur.nodes:{});if(!ns.length){view.x=160;view.y=140;view.scale=.86;applyTransform();return;}const boxes=ns.map(n=>{const el=worldEl.querySelector(`.node[data-id="${n.id}"]`);return{x:n.x,y:n.y,w:(el&&el.offsetWidth)||360,h:(el&&el.offsetHeight)||170};});const minX=Math.min(...boxes.map(b=>b.x)),maxX=Math.max(...boxes.map(b=>b.x+b.w)),minY=Math.min(...boxes.map(b=>b.y)),maxY=Math.max(...boxes.map(b=>b.y+b.h));const cw=canvasEl.clientWidth,ch=canvasEl.clientHeight;const largura=Math.max(360,maxX-minX),altura=Math.max(170,maxY-minY);const s=Math.min(1,Math.min((cw-144)/largura,(ch-144)/altura));if(readable&&s<.78){const alvos=new Set((cur.wires||[]).map(w=>w.to)),inicio=ns.find(n=>n.type==='trigger')||ns.find(n=>!alvos.has(n.id))||ns[0];view.scale=.82;view.x=112-inicio.x*view.scale;view.y=Math.max(132,canvasEl.clientHeight/2-(inicio.y+85)*view.scale);}else{view.scale=Math.max(.25,s);view.x=(cw-largura*view.scale)/2-minX*view.scale;view.y=(ch-altura*view.scale)/2-minY*view.scale;}canvasEl.classList.toggle('overview-scale',!readable&&view.scale<.78);applyTransform();}
...
async function boot(){
 try{setStatus('Carregando…','#f59e0b');
  const [pi,stg,co,ins,au]=await Promise.all([
   ...
   sbGet('/instancias?select=id,nome,ativa,conectada,status_dapi,numero_conectado,corretor_id&order=id'),
...
/* ---------- abrir / hidratar ---------- */
function updateFlowMeta(){const el=document.getElementById('flowSub');if(!el||!cur)return;const blocks=Object.keys(cur.nodes||{}).length,wires=(cur.wires||[]).length;el.textContent=(cur.grupo?cur.grupo+' · ':'')+blocks+' '+(blocks===1?'bloco':'blocos')+' · '+wires+' '+(wires===1?'conexão':'conexões')+(cur.ativa===false?' · inativa':'');}
async function openAutomacao(id){
 ...
 try{setStatus('Abrindo…','#f59e0b');
  const rows=await sbGet('/automacoes?id=eq.'+id+'&select=id,nome,grupo,ativa,status,publicado_em,arquivada,mapa,mapa_rascunho,titulos,versao_publicada_id,webhook_token,webhook_token_enforced');const row=rows[0];
  if(!row){toast('Automação não encontrada','err');return;}
  cur=hydrate(row);selectedId=null;dirty=false;
  if(_ctx&&_ctx.onAutomationOpened){try{_ctx.onAutomationOpened({id:cur.id,nome:cur.nome,grupo:cur.grupo});}catch(_e){}}
...
function hydrate(row){
 const mapa=row.mapa_rascunho||row.mapa||{},reconciled=reconcileAutomationMap(mapa),ed=reconciled.editor,au=reconciled.automation;
 const optById=reconciled.automationById;
 const edB=reconciled.editorBlocks,ids=reconciled.blockIds;
 const nodes={},titulos=(row.titulos&&typeof row.titulos==='object')?row.titulos:{};
 ids.forEach(id=>{const eb=edB[id]||{},ab=optById[id]||{};const type=ab.type||FAM2TYPE[eb.fam]||'action';
  ...
  nodes[id]={id,type,title:String(titulos[id]||''),sub:eb.sub||'',x:(eb.x??(ab.presentation&&ab.presentation.x)??80),y:(eb.y??(ab.presentation&&ab.presentation.y)??80),note:eb.note||'',ramos,opts:o,sourceBlockId:ab.sourceBlockId||(eb.extra&&eb.extra.sourceBlockId)||undefined};
 });
 const wireIntegrity=reconcileAutomationWires(ed.wires,au.blocks);
 let wires=wireIntegrity.wires;
 wires=wires.filter(w=>nodes[w.from]&&nodes[w.to]);
 return {id:row.id,nome:row.nome,grupo:row.grupo,ativa:row.ativa,status:row.status||'publicado',publicado_em:row.publicado_em,arquivada:!!row.arquivada,titulos,versao_publicada_id:row.versao_publicada_id,webhook_token:row.webhook_token||'',webhook_token_enforced:row.webhook_token_enforced===true,name:au.name||row.nome,provider:au.provider||'apecerto-erp',anotacoes:au.anotacoes||[],uid:ed.uid||100,notes:ed.notes||{},nodes,wires,integrity:{recoveredRuntimeIds:reconciled.recoveredRuntimeIds,editorOnlyIds:reconciled.editorOnlyIds,recoveredWireKeys:wireIntegrity.recoveredWireKeys,editorOnlyWireKeys:wireIntegrity.editorOnlyWireKeys}};
}
...
function corretoresDistribuidosNoFluxo(blockId){
 const nomes=new Set();
 if(!cur)return [];
 let permitidos=null;
 if(blockId){
  permitidos=new Set();const fila=[blockId];
 ...
 Object.values(cur.nodes||{}).filter(n=>(!permitidos||permitidos.has(n.id))&&(n.type==='distribution-simple'||n.type==='distribution')).forEach(n=>{
  const d=(n.opts&&n.opts.distribuicao)||{};
  (d.items||[]).filter(x=>x.on!==false&&(+x.peso||0)>0).forEach(x=>nomes.add(String(x.corretor||'').trim().toLowerCase()));
 });
 return (ref.corretores||[]).filter(c=>nomes.has(String(c.nome||'').trim().toLowerCase()));
}
function instanciaAutomacaoValida(i,corretorId){
 return +i.corretor_id===+corretorId&&i.ativa!==false&&i.conectada===true&&i.status_dapi==='connected';
}
function rotasInstanciaIniciais(){
 const rotas={};
 corretoresDistribuidosNoFluxo().forEach(c=>{
  const opcoes=(ref.instancias||[]).filter(i=>instanciaAutomacaoValida(i,c.id));
  if(opcoes.length===1)rotas[String(c.id)]=+opcoes[0].id;
 });
 return rotas;
}
...
function nodeSummary(n){
 const o=n.opts||{};
 if(n.type==='trigger'){const t=(o.triggers||[])[0]||{};return (TRIGGERS.find(x=>x[0]===t.name)||[,t.name||'Escolha o gatilho'])[1];}
 if(n.type==='field-operation'){const total=(o.fieldOperations||[]).length;return total?total+' '+(total===1?'mapeamento configurado':'mapeamentos configurados'):'Nenhum mapeamento configurado';}
 if(n.type==='condition'){const itens=o.conditions||[];if(!itens.length)return'Nenhuma regra configurada';const nome=(condMeta(itens[0].name)||[])[1]||'Condição';return itens.length===1?nome:nome+' + '+(itens.length-1)+' regra(s)';}
 if(n.type==='action'){const itens=o.actions||[];if(!itens.length)return'Nenhuma ação configurada';const nome=(acaoMeta(itens[0].name)||[])[1]||'Ação';return itens.length===1?nome:nome+' + '+(itens.length-1)+' ação(ões)';}
 if(n.type==='randomizer'){const rs=n.ramos||[];return rs.length?rs.map(r=>(r.name||'Caminho')+' '+(+r.perc||0)+'%').join(' · '):'Nenhum caminho configurado';}
 if(n.type==='distribution-simple'||n.type==='distribution'){const itens=((o.distribuicao||{}).items||[]).filter(x=>x.on!==false&&(+x.peso||0)>0);return itens.length+' '+(itens.length===1?'corretor no rodízio':'corretores no rodízio');}
 if(n.type==='send-approach'){const total=(o.abordagemIds||[]).length,grupo=String(o.abordagemGrupo||'').trim();return (grupo||'Sem grupo')+' · '+total+' '+(total===1?'abordagem':'abordagens');}
 if(n.type==='resposta')return'Aguardar até '+(o.janelaValor||12)+' '+(o.janelaUnidade||'horas');
 if(n.type==='time')return o.wait_type?('Aguardar '+(o.valor||0)+' '+(o.unidade||'minutos')):'Escolha a duração da espera';
 if(n.type==='ai-agent'){const ag=(ref.agentes||[]).find(a=>+a.id===+o.agenteId);return ag?('Analisar com '+ag.nome):'Escolha o agente de IA';}
 return'Clique para revisar a configuração';
}
function compactPorts(n){
 if(n.type==='trigger')return portRow('out','Quando acontecer','ok');
 if(n.type==='condition')return(n.opts.conditions||[]).filter(c=>c&&c.id).map(c=>portRow(c.id,'Regra: '+((condMeta(c.name)||[])[1]||'condição'),'branch')).join('')+portRow('true','Todas verdadeiras','branch')+portRow('false','Nenhuma atendida','err');
 if(n.type==='randomizer')return(n.ramos||[]).filter(r=>r&&r.id).map(r=>portRow(r.id,(r.name||'Caminho')+' · '+(+r.perc||0)+'%','branch')).join('');
 if(n.type==='resposta')return portRow('respondeu','Respondeu','branch')+portRow('naoRespondeu','Não respondeu','err');
 if(n.type==='time')return portRow('out','Continuar','ok');
 if(n.type==='distribution')return portRow('respondeu','Respondeu','branch')+portRow('naoRespondeu','Não respondeu','err')+portRow('out','Sucesso','ok')+portRow('err','Ninguém disponível','err');
 if(n.type==='chat')return portRow('out','Sucesso','ok')+portRow('timeout','Tempo esgotado','err')+portRow('err','Erro','err');
 if(['action','field-operation','distribution-simple','send-approach','ai-agent','api'].includes(n.type))return portRow('out','Sucesso','ok')+portRow('err','Erro','err');
 return portRow('out','Próximo passo','ok');
}
async function renameNode(n){
 const atual=String(n.title||'');const novo=prompt('Nome deste bloco (deixe vazio para voltar ao tipo):',atual);if(novo===null)return;
 const valor=novo.trim(),antes={...(cur.titulos||{})},depois={...antes};if(valor)depois[n.id]=valor;else delete depois[n.id];
 try{await sbPatch('/automacoes?id=eq.'+cur.id,{titulos:depois});cur.titulos=depois;n.title=valor;renderNodes();toast(valor?'Bloco nomeado':'Nome do bloco removido','ok');}
 catch(e){cur.titulos=antes;toast('Erro ao salvar o nome: '+e.message,'err');}
}
...
function renderNodes(){
 ...
 if(!cur)return;
 updateFlowMeta();
 Object.values(cur.nodes).sort((a,b)=>(a.x-b.x)||(a.y-b.y)).forEach((n,index)=>{
 const meta=TYPES[n.type]||TYPES.action,col=`var(${meta.cvar})`;
  const expanded=selectedId===n.id,title=String(n.title||'').trim(),displayTitle=title||nodeSummary(n);
  const recovered=!!(cur.integrity&&cur.integrity.recoveredRuntimeIds&&cur.integrity.recoveredRuntimeIds.includes(n.id));
  const editorOnly=!!(cur.integrity&&cur.integrity.editorOnlyIds&&cur.integrity.editorOnlyIds.includes(n.id));
  const integrityBadge=recovered?'<span class="node-integrity recovered">RECUPERADO</span>':(editorOnly?'<span class="node-integrity editor-only">REVISAR</span>':'');
  const el=document.createElement('div');
  el.className='node editable'+(expanded?' selected':'')+(recovered?' integrity-recovered':'')+(editorOnly?' integrity-editor-only':'');el.dataset.id=n.id;el.style.left=n.x+'px';el.style.top=n.y+'px';
  const inDot=n.type==='trigger'?'':'<span class="in-dot" data-in></span>';
  el.innerHTML=`<div class="bar" style="background:${col}"></div>${inDot}
   <div class="hd" data-drag><div class="nic" style="background:${col}22;color:${col}">${ico(meta.vis,16,col)}</div><button type="button" class="node-heading" data-node-toggle aria-expanded="${expanded}"><span class="node-type">${esc(meta.label)}</span><span class="ttl">${esc(displayTitle)}</span></button>${integrityBadge}<span class="node-order">${String(index+1).padStart(2,'0')}</span>
    <button type="button" data-renamenode class="node-icon-action" title="Dar um nome a este bloco" aria-label="Dar um nome a este bloco">${ico('edit',13)}</button><button type="button" data-delnode class="node-icon-action danger" title="Excluir bloco" aria-label="Excluir bloco">${ico('trash',13)}</button></div>
  ...
  el.querySelector('[data-drag]').addEventListener('mousedown',e=>{if(e.target.closest('button'))return;startDrag(e,n);});
  el.querySelector('[data-renamenode]').addEventListener('click',e=>{e.stopPropagation();void renameNode(n);});
  el.querySelector('[data-delnode]').addEventListener('click',e=>{e.stopPropagation();delNode(n.id);});
  if(expanded)bindBody(n,el);
  el.querySelectorAll('.port-dot').forEach(d=>d.addEventListener('mousedown',e=>startWire(e,n,d.dataset.port)));
  el.querySelectorAll('[data-foot] .st').forEach(st=>{st.title='Ver logs deste bloco';st.addEventListener('click',e=>{e.stopPropagation();const b=st.querySelector('[data-c]');openBlockLogs(n,(b&&b.dataset.c)||'ok');});});
 });
...
async function openBlockLogs(n,tab){
 ...
 function paint(active){
  ...
  const panel=showPanel('Logs do bloco · '+(TYPES[n.type]||TYPES.action).label,html);
  panel.querySelectorAll('[data-logtab]').forEach(b=>b.onclick=()=>paint(b.dataset.logtab));
 }
 paint(tab);
}
function markSel(){worldEl.querySelectorAll('.node').forEach(el=>el.classList.toggle('selected',el.dataset.id===selectedId));scheduleEdges();}
...
function bodyHtml(n){
 if(n.type==='trigger'){const t=(n.opts.triggers||[])[0]||{name:'json-http-request-trigger',options:{}},o=t.options||{};let ex='';
  ...
  if(t.name==='checagem-diaria-trigger')ex=`<div class="ne-lb">Reavaliar cada lead no máximo a cada</div><input class="ne-inp" type="number" min="1" max="168" data-tk="intervaloHoras" value="${esc(o.intervaloHoras||24)}"><div class="ne-lb">Aguardar após a última interação (minutos)</div><input class="ne-inp" type="number" min="0" max="1440" data-tk="atrasoInteracaoMinutos" value="${esc(o.atrasoInteracaoMinutos??10)}"><div class="ne-lb">Máximo por ciclo</div><input class="ne-inp" type="number" min="1" max="50" data-tk="limitePorCiclo" value="${esc(o.limitePorCiclo||12)}"><div style="font-size:10.5px;color:var(--ink-faint);margin-top:7px;line-height:1.45">A regra fica publicada neste bloco. O relógio apenas procura itens devidos e não pode reavaliar o mesmo card duas vezes no mesmo dia local.</div>`;
 ...
 if(n.type==='distribution'){if(!n.opts)n.opts={};const d=n.opts.distribuicao=n.opts.distribuicao||{items:[],onlineOnly:false,tambemNegocio:false};const its=d.items=d.items||[];
  ...
  return `<div style="font-size:11px;color:var(--ink-faint);padding:2px 0 6px;line-height:1.4">Distribui o lead entre os corretores por <b>peso</b> (roleta). Quem tem peso maior recebe mais leads.</div>`+
   ...
   (function(){const pr=Array.isArray(d.protecao)?d.protecao:['negociacao','visita_agendada','visita_realizada'];const opts=[['negociacao','Negociação ativa'],['venda','Negociação/venda (mapa antigo)'],['visita_agendada','Visita agendada'],['visita_realizada','Visita realizada'],['sempre','Sempre manter o dono (nunca redistribui)']];
 ...
 if(n.type==='distribution-simple'){if(!n.opts)n.opts={};
  ...
  const linhas=its.map((it,i)=>{const ativo=it.on!==false;const pz=+it.peso||0;
   ...
   return `<div class="ne-row" data-dsrow="${i}" style="padding:7px 9px"><div style="display:flex;align-items:center;gap:8px">`+
    ...
    `<span style="font-size:11px;font-weight:700;color:var(--brand);width:38px;text-align:right">${pct}%</span>`+
    `</div></div>`;}).join('');
  const pr=Array.isArray(d.protecao)?d.protecao:['negociacao','visita_agendada','visita_realizada'];
  const optsProt=[['negociacao','Negociação ativa'],['venda','Negociação/venda (mapa antigo)'],['visita_agendada','Visita agendada'],['visita_realizada','Visita realizada'],['sempre','Sempre manter o dono (nunca redistribui)']];
 ...
 if(n.type==='send-approach'){const o=n.opts||{};
  return `<div style="font-size:11.5px;color:var(--ink-soft);padding:2px 0 6px;line-height:1.45">Envia a abordagem pela instância <b>explicitamente escolhida abaixo</b> para o corretor dono do lead. Este bloco não distribui, não troca o corretor e não usa outra instância como alternativa.</div>`+
   (function(){const all=(ref.abordagens||[]).filter(a=>a.ativo!==false);const groups=[...new Set(all.map(a=>String(a.grupo||'').trim()))].sort((a,b)=>a.localeCompare(b,'pt-BR'));const chosen=String(o.abordagemGrupo||'').trim();const abList=all.filter(a=>String(a.grupo||'').trim()===chosen).sort((a,b)=>(+a.ordem||0)-(+b.ordem||0)||(+a.id||0)-(+b.id||0));const selAb=Array.isArray(o.abordagemIds)?o.abordagemIds.map(Number):[];const count=selAb.length;
    const rotas=(o.instanciaPorCorretor&&typeof o.instanciaPorCorretor==='object')?o.instanciaPorCorretor:{};const membros=corretoresDistribuidosNoFluxo(n.id);
    const linhasRotas=membros.map(c=>{const atual=+rotas[String(c.id)]||0;const opcoes=(ref.instancias||[]).filter(i=>+i.corretor_id===+c.id&&(instanciaAutomacaoValida(i,c.id)||+i.id===atual));
     return `<div class="ne-row" style="padding:7px 9px"><div style="font-size:11.5px;font-weight:650;color:var(--ink);margin-bottom:4px">${esc(c.nome)}</div><select class="ne-sel" data-sapinst="${c.id}"><option value="">— escolha a instância exata —</option>${opcoes.map(i=>`<option value="${i.id}" ${+i.id===atual?'selected':''}>${esc(i.nome)}${i.numero_conectado?' · '+esc(String(i.numero_conectado).slice(-4)):''}${instanciaAutomacaoValida(i,c.id)?'':' · indisponível'}</option>`).join('')}</select></div>`;
    }).join('');
    return `<div class="ne-lb" style="margin-top:0">1. Grupo de abordagens</div>`+
     ...
     `<div class="ne-lb">3. Instância exata por corretor</div>`+
     (membros.length?linhasRotas:`<div style="font-size:11px;color:#dc2626">Conecte antes um bloco de distribuição com corretores ativos.</div>`)+
     `<div style="font-size:10.5px;color:var(--ink-faint);margin-top:6px;line-height:1.45">A escolha fica salva e publicada neste bloco. Se a instância escolhida estiver indisponível, o envio falha pela saída de erro; <b>não existe troca automática</b>.</div>`+
 ...
 if(n.type==='field-operation'){const ops=n.opts.fieldOperations||[];
  return `<div style="font-size:11px;color:var(--ink-faint);padding:2px 0 2px;line-height:1.4">Escolha o <b>Destino</b> (campo do lead) e clique em <b>JSON</b> para escolher qual campo que chegou no webhook vai preenchê-lo. A saída deste módulo é entregue ao próximo bloco.</div>${ops.map((op,i)=>fieldOpRow(op,i)).join('')}<button class="ne-add" data-addfop>${ico('plus',14)} adicionar mapeamento</button><button class="ne-add" data-addrawjson>${ico('clip',14)} guardar JSON completo</button><button class="ne-add" data-addmetatracking>${ico('monitor',14)} registrar rastreamento Meta</button><button class="ne-add" data-addsitetracking>${ico('monitor',14)} registrar rastreamento do site</button>${portRow('out','Próximo passo','ok')}${portRow('err','Se ocorrer erro','err')}`;
 }
...
function fieldOpRow(op,i){const o=op.options||{};
 ...
 if(op.name==='sync-meta-attribution-field-operation'){
  return `<div class="ne-row" data-foprow="${i}"><div class="ne-rowh"><div style="flex:1;font-size:11.5px;font-weight:600;color:var(--ink)">Registrar rastreamento Meta</div><button class="ne-del" data-fopdel>${ico('trash',13)}</button></div><div style="font-size:11px;color:var(--ink-faint);line-height:1.4">Grava campanha, conjunto, anúncio e IDs recebidos no JSON. Só acontece quando esta operação está presente neste bloco.</div></div>`;
 }
 if(op.name==='sync-site-attribution-field-operation'){
  return `<div class="ne-row" data-foprow="${i}"><div class="ne-rowh"><div style="flex:1;font-size:11.5px;font-weight:600;color:var(--ink)">Registrar rastreamento do site</div><button class="ne-del" data-fopdel>${ico('trash',13)}</button></div><div style="font-size:11px;color:var(--ink-faint);line-height:1.4">Liga sessão, página, origem e campanha do site ao lead. Só acontece quando esta operação está presente neste bloco.</div></div>`;
 }
...
function condRow(c,i){const m=condMeta(c.name),cfg=m[3],o=c.options||{};let f='';
 if(cfg==='pipeline')f=`<select class="ne-sel" data-cf="pipeline">${pipeOpts(o.pipeline_id)}</select>`;
  ...
  else if(cfg==='momento')f=`<select class="ne-sel" data-cf="momento"><option value="">Selecione o momento…</option>${(ref.momentos||[]).map(m=>`<option value="${esc(m.slug)}"${o.momento===m.slug?' selected':''}>${esc(m.rotulo)}</option>`).join('')}</select><input class="ne-inp" data-cf="observacao" value="${esc(o.observacao||'')}" placeholder="Observação (opcional)">`;
...
function actRow(a,i){const m=acaoMeta(a.name),cfg=m[3],o=a.options||{};let f='';
 if(cfg==='pipeline_stage')f=`<select class="ne-sel" data-af="pipeline">${pipeOpts(o.pipeline_id)}</select><select class="ne-sel" data-af="etapa">${stageOpts(o.etapa_id,o.pipeline_id)}</select>`;
  ...
  [['aplicarMomento','Momento'],['aplicarEtapa','Etapa'],['aplicarTemperatura','Temperatura'],['aplicarAcao','Próxima ação e prazo'],['aplicarQualidade','Nota do atendimento']].map(([k,l])=>`<label style="display:flex;align-items:center;gap:7px;font-size:11.5px;color:var(--ink);cursor:pointer"><input type="checkbox" data-af-bool="${k}" ${o[k]!==false?'checked':''} style="width:15px;height:15px"> ${l}</label>`).join('');
...
function bindBody(n,el){
 ...
 if(n.type==='distribution'){n.opts.distribuicao=n.opts.distribuicao||{items:[],onlineOnly:true,tambemNegocio:false};const d=n.opts.distribuicao;d.items=d.items||[];
  ...
  qa('[data-distprot]').forEach(cb=>cb.onchange=()=>{const k=cb.dataset.distprot;d.protecao=Array.isArray(d.protecao)?d.protecao:['negociacao','visita_agendada','visita_realizada'];const ix=d.protecao.indexOf(k);if(cb.checked&&ix<0)d.protecao.push(k);else if(!cb.checked&&ix>=0)d.protecao.splice(ix,1);setDirty();});
 ...
 if(n.type==='distribution-simple'){n.opts.distribuicao=n.opts.distribuicao||{items:[],onlineOnly:true,tambemNegocio:true,regraElegibilidade:{modo:'dia-operacional',timezone:'America/Sao_Paulo',inicio:'09:30',fim:'18:30',presencaAtualPrioritaria:true,comparecimentoForaDoHorario:true,fimDeSemana:'todos-configurados'}};const d=n.opts.distribuicao;d.items=d.items||[];d.tambemNegocio=true;
  d.regraElegibilidade=d.regraElegibilidade||{modo:d.onlineOnly===false?'todos-configurados':'presenca-atual',timezone:'America/Sao_Paulo',inicio:'09:30',fim:'18:30',presencaAtualPrioritaria:true,comparecimentoForaDoHorario:true,fimDeSemana:'todos-configurados'};
  if(!Array.isArray(d.protecao))d.protecao=['negociacao','visita_agendada','visita_realizada'];
 ...
 if(n.type==='send-approach'){const o=n.opts=n.opts||{};
  ...
  qa('[data-sapinst]').forEach(sel=>sel.onchange=()=>{o.instanciaPorCorretor=(o.instanciaPorCorretor&&typeof o.instanciaPorCorretor==='object')?o.instanciaPorCorretor:{};const k=String(sel.dataset.sapinst);if(sel.value)o.instanciaPorCorretor[k]=+sel.value;else delete o.instanciaPorCorretor[k];setDirty();});}
 ...
 if(n.type==='field-operation'){n.opts.fieldOperations=n.opts.fieldOperations||[];
  ...
  const am=q('[data-addmetatracking]');if(am)am.onclick=e=>{e.stopPropagation();if(!n.opts.fieldOperations.some(op=>op.name==='sync-meta-attribution-field-operation'))n.opts.fieldOperations.push({name:'sync-meta-attribution-field-operation',group:'field',stepId:_uuid(),options:{}});setDirty();reNode(n);};
  const ast=q('[data-addsitetracking]');if(ast)ast.onclick=e=>{e.stopPropagation();if(!n.opts.fieldOperations.some(op=>op.name==='sync-site-attribution-field-operation'))n.opts.fieldOperations.push({name:'sync-site-attribution-field-operation',group:'field',stepId:_uuid(),options:{}});setDirty();reNode(n);};
...
/* ---------- arrastar nó ---------- */
function startDrag(e,n){e.preventDefault();e.stopPropagation();const sx=e.clientX,sy=e.clientY,ox=n.x,oy=n.y;
 const _dragEl=worldEl.querySelector(`.node[data-id="${n.id}"]`);
 selectedId=n.id;markSel();if(_dragEl)_dragEl.classList.add('dragging');
 function mv(ev){n.x=Math.round((ox+(ev.clientX-sx)/view.scale)/8)*8;n.y=Math.round((oy+(ev.clientY-sy)/view.scale)/8)*8;if(_dragEl){_dragEl.style.left=n.x+'px';_dragEl.style.top=n.y+'px';}scheduleEdges();}
 function up(){window.removeEventListener('mousemove',mv);window.removeEventListener('mouseup',up);if(_dragEl)_dragEl.classList.remove('dragging');setDirty();}
...
function drawEdges(){if(!cur){edgesEl.innerHTML='';return;}const P=[];
 cur.wires.forEach(w=>{const f=worldEl.querySelector(`.node[data-id="${w.from}"] .port-dot[data-port="${w.port}"]`),t=worldEl.querySelector(`.node[data-id="${w.to}"] [data-in]`);if(!f||!t)return;const a=dotWorld(f),b=dotWorld(t);const failure=w.port==='err'||w.port==='timeout',col=failure?'var(--err)':(w.port==='true'?'var(--c-condition)':(w.port==='false'?'#f59e0b':'var(--brand)'));P.push(`<path d="${orthPath(a,b)}" stroke="${col}" stroke-width="2.4" fill="none" ${failure?'stroke-dasharray="7 5"':''} stroke-linecap="round" opacity=".94"/>`);});
...
/* ---------- criar bloco ---------- */
function addNode(type,x,y){const id='b'+(cur.uid++),position=findCollisionFreePosition(cur.nodes,{x:x??140,y:y??160});const base={id,type,sub:'',x:position.x,y:position.y,note:'',ramos:[],opts:{}};
...
function organizeHorizontal(){
 if(!cur||!Object.keys(cur.nodes||{}).length)return false;
 selectedId=null;renderNodes();
 const ids=Object.keys(cur.nodes),saidas={};ids.forEach(id=>saidas[id]=[]);
 (cur.wires||[]).filter(w=>cur.nodes[w.from]&&cur.nodes[w.to]).forEach(w=>saidas[w.from].push(w.to));
 const alvos=new Set((cur.wires||[]).map(w=>w.to)),raizes=ids.filter(id=>!alvos.has(id));
 const coluna={},fila=[...(raizes.length?raizes:[ids[0]])],visitados=new Set(fila);fila.forEach(id=>coluna[id]=0);
 let guarda=0;while(fila.length&&guarda++<6000){const id=fila.shift();(saidas[id]||[]).forEach(destino=>{if(visitados.has(destino))return;visitados.add(destino);coluna[destino]=(coluna[id]||0)+1;fila.push(destino);});}
 ids.forEach(id=>{if(coluna[id]==null)coluna[id]=0;});
 const grupos={};ids.slice().sort((a,b)=>(coluna[a]-coluna[b])||((cur.nodes[a].y||0)-(cur.nodes[b].y||0))).forEach(id=>(grupos[coluna[id]]=grupos[coluna[id]]||[]).push(id));
 Object.keys(grupos).forEach(c=>{let y=96;grupos[c].forEach(id=>{const el=worldEl.querySelector(`.node[data-id="${id}"]`);cur.nodes[id].x=96+(+c)*470;cur.nodes[id].y=y;y+=((el&&el.offsetHeight)||150)+56;});});
 setDirty();renderNodes();fitView(true);toast('Fluxo organizado. Clique em Salvar para confirmar.','ok');return true;
}
function organizeVertical(){
 ...
 (cur.wires||[]).filter(w=>cur.nodes[w.from]&&cur.nodes[w.to]).forEach(w=>saidas[w.from].push(w.to));
 const alvos=new Set((cur.wires||[]).map(w=>w.to)),raizes=ids.filter(id=>!alvos.has(id));
 const linha={},fila=[...(raizes.length?raizes:[ids[0]])],visitados=new Set(fila);fila.forEach(id=>linha[id]=0);
 let guarda=0;while(fila.length&&guarda++<6000){const id=fila.shift();(saidas[id]||[]).forEach(destino=>{if(visitados.has(destino))return;visitados.add(destino);linha[destino]=(linha[id]||0)+1;fila.push(destino);});}
 ids.forEach(id=>{if(linha[id]==null)linha[id]=0;});
 const grupos={};ids.slice().sort((a,b)=>(linha[a]-linha[b])||((cur.nodes[a].x||0)-(cur.nodes[b].x||0))).forEach(id=>(grupos[linha[id]]=grupos[linha[id]]||[]).push(id));
 Object.keys(grupos).forEach(r=>{let x=96;grupos[r].forEach(id=>{const el=worldEl.querySelector(`.node[data-id="${id}"]`);cur.nodes[id].x=x;cur.nodes[id].y=96+(+r)*300;x+=((el&&el.offsetWidth)||340)+56;});});
 setDirty();renderNodes();fitView(true);toast('Fluxo organizado na vertical. Clique em Salvar para confirmar.','ok');return true;
}
...
/* ---------- simular + monitor ---------- */
async function simular(){if(!cur){toast('Abra uma automação');return;}
 const issues=computeIssues(),erros=issues.filter(x=>x.lvl==='erro'),alertas=issues.filter(x=>x.lvl==='alerta');
 const inicio=Object.values(cur.nodes).find(n=>n.type==='trigger'),vistos=new Set(),fila=inicio?[inicio.id]:[] ,ordem=[];
 while(fila.length){const id=fila.shift();if(vistos.has(id)||!cur.nodes[id])continue;vistos.add(id);ordem.push(cur.nodes[id]);(cur.wires||[]).filter(w=>w.from===id).forEach(w=>fila.push(w.to));}
 const passos=ordem.map((n,i)=>`<article class="preview-step"><span>${String(i+1).padStart(2,'0')}</span><div><b>${esc(n.title||(TYPES[n.type]||TYPES.action).label)}</b><p>${esc(nodeSummary(n))}</p></div></article>`).join('');
 const estado=erros.length?`<div class="preview-result error"><b>${erros.length} erro(s) impedem a publicação</b><span>Abra Validar para navegar até cada problema.</span></div>`:`<div class="preview-result ok"><b>Estrutura pronta para teste real</b><span>${alertas.length?alertas.length+' alerta(s) não impeditivo(s).':'Nenhum erro ou alerta estrutural.'}</span></div>`;
 showPanel('Prévia segura — '+cur.nome,`<div class="preview-safe"><p><b>Nenhum lead, mensagem ou negócio será alterado.</b> Esta prévia valida o desenho e mostra a ordem alcançável a partir do gatilho.</p>${estado}<div class="preview-list">${passos||'<p>Nenhum passo alcançável.</p>'}</div></div>`);
}
let monRows=[];
...
function showPanel(title,html){let p=ROOT.querySelector('#rpanel');if(!p){p=document.createElement('aside');p.id='rpanel';p.className='runtime-panel';p.setAttribute('aria-label',title);MAIN.appendChild(p);}p.innerHTML=`<header><h4>${esc(title)}</h4><button type="button" data-rpclose aria-label="Fechar">${ico('x',18,'var(--ink-faint)')}</button></header><div class="runtime-panel-body">${html}</div>`;p.querySelector('[data-rpclose]').onclick=()=>p.remove();return p;}

/* ---------- toolbar / sidebar ---------- */
document.getElementById('tbSave').onclick=save;
function downloadAutomation(map,metadata){const payload=createAutomationExport(map,metadata),b=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(b),a=document.createElement('a');a.href=url;a.download=(metadata.name||'automacao').replace(/[^a-z0-9_-]+/gi,'-').replace(/^-|-$/g,'').toLowerCase()+'.apecerto.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
document.getElementById('tbDownload').onclick=()=>{if(!cur)return;downloadAutomation(compile(),{name:cur.nome,group:cur.grupo});toast('JSON seguro exportado','ok');};
...
function duplicateSelectedNode(){if(!cur||!selectedId)return false;const s=cur.nodes[selectedId];const id='b'+(cur.uid++);const c=JSON.parse(JSON.stringify(s));c.id=id;
 // duplica PARA BAIXO, na mesma coluna (mesma orientação)
 const srcEl=worldEl.querySelector(`.node[data-id="${selectedId}"]`);const h=srcEl?srcEl.offsetHeight:180;const position=findCollisionFreePosition(cur.nodes,{x:s.x,y:s.y+h+48},{width:srcEl?srcEl.offsetWidth:340,height:h});c.x=position.x;c.y=position.y;
 if(c.ramos)c.ramos=c.ramos.map(r=>({...r,id:'r'+(cur.uid++)}));cur.nodes[id]=c;selectedId=id;setDirty();renderNodes();markSel();return true;}
document.getElementById('tbDup').onclick=duplicateSelectedNode;
...
document.getElementById('tbNext').onclick=()=>{if(!cur)return;const ids=Object.keys(cur.nodes);if(!ids.length)return;const i=ids.indexOf(selectedId);selectedId=ids[(i+1)%ids.length];renderNodes();const n=cur.nodes[selectedId];view.x=canvasEl.clientWidth/2-(n.x+170)*view.scale;view.y=canvasEl.clientHeight/2-(n.y+90)*view.scale;applyTransform();};
(function extra(){const tb=ROOT.querySelector('.toolbar');const sep=document.createElement('div');sep.className='tb-sep';tb.appendChild(sep);const s=document.createElement('button');s.className='tb-btn tb-preview';s.title='Prévia segura';s.setAttribute('aria-label','Prévia segura');s.innerHTML=ico('play',16)+'<span>Prévia</span>';s.onclick=simular;tb.appendChild(s);})();

const PALETTE_GROUPS=[['Entrada e lógica',['trigger','condition','time','resposta']],['Dados e roteamento',['field-operation','randomizer','distribution-simple']],['Relacionamento e IA',['send-approach','action','ai-agent']]];
const TYPE_DESCRIPTIONS={trigger:'Define quando o fluxo começa',condition:'Cria caminhos por regras',time:'Pausa por uma duração',resposta:'Separa respondeu e não respondeu','field-operation':'Mapeia e normaliza dados',randomizer:'Divide por percentuais','distribution-simple':'Distribui entre corretores','send-approach':'Envia abordagem do produto',action:'Executa mudanças no CRM','ai-agent':'Analisa com um agente'};
let paletteQuery='';
function renderPalette(){const body=document.getElementById('paletteBody'),query=paletteQuery.trim().toLowerCase();const sections=PALETTE_GROUPS.map(([label,types])=>{const items=types.filter(type=>PUBLISHABLE_TYPES.has(type)).map(type=>[type,TYPES[type]]).filter(([type,m])=>!query||(m.label+' '+(TYPE_DESCRIPTIONS[type]||'')).toLowerCase().includes(query));if(!items.length)return'';return `<section class="palette-section"><h4>${esc(label)}</h4>${items.map(([t,m])=>`<button class="pal-item" data-type="${t}" data-transfer-type="${t}" draggable="true"><span class="ico">${ico(m.vis,17,`var(${m.cvar})`)}</span><span class="pal-copy"><b>${esc(m.label)}</b><small>${esc(TYPE_DESCRIPTIONS[t]||'')}</small></span><span class="pal-drag" aria-hidden="true">⋮⋮</span></button>`).join('')}</section>`;}).join('');body.innerHTML=`<label class="palette-search"><span>${ico('search',14,'currentColor')}</span><input type="search" placeholder="Pesquisar módulos" value="${esc(paletteQuery)}" aria-label="Pesquisar módulos"></label><div class="palette-scroll">${sections||'<p class="palette-empty">Nenhum módulo encontrado.</p>'}</div>`;
 const search=body.querySelector('.palette-search input');search.oninput=e=>{paletteQuery=e.target.value;renderPalette();const next=body.querySelector('.palette-search input');next.focus();next.setSelectionRange(paletteQuery.length,paletteQuery.length);};
 body.querySelectorAll('[data-type]').forEach(b=>{b.onclick=()=>{if(!cur){toast('Abra ou crie uma automação');return;}addNode(b.dataset.type,(canvasEl.clientWidth/2-view.x)/view.scale,(canvasEl.clientHeight/2-view.y)/view.scale);};b.ondragstart=e=>{e.dataTransfer.setData('application/x-apecerto-block',b.dataset.transferType);e.dataTransfer.effectAllowed='copy';};});}
canvasEl.addEventListener('dragover',e=>{if(e.dataTransfer&&Array.from(e.dataTransfer.types||[]).includes('application/x-apecerto-block')){e.preventDefault();e.dataTransfer.dropEffect='copy';canvasEl.classList.add('drop-ready');}});
canvasEl.addEventListener('dragleave',e=>{if(!canvasEl.contains(e.relatedTarget))canvasEl.classList.remove('drop-ready');});
...
async function importAutomationFile(file){
 if(!file)return;if(file.size>4*1024*1024){toast('O JSON ultrapassa o limite de 4 MB.','err');return;}
 try{
  const parsed=parseAutomationImport(await file.text()),suggested=parsed.name+(parsed.name.toLowerCase().includes('importada')?'':' (importada)'),name=prompt('Nome da nova automação:',suggested);
  if(name===null||!name.trim())return;
  const group=prompt('Grupo/produto (opcional):',parsed.group||'');if(group===null)return;
  const map=JSON.parse(JSON.stringify(parsed.map));map.automation=map.automation||{};map.automation.name=name.trim();
  const rows=await sbPost('/automacoes',{nome:name.trim(),grupo:group.trim()||null,ativa:false,status:'rascunho',mapa:map,mapa_rascunho:map});const nv=rows[0];
  ref.automacoes.push({id:nv.id,nome:nv.nome,grupo:nv.grupo,ativa:false,status:'rascunho',arquivada:false});renderSidebar(document.getElementById('sbSearch').value);await openAutomacao(nv.id);toast('Importada como novo rascunho inativo. Revise e valide antes de publicar.','ok');
 }catch(e){toast('Não foi possível importar: '+e.message,'err');}
}
function setupImportButton(){
 const before=document.getElementById('btnAbordagens'),button=document.createElement('button'),input=document.createElement('input');button.type='button';button.className='sb-add sb-secondary';button.id='btnImportAutomation';button.innerHTML=ico('attach',14)+' Importar JSON';button.title='Importar sempre como nova automação inativa';input.type='file';input.accept='.json,.apecerto.json,application/json';input.hidden=true;before.parentNode.insertBefore(button,before);before.parentNode.insertBefore(input,before);button.onclick=()=>{input.value='';input.click();};input.onchange=()=>void importAutomationFile(input.files&&input.files[0]);
}
...
function isEditableTarget(target){return !!(target&&target.closest&&target.closest('input,textarea,select,[contenteditable="true"]'));}
window.addEventListener('keydown',e=>{const k=e.key.toLowerCase(),command=e.ctrlKey||e.metaKey;if(command&&k==='z'){e.preventDefault();e.shiftKey?redo():undo();return;}if(command&&k==='y'){e.preventDefault();redo();return;}if(command&&k==='s'){e.preventDefault();save();return;}if(command&&k==='d'&&!isEditableTarget(e.target)){e.preventDefault();duplicateSelectedNode();return;}if(isEditableTarget(e.target))return;if((k==='delete'||k==='backspace')&&cur&&selectedId){e.preventDefault();delNode(selectedId);return;}if(k==='escape'){selectedId=null;renderNodes();closeMini();const panel=ROOT.querySelector('#rpanel');if(panel)panel.remove();return;}if(k==='0'&&cur){e.preventDefault();fitView(true);return;}if(cur&&selectedId&&['arrowup','arrowdown','arrowleft','arrowright'].includes(k)){e.preventDefault();const n=cur.nodes[selectedId],step=e.shiftKey?24:8;if(k==='arrowup')n.y-=step;if(k==='arrowdown')n.y+=step;if(k==='arrowleft')n.x-=step;if(k==='arrowright')n.x+=step;setDirty();renderNodes();}});
...
/* ---- validação ---- */
function centerBlock(id){const n=cur&&cur.nodes[id];if(!n)return;selectedId=id;renderNodes();view.x=canvasEl.clientWidth/2-(n.x+170)*view.scale;view.y=canvasEl.clientHeight/2-(n.y+90)*view.scale;applyTransform();}
...
function computeIssues(){
 ...
 const recovered=(cur.integrity&&cur.integrity.recoveredRuntimeIds)||[],editorOnly=(cur.integrity&&cur.integrity.editorOnlyIds)||[],recoveredWires=(cur.integrity&&cur.integrity.recoveredWireKeys)||[],editorOnlyWires=(cur.integrity&&cur.integrity.editorOnlyWireKeys)||[];
 if(recovered.length||editorOnly.length||recoveredWires.length||editorOnlyWires.length)add('alerta',null,'O desenho antigo estava divergente do mapa executado. O construtor preservou todos os módulos e recuperou as conexões executáveis. Revise os itens destacados antes de salvar.');
 ...
 list.forEach(n=>{
  ...
  if(n.type==='trigger'){const ts=n.opts.triggers||[];if(ts.length!==1)add('erro',n.id,'O bloco de início precisa ter exatamente um gatilho.');ts.forEach(t=>{if(!PUBLISHABLE_TRIGGERS.has(t.name))add('erro',n.id,'Gatilho não implementado: '+t.name+'.');if(t.name==='checagem-diaria-trigger'){const o=t.options||{},h=Number(o.intervaloHoras??24),a=Number(o.atrasoInteracaoMinutos??10),l=Number(o.limitePorCiclo??12);if(!Number.isFinite(h)||h<1||h>168)add('erro',n.id,'A checagem da Sara precisa de intervalo entre 1 e 168 horas.');if(!Number.isFinite(a)||a<0||a>1440)add('erro',n.id,'O atraso após interação precisa ficar entre 0 e 1440 minutos.');if(!Number.isFinite(l)||l<1||l>50)add('erro',n.id,'O lote da Sara precisa ficar entre 1 e 50 leads.');}});}
  ...
  if(n.type==='send-approach'){const ids=(n.opts.abordagemIds||[]).map(Number);if(!ids.length)add('erro',n.id,'Selecione pelo menos uma abordagem.');if(new Set(ids).size!==ids.length)add('erro',n.id,'A mesma abordagem foi selecionada mais de uma vez.');const group=String(n.opts.abordagemGrupo||'').trim();const allowed=new Set((ref.abordagens||[]).filter(a=>a.ativo!==false&&String(a.grupo||'').trim()===group).map(a=>+a.id));if(ids.some(id=>!allowed.has(id)))add('erro',n.id,'Há uma abordagem fora do grupo escolhido ou inativa.');const rotas=(n.opts.instanciaPorCorretor&&typeof n.opts.instanciaPorCorretor==='object')?n.opts.instanciaPorCorretor:{};const membros=corretoresDistribuidosNoFluxo(n.id);if(!membros.length)add('erro',n.id,'Enviar abordagem precisa receber um corretor de um bloco de distribuição.');membros.forEach(c=>{const iid=+rotas[String(c.id)]||0;const inst=(ref.instancias||[]).find(i=>+i.id===iid);if(!iid)add('erro',n.id,'Escolha a instância exata de '+c.nome+'.');else if(!inst||!instanciaAutomacaoValida(inst,c.id))add('erro',n.id,'A instância escolhida de '+c.nome+' não está conectada e disponível.');});}
...
function showValidation(I){
 ...
 const panel=showPanel('Validação — '+cur.nome, head + (I.length?[...erros,...alertas].map(rowH).join(''):'<div style="color:var(--ink-faint);font-size:12px">Tudo certo.</div>'));
 panel.querySelectorAll('[data-vgo]').forEach(b=>{if(b.dataset.vgo)b.onclick=()=>centerBlock(b.dataset.vgo);});
}
...
(function fase1btns(){
 ...
 const validar=mk('btnValidate','Validar automação','<circle cx="12" cy="12" r="9"/><path d="M8.5 12l2.2 2.2L15.5 9.5"/>',validate);validar.classList.add('tb-validate');validar.insertAdjacentHTML('beforeend','<span>Validar</span>');
 updUndo();
})();
...
function renderStateBadges(){
 ...
 if(!host){host=document.createElement('span');host.id='stateBadges';host.className='state-badges';sub.parentNode.appendChild(host);}
 const pub=(cur.status||'publicado')==='publicado',ativa=cur.ativa===true;
 const chip=(t,kind)=>`<span class="state-chip ${kind}">${t}</span>`;
 host.innerHTML=(cur.arquivada?chip('ARQUIVADA','archived'):'')+chip(pub?'PUBLICADO':'RASCUNHO',pub?'published':'draft')+chip(ativa?'ATIVO':'INATIVO',ativa?'active':'inactive');
}
function renderIntegrityBanner(){
 const title=ROOT.querySelector('.flow-title');if(!title||!cur)return;
 let banner=title.querySelector('#integrityBanner');if(!banner){banner=document.createElement('div');banner.id='integrityBanner';banner.className='integrity-banner';title.appendChild(banner);}
 const recovered=(cur.integrity&&cur.integrity.recoveredRuntimeIds)||[],editorOnly=(cur.integrity&&cur.integrity.editorOnlyIds)||[],recoveredWires=(cur.integrity&&cur.integrity.recoveredWireKeys)||[],editorOnlyWires=(cur.integrity&&cur.integrity.editorOnlyWireKeys)||[];
 if(!recovered.length&&!editorOnly.length&&!recoveredWires.length&&!editorOnlyWires.length){banner.hidden=true;banner.textContent='';return;}
 const parts=[];if(recovered.length)parts.push(recovered.length+' módulo(s) executado(s) recuperado(s)');if(editorOnly.length)parts.push(editorOnly.length+' módulo(s) exclusivo(s) do editor preservado(s)');if(recoveredWires.length)parts.push(recoveredWires.length+' conexão(ões) executável(is) recuperada(s)');
 banner.hidden=false;banner.innerHTML='<b>Proteção de integridade ativa</b><span>'+esc(parts.join(' · '))+'. Revise antes de salvar.</span>';
 banner.title='Executados recuperados: '+recovered.join(', ')+(editorOnly.length?' | Exclusivos do editor: '+editorOnly.join(', '):'')+(recoveredWires.length?' | Conexões recuperadas: '+recoveredWires.join(', '):'');
}
...
async function sbAction(k,id){const a=ref.automacoes.find(z=>z.id===id);try{
 ...
 else if(k==='exp'){const r=(await sbGet('/automacoes?id=eq.'+id+'&select=nome,grupo,mapa,mapa_rascunho'))[0];downloadAutomation(r.mapa_rascunho||r.mapa,{name:r.nome,group:r.grupo});toast('JSON seguro exportado','ok');}
...
async function publish(){if(!cur){toast('Abra uma automação');return;}
 ...
 const integrity=cur.integrity||{},hasReconciliation=(integrity.recoveredRuntimeIds||[]).length||(integrity.editorOnlyIds||[]).length||(integrity.recoveredWireKeys||[]).length||(integrity.editorOnlyWireKeys||[]).length;
 if(hasReconciliation&&!confirm('Este fluxo foi reconciliado para preservar módulos que estavam divergentes entre o editor e o motor. Você revisou todos os blocos e deseja publicar a estrutura reconciliada?'))return;
...
async function openVersions(){if(!cur){toast('Abra uma automação');return;}
 try{const vs=await sbGet('/automacao_versoes?automacao_id=eq.'+cur.id+'&select=id,versao,nome,observacao,criado_em&order=versao.desc');
  ...
  const panel=showPanel('Versões — '+cur.nome,html);
  panel.querySelectorAll('[data-vrest]').forEach(b=>b.onclick=()=>restoreVersion(+b.dataset.vrest));
  panel.querySelectorAll('[data-vcmp]').forEach(b=>b.onclick=()=>compareVersion(+b.dataset.vcmp));
 }catch(e){toast('Erro: '+e.message,'err');}}
async function restoreVersion(vid){if(!confirm('Restaurar esta versão no editor? (não publica automaticamente)'))return;
 try{const v=(await sbGet('/automacao_versoes?id=eq.'+vid+'&select=mapa'))[0];const t=hydrate({id:cur.id,nome:cur.nome,grupo:cur.grupo,ativa:cur.ativa,status:cur.status,publicado_em:cur.publicado_em,arquivada:cur.arquivada,titulos:cur.titulos,mapa:v.mapa});cur.nodes=t.nodes;cur.wires=t.wires;cur.uid=t.uid;renderNodes();fitView(true);setDirty();toast('Versão restaurada no editor','ok');
 }catch(e){toast('Erro: '+e.message,'err');}}
...
(function fase2btns(){const tb=ROOT.querySelector('.toolbar');const sep=document.createElement('div');sep.className='tb-sep';tb.appendChild(sep);
 function mk(id,title,inner,fn){const b=document.createElement('button');b.className='tb-btn';b.id=id;b.title=title;b.setAttribute('aria-label',title);b.innerHTML='<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">'+inner+'</svg>';b.onclick=fn;tb.appendChild(b);return b;}
 const publicar=mk('btnPublish','Publicar','<path d="M12 19V6"/><path d="m5 12 7-7 7 7"/>',publish);publicar.classList.add('tb-publish');publicar.insertAdjacentHTML('beforeend','<span>Publicar</span>');
 mk('btnVersions','Versões','<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>',openVersions);
})();
injectSidebarTabs();
...
/* INIT */
_ctx.__organizeHorizontal=organizeHorizontal;
_ctx.__organizeVertical=organizeVertical;
_ctx.__hasUnsavedChanges=()=>dirty;
setupImportButton();
renderPalette();
ROOT.querySelectorAll('button[title]').forEach(b=>{if(!b.getAttribute('aria-label'))b.setAttribute('aria-label',b.title);});
const saveButton=document.getElementById('tbSave');saveButton.classList.add('tb-save');saveButton.insertAdjacentHTML('beforeend','<span>Salvar</span>');
const toolbar=ROOT.querySelector('.toolbar');
function toolbarGroup(label,selectors){const group=document.createElement('div');group.className='toolbar-group';group.setAttribute('role','group');group.setAttribute('aria-label',label);group.dataset.label=label;selectors.forEach(selector=>{const element=ROOT.querySelector(selector);if(element)group.appendChild(element);});if(group.children.length)toolbar.appendChild(group);}
...
return {
  ...
  organizeHorizontal: function(){ return !!(_ctx&&_ctx.__organizeHorizontal&&_ctx.__organizeHorizontal()); },
  organizeVertical: function(){ return !!(_ctx&&_ctx.__organizeVertical&&_ctx.__organizeVertical()); },
  hasUnsavedChanges: function(){ return !!(_ctx&&_ctx.__hasUnsavedChanges&&_ctx.__hasUnsavedChanges()); }
};
})();

export default ApeCertoAutomationBuilder;

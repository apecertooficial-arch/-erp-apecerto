...
type PerformanceFeedback = { status?: "ok" | "restrito" | "indisponivel"; historico_total?: number; estruturados_total?: number; legados_total?: number; feedback_visita_min?: number; itens?: PerformanceFeedbackItem[] };
  ...
  return <div className="calendar-workspace" onMouseUp={() => finishDrag()} onMouseLeave={() => setDrag(null)}>
    ...
    {loading ? <div className="workspace-loading">Carregando agenda...</div> : error ? <div className="workspace-error">{error}<button type="button" onClick={() => void load()}>Tentar novamente</button></div> : <section className="calendar-main" aria-label="Agenda">
      ...
      {isAdmin && <section className="calendar-feedback-performance" aria-label="Qualidade dos feedbacks de visita">
        <header><div><small>QUALIDADE DOS FEEDBACKS</small><h2>Série 0–10 por corretor</h2><p>Somente respostas estruturadas recebem nota. O histórico legado continua preservado, sem avaliação retroativa.</p></div>{data.performance_feedback?.status === "ok" && <strong>{data.performance_feedback.estruturados_total ?? 0} avaliados</strong>}</header>
        {data.performance_feedback?.status === "ok" && (data.performance_feedback.estruturados_total ?? 0) > 0
          ? <div className="calendar-feedback-performance-lista">{(data.performance_feedback.itens ?? []).map((item) => <article key={item.corretor_id ?? item.corretor}><strong>{item.corretor}</strong><span><b>{item.nota_media.toFixed(1)}</b>/10 média</span><small>{item.feedbacks} feedback{item.feedbacks === 1 ? "" : "s"} · {item.dentro_prazo_percentual.toFixed(0)}% até {data.performance_feedback?.feedback_visita_min ?? "—"} min · resposta média {item.resposta_media_min} min</small></article>)}</div>

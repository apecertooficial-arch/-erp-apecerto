...
type Message = { id: string; conversa_id: string; direcao: string; tipo: string; conteudo: string | null; media_url?: string | null; raw?: unknown; criado_em: string | null };
...
export function LiveChatWorkspace({ accessToken }: { accessToken: string }) {
  ...
  return <div className="live-chat">
    ...
    <main>
      <aside className="conversation-list"><nav><button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")} type="button">Todas <b>{data?.conversations.length || 0}</b></button><button className={filter === "unanswered" ? "active" : ""} onClick={() => setFilter("unanswered")} type="button">Sem resposta <b>{unansweredCount}</b></button><button className={filter === "critical" ? "active" : ""} onClick={() => setFilter("critical")} type="button">Críticas <b>{criticalCount}</b></button></nav>{filtered.map((item) => { const person = contactById.get(item.contato_id); const last = data?.latest[item.id]; return <button className={item.id === selectedId ? "active" : ""} type="button" onClick={() => setSelectedId(item.id)} key={item.id}><span>{(person?.nome || "C").slice(0, 2).toUpperCase()}</span><div><strong>{person?.nome || person?.telefone || "Contato"}</strong><small>{last?.conteudo || (last?.tipo ? `◉ ${last.tipo}` : "Conversa iniciada")}</small><em>{item.ultima_msg_em ? when.format(new Date(item.ultima_msg_em)) : ""}</em></div></button>; })}{filtered.length === 0 && <div className="conversation-empty">Nenhuma conversa neste filtro.</div>}</aside>
      <section className="chat-thread">
        ...
        <div className="message-stream" ref={messageStream}>{messages.map((message) => <article className={isOutgoing(message.direcao) ? "out" : "in"} key={message.id}><MessageMedia message={message} /><span>{message.conteudo}</span><small>{message.criado_em ? when.format(new Date(message.criado_em)) : ""}</small></article>)}{messages.length === 0 && <div className="crm-empty-view">Nenhuma mensagem encontrada nesta instância.</div>}</div>
        <form className="chat-composer" onSubmit={(event) => { event.preventDefault(); void send(draft); }}><div className="composer-tools"><button className={recording ? "recording" : ""} type="button" title="Gravar áudio" onClick={() => void toggleRecording()}>{recording ? "■" : "🎤"}</button><button type="button" title="Enviar imagem, vídeo ou documento" onClick={() => fileInput.current?.click()}>📎</button><button type="button" title="Agendar mensagem" onClick={() => setScheduleOpen(true)}>🕓</button><button type="button" title="Enviar abordagem" onClick={() => setApproachOpen(true)}>🏡</button><input ref={fileInput} hidden type="file" accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" onChange={(event) => void upload(event.target.files?.[0])} /></div><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Escreva uma mensagem…" /><button disabled={busy || !draft.trim() || !instanceId} type="submit">➤</button></form>
      </section>
...
export function QuickActionModal({ action, lead, deal, brokers, products, onClose, onSave }: {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (action === "visit") await onSave({ action: "createVisit", leadId: lead.id, dealId: deal?.id, productId: productId || null, productName: selectedProduct?.nome, date, startTime, endTime, local, observations: description, reminder: true }, "/api/crm");
      ...
      if (action === "financing") await onSave({ action, leadId: lead.id, dealId: deal?.id, productId: productId || null, productName: selectedProduct?.nome, name: lead.nome, phone: lead.telefone, email: lead.email, value, downPayment, financing: Math.max(0, Number(value) - Number(downPayment)), consent });
  ...
  const invalid = busy
    ...
    || ((action === "proposal" || action === "financing") && !(Number(value) > 0))
  ...
  return <div className="crm-center-modal quick-action-modal" role="dialog" aria-modal="true" aria-label={config[action].title}><form onSubmit={(event) => { event.preventDefault(); void submit(); }}><header><div><span>{config[action].eyebrow}</span><h2>{config[action].title}</h2><p>{config[action].description}</p></div><button aria-label="Fechar" type="button" onClick={onClose}>×</button></header><div className="quick-action-form">
    ...
    {action === "financing" && <><label>Produto<select value={productId} onChange={(event) => setProductId(event.target.value)}><option value="">Produto do negócio</option>{products.map((item) => <option value={item.id} key={item.id}>{item.nome}</option>)}</select></label><label>Valor do imóvel<input required min="1" step="0.01" type="number" value={value} onChange={(event) => setValue(event.target.value)} /></label><label>Entrada<input min="0" step="0.01" type="number" value={downPayment} onChange={(event) => setDownPayment(event.target.value)} /></label><label>Valor a financiar<input readOnly value={Math.max(0, Number(value) - Number(downPayment)) || ""} /></label><label className="wide check-line"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /> Consentimento LGPD já confirmado pelo cliente</label></>}
...
function ScheduleModal({ initialText, onClose, onSchedule }: { initialText: string; onClose: () => void; onSchedule: (content: string, when: string) => Promise<void> }) {

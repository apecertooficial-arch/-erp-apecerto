...
SELECT public.test_assert((SELECT max_tentativas FROM public.ncrm_workflow_config WHERE versao=3)=6,
  format('#op4 workflow tem primeira abordagem mais cinco cadências (obtido=%s)',
    coalesce((SELECT max_tentativas::text FROM public.ncrm_workflow_config WHERE versao=3),'NULL')));

...
cp "$ROOT/supabase/migrations/20260810170000_funil_2_operacao_completa.sql" "$STAGE/mig_f2_operacao.sql"
cp "$ROOT/supabase/rollbacks/20260810170000_funil_2_operacao_completa_rollback.sql" "$STAGE/down_f2_operacao.sql"
cp "$ROOT/supabase/migrations/20260810180000_funil_2_pesca_simples.sql" "$STAGE/mig_f2_pesca.sql"
cp "$ROOT/supabase/migrations/20260810190000_funil_2_conversa_pos_pesca.sql" "$STAGE/mig_f2_conversa.sql"
cp "$ROOT/supabase/rollbacks/20260810190000_funil_2_conversa_pos_pesca_rollback.sql" "$STAGE/down_f2_conversa.sql"
cp "$ROOT/supabase/migrations/20260810200000_funil_2_config_operacao.sql" "$STAGE/mig_f2_config.sql"
cp "$ROOT/supabase/rollbacks/20260810200000_funil_2_config_operacao_rollback.sql" "$STAGE/down_f2_config.sql"
cp "$ROOT/supabase/migrations/20260810210000_funil_2_config_grants.sql" "$STAGE/mig_f2_config_grants.sql"
cp "$ROOT/supabase/rollbacks/20260810210000_funil_2_config_grants_rollback.sql" "$STAGE/down_f2_config_grants.sql"
cp "$ROOT/supabase/rollbacks/20260810180000_funil_2_pesca_simples_rollback.sql" "$STAGE/down_f2_pesca.sql"
...
cp "$ROOT/supabase/migrations/20260811020000_funil_2_sara_reclassificacao.sql" "$STAGE/mig_f2_sara.sql"
cp "$ROOT/supabase/rollbacks/20260811020000_funil_2_sara_reclassificacao_rollback.sql" "$STAGE/down_f2_sara.sql"
cp "$ROOT/supabase/migrations/20260811033000_funil_2_sara_service_role_grants.sql" "$STAGE/mig_f2_sara_grants.sql"
cp "$ROOT/supabase/rollbacks/20260811033000_funil_2_sara_service_role_grants_rollback.sql" "$STAGE/down_f2_sara_grants.sql"
cp "$ROOT/tests/crm-nova-era/99m_tests_funil2_sara.sql" "$STAGE/f2_sara.sql"
chmod -R a+rX "$STAGE"
...
MIG_F2="$STAGE/mig_f2.sql"; MIG_F2_CLAREZA="$STAGE/mig_f2_clareza.sql"; MIG_F2_OPERACAO="$STAGE/mig_f2_operacao.sql"; MIG_F2_PESCA="$STAGE/mig_f2_pesca.sql"; MIG_F2_CONVERSA="$STAGE/mig_f2_conversa.sql"; MIG_F2_CONFIG="$STAGE/mig_f2_config.sql"; MIG_F2_CONFIG_GRANTS="$STAGE/mig_f2_config_grants.sql"; MIG_F2_SARA="$STAGE/mig_f2_sara.sql"; MIG_F2_SARA_GRANTS="$STAGE/mig_f2_sara_grants.sql"; DOWN_F2_SARA_GRANTS="$STAGE/down_f2_sara_grants.sql"; DOWN_F2_SARA="$STAGE/down_f2_sara.sql"; F2_SARA="$STAGE/f2_sara.sql"; DOWN_F2_CONFIG_GRANTS="$STAGE/down_f2_config_grants.sql"; DOWN_F2_CONFIG="$STAGE/down_f2_config.sql"; DOWN_F2_CONVERSA="$STAGE/down_f2_conversa.sql"; DOWN_F2_PESCA="$STAGE/down_f2_pesca.sql"; DOWN_F2_OPERACAO="$STAGE/down_f2_operacao.sql"; DOWN_F2="$STAGE/down_f2.sql"; F2="$STAGE/f2.sql"
...
PSQL -f "$MIG_F2_OPERACAO"
PSQL -f "$MIG_F2_PESCA"
PSQL -f "$MIG_F2_CONVERSA"
PSQL -f "$MIG_F2_CONFIG"
PSQL -f "$MIG_F2_CONFIG_GRANTS"
PSQL -f "$F2"
PSQL -f "$MIG_F2_SARA"
PSQL -f "$MIG_F2_SARA_GRANTS"
PSQL -f "$F2_SARA"
PSQL -f "$DOWN_F2_SARA_GRANTS"
PSQL -f "$DOWN_F2_SARA"
PSQL -f "$DOWN_F2_CONFIG_GRANTS"
PSQL -f "$DOWN_F2_CONFIG"
PSQL -f "$DOWN_F2_CONVERSA"
PSQL -f "$DOWN_F2_PESCA"
PSQL -f "$DOWN_F2_OPERACAO"
PSQL -f "$DOWN_F2"

...
export type ChatData = {
  ...
  pendingTransfers: Array<{ id: number; negocio_id: number; lead_id: number; cliente: string | null; corretor_origem: string | null; tipo: string; motivo: string; fit_comercial: string | null; criado_em: string }>;
};
const payloadChatValido = (result: unknown): result is ChatData => {
  if (!result || typeof result !== "object") return false;
  const payload = result as Partial<ChatData>;
  return [payload.conversations, payload.contacts, payload.instances, payload.dapi, payload.leads, payload.deals, payload.brokers, payload.products, payload.media, payload.activities, payload.approaches, payload.pendingTransfers].every(Array.isArray)
...
export function LiveChatWorkspace({ accessToken, initialLeadId = null, onInitialLeadHandled }: { accessToken: string; initialLeadId?: number | null; onInitialLeadHandled?: () => void }) {
  ...
  const loadMessages = async (id: string) => {
    ...
    if (!Array.isArray(result.messages)) throw new Error("Não foi possível confirmar as mensagens recebidas.");
  ...
  useEffect(() => { activeConversation.current = selectedId; setMessageLoadError(null); if (selectedId) { setMessages([]); void loadMessages(selectedId).catch((reason) => setMessageLoadError(reason instanceof Error ? reason.message : "Erro ao carregar mensagens.")); } else { setMessages([]); } }, [selectedId]);
  ...
  return <div className="live-chat">
    ...
    {pendingTransfers.length > 0 && <section className="live-transfer-inbox" aria-label="Transferências aguardando sua decisão"><strong>{pendingTransfers.length} transferência{pendingTransfers.length === 1 ? "" : "s"} aguardando sua decisão</strong>{pendingTransfers.map((item) => <article key={item.id}><div><b>{item.cliente || "Cliente sem nome"}</b><span>Origem: {item.corretor_origem || "não informada"} · {item.motivo}</span>{item.fit_comercial && <small>Fit comercial: {item.fit_comercial}</small>}</div><button disabled={busy} type="button" onClick={() => void call({ action: "decideTransfer", transferId: item.id, accept: false })}>Recusar</button><button className="accept" disabled={busy} type="button" onClick={() => void call({ action: "decideTransfer", transferId: item.id, accept: true })}>Aceitar</button></article>)}</section>}
    ...
    {quickAction && data && lead && <QuickActionModal action={quickAction} lead={lead} deal={deal ?? null} brokers={data.brokers} products={data.products} accessToken={accessToken} onClose={() => setQuickAction(null)} onSave={async (payload, endpoint) => { await call(payload, endpoint); setQuickAction(null); }} />}
    ...
    {scheduleOpen && <ScheduleModal initialText={draft} accessToken={accessToken} onClose={() => setScheduleOpen(false)} onSchedule={async (content, scheduledFor) => { if (!contact || !instanceId) return; await call({ action: "schedule", phone: contact.telefone, instanceId, leadId: lead?.id, content, when: scheduledFor }); setDraft(""); setScheduleOpen(false); void loadScheduled(lead?.id ?? null); }} onScheduleApproach={async (approachId, scheduledFor) => { if (!contact || !instanceId) return; await call({ action: "scheduleApproach", approachId, phone: contact.telefone, instanceId, leadId: lead?.id, leadName: lead?.nome || contact.nome, when: scheduledFor }); setScheduleOpen(false); void loadScheduled(lead?.id ?? null); }} />}
    {approachOpen && data && <ApproachModal approaches={data.approaches} onClose={() => setApproachOpen(false)} onSend={async (approachId) => { if (!contact || !instanceId) return; await call({ action: "sendApproach", approachId, phone: contact.telefone, instanceId, leadId: lead?.id, leadName: lead?.nome || contact.nome }); setApproachOpen(false); void loadScheduled(lead?.id ?? null); }} />}
  </div>;
}
...
function WaAudio({ src }: { src: string }) {
  ...
  // Velocidade estilo WhatsApp: 1x → 1,5x → 2x → 1x. O corretor clica na pílula e cicla.
  const RATES = [1, 1.5, 2];
  const cycleRate = () => { const next = RATES[(RATES.indexOf(rate) + 1) % RATES.length]; setRate(next); if (ref.current) ref.current.playbackRate = next; };
  const rateLabel = rate === 1.5 ? "1,5×" : `${rate}×`;
  return <div className="wa-audio">
    ...
    <button type="button" className={`wa-audio-rate${rate > 1 ? " active" : ""}`} onClick={cycleRate} title="Velocidade de reprodução" aria-label={`Velocidade de reprodução ${rateLabel}`}>{rateLabel}</button>
    <audio ref={ref} src={src} preload="metadata"
      onPlay={() => { if (ref.current) ref.current.playbackRate = rate; setPlaying(true); }} onPause={() => setPlaying(false)} onEnded={() => { setPlaying(false); setCur(0); }}
      onLoadedMetadata={(e) => { const a = e.target as HTMLAudioElement; a.playbackRate = rate; setDur(a.duration); }}
...
export function QuickActionModal({ action, lead, deal, brokers, products, gerentes = [], accessToken, onClose, onSave }: {
  action: QuickAction;
  lead: ChatData["leads"][number];
  deal: ChatData["deals"][number] | null;
  brokers: ChatData["brokers"];
  products: ChatData["products"];
  gerentes?: Array<{ id: number; nome: string; geral: boolean }>;
  accessToken?: string;
  onClose: () => void;
  ...
  const [transferType, setTransferType] = useState<"voluntaria" | "fit_comercial">("voluntaria");
  const [transferReason, setTransferReason] = useState("");
  const [commercialFit, setCommercialFit] = useState("");
  const [consent, setConsent] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  // Conflito do gerente que vai atender: mostra horários já marcados ao escolher data/hora,
  // pra dois corretores não agendarem o mesmo gerente no mesmo horário (Djair/Elizângela).
  const corretorId = (deal as { corretor_id?: number | null } | null)?.corretor_id ?? null;
  const [disp, setDisp] = useState<{ loading: boolean; conflitos: Array<{ cliente_nome: string | null; hora_inicio: string | null; hora_fim: string | null }>; gerenteNome: string | null; error?: boolean } | null>(null);
  useEffect(() => {
    ...
    void fetch("/api/agenda", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "gerenteDisponibilidade", corretorId, date, startTime, endTime }) })
      ...
      .then((result: { gerente_id?: number | null; conflitos?: Array<{ cliente_nome: string | null; hora_inicio: string | null; hora_fim: string | null }> }) => {
        if (!alive) return;
        const gname = gerentes.find((g) => g.id === (result.gerente_id ?? -1))?.nome ?? gerentes.find((g) => g.geral)?.nome ?? null;
  ...
  const config: Record<QuickAction, { eyebrow: string; title: string; description: string; submit: string }> = {
    ...
    transfer: { eyebrow: "DISTRIBUIÇÃO", title: "Transferir atendimento", description: "Informe destino e motivo. Para corretores, o destino precisa aceitar; gestão autorizada conclui diretamente.", submit: "Transferir" },
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (action === "transfer") await onSave({ action, leadId: lead.id, dealId: deal?.id, brokerId: Number(brokerId), transferType, reason: transferReason, commercialFit });
  ...
  const invalid = busy
    ...
    || (action === "visit" && (!date || !startTime || (withManager && (!disp || disp.loading || disp.error || disp.conflitos.length > 0))))
    || (action === "proposal" && !(Number(value) > 0))
    || (action === "financing" && !(Number(value) > 0 && Number(financing) > 0 && Number(financing) < Number(value)))
    || (action === "transfer" && (!brokerId || transferReason.trim().length < 3 || (transferType === "fit_comercial" && commercialFit.trim().length < 3)))
  ...
  return <div className="crm-center-modal quick-action-modal" role="dialog" aria-modal="true" aria-label={config[action].title}><form onSubmit={(event) => { event.preventDefault(); void submit(); }}><header><div><span>{config[action].eyebrow}</span><h2>{config[action].title}</h2><p>{config[action].description}</p></div><button aria-label="Fechar" type="button" onClick={onClose}>×</button></header><div className="quick-action-form">
    ...
    {action === "visit" && <><label>Data<input required type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label>Produto<select value={productId} onChange={(event) => setProductId(event.target.value)}><option value="">Sem produto definido</option>{products.map((item) => <option value={item.id} key={item.id}>{item.nome}</option>)}</select></label><label>Início<input required type="time" value={startTime} onChange={(event) => setStartTime(event.target.value)} /></label><label>Fim<input type="time" value={endTime} onChange={(event) => setEndTime(event.target.value)} /></label><label className="wide">Local<input value={local} onChange={(event) => setLocal(event.target.value)} placeholder="Preenchido pelo endereço do produto quando vazio" /></label><label className="wide">Observações<textarea rows={3} value={description} onChange={(event) => setDescription(event.target.value)} /></label><label className="check wide"><input type="checkbox" checked={withManager} onChange={(event) => setWithManager(event.target.checked)} /> Ir com gerente</label>{disp && <div className={`cal-disp ${disp.error || disp.conflitos.length ? "conflito" : "livre"}`}>{disp.loading ? "Checando horários já marcados do gerente…" : disp.error ? "Não foi possível confirmar a agenda do gerente. Tente outro horário ou agende sem gerente." : disp.conflitos.length ? `⚠ ${disp.gerenteNome || "O gerente"} já tem ${disp.conflitos.length} visita${disp.conflitos.length > 1 ? "s" : ""} nesse horário (${disp.conflitos.map((c) => `${c.hora_inicio?.slice(0, 5) ?? ""} ${c.cliente_nome ?? ""}`.trim()).filter(Boolean).join("; ")}). Escolha outro horário pra não conflitar.` : `✓ ${disp.gerenteNome || "Gerente"} livre nesse horário.`}</div>}</>}
    ...
    {action === "transfer" && <><label>Novo corretor<select required value={brokerId} onChange={(event) => setBrokerId(event.target.value)}><option value="">Selecione um corretor</option>{brokers.filter((item) => item.id !== deal?.corretor_id).map((item) => <option value={item.id} key={item.id}>{item.nome}{item.online ? " · online" : ""}</option>)}</select></label><label>Tipo<select value={transferType} onChange={(event) => setTransferType(event.target.value as "voluntaria" | "fit_comercial")}><option value="voluntaria">Transferência voluntária</option><option value="fit_comercial">Fit comercial</option></select></label><label className="wide">Motivo da transferência<textarea required maxLength={500} rows={3} value={transferReason} onChange={(event) => setTransferReason(event.target.value)} placeholder="Explique por que o atendimento deve mudar de responsável" /></label>{transferType === "fit_comercial" && <label className="wide">Fit comercial<textarea required maxLength={500} rows={3} value={commercialFit} onChange={(event) => setCommercialFit(event.target.value)} placeholder="Registre o perfil, especialidade ou contexto que justifica o novo responsável" /></label>}<p className="quick-action-hint">Etapa, momento, negociação e visitas existentes serão preservados.</p></>}

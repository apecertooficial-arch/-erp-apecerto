...
| Pós-visita | feedback e cobrança | CRM/gestão | `/api/agenda`, `/api/funil2`, `f2_registrar_resultado_visita`, `f2_visitas_resultado_pendente` | `f2_visita`, `f2_lead`, eventos | APIs exigem corretor dono; gestão somente acompanha; RPC remota ainda permite admin direto | 63 testes direcionados; 437/437 no gate frontend; navegador desktop/móvel sanitizado | `TRACE_VISITA_FEEDBACK_COBRANCA.md` | parcial; gravação, erro fail-closed, virada de mês, responsável/idade e separação gerente × corretor cobertos; endurecimento da RPC, escalonamento persistido e contrato sem expiração pendentes |

...
-- A limitação de duas cópias pertence ao laboratório, não à experiência do
-- usuário. A pesca agora substitui silenciosamente a cópia mais antiga e
-- sempre inicia o novo lead em Novo / Primeira abordagem. O parâmetro antigo
-- é mantido apenas para compatibilidade da assinatura da RPC.
...
BEGIN
  ...
  -- Serializa duas pescas simultâneas e mantém o limite físico do laboratório.
  ...
  -- A importação mantém o vínculo com o negócio original (e portanto o chat),
  -- mas a cópia entra sempre no começo do fluxo de trabalho do laboratório.
  v_novo := public.f2_importar_negocio(p_negocio_id);
  UPDATE public.f2_lead
  SET etapa='novo',
      momento_codigo='PRIMEIRA_ABORDAGEM',
      acao_codigo='PRIMEIRA_ABORDAGEM',
      acao_rotulo='Fazer a primeira abordagem',
      proxima_acao_em=now()+interval '5 minutes',
      cadencia_passo=0,
      ultima_acao_confirmada_em=NULL,
      ultima_acao_fonte=NULL,
      ultima_reavaliacao_sara_em=NULL,
      ultima_reavaliacao_resumo=NULL,
      versao=versao+1,
      atualizado_em=now(),
  ...
  INSERT INTO public.f2_evento(
    ...
    v_novo,'momento_alterado','Lead pescado: primeira abordagem',
  ...
  VALUES(
    'pesca',p_negocio_id::text,'pescar_lead',
    jsonb_build_object('novo_id',v_novo,'copia_anterior_removida',v_substituido),v_uid
  );

  RETURN jsonb_build_object(
    'ok',true,'id',v_novo,'etapa','novo','momento','PRIMEIRA_ABORDAGEM'
  );
END;
$fn$;

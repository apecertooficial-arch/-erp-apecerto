...
/* A D-API mistura formatos: "5511999999999@s.whatsapp.net", "5511999999999",
   ...
   resolve os tres. O jid de grupo termina em @g.us e e descartado antes -- 
   conversa de grupo nao e relacionamento com o cliente. */
...
Deno.serve(async (req) => {
  ...
  for (const i of vivas) {
    ...
    // CONVERSAS, pagina a pagina. Para quando a pagina volta vazia ou repete o
    // que ja veio -- API sem paginacao documentada as vezes ignora o parametro
    // e devolveria a mesma pagina para sempre.
    for (let pg = 1; pg <= paginas; pg++) {
      try {
        ...
        for (const l of linhas) {
          ...
          if (!tel) continue;
          const nome = String(ct.pushName ?? o.chat_name ?? o.chatName ?? "").includes("@") ? undefined
                     : String(ct.pushName ?? o.chat_name ?? o.chatName ?? "") || undefined;

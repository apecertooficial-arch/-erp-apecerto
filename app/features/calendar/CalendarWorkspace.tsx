...
export function CalendarWorkspace({ accessToken }: { accessToken: string }) {
  ...
  const [form, setForm] = useState({ dealId: "", productId: "", date: todayIso, startTime: "10:00", endTime: "11:00", local: "", observations: "", withManager: false, reminder: true });
  const [drag, setDrag] = useState<{ date: string; start: number; end: number } | null>(null);
  const [editing, setEditing] = useState<Visit | null>(null);
  const [editForm, setEditForm] = useState({ date: "", startTime: "", endTime: "", local: "", observations: "", withManager: false });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await fetch("/api/crm", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "gerenteDisponibilidade", corretorId: editing.corretor_id, date: editForm.date, startTime: editForm.startTime, endTime: editForm.endTime || null, visitId: editing.id }) });

...
const normalizarTexto = (v: unknown) => String(v ?? "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
...
export function CrmWorkspace({ accessToken, initialDealId = null, onInitialDealHandled, initialChatDealId = null, onInitialChatHandled, initialView, initialCreateSale = false, onInitialViewHandled, sessionRole = "corretor", canReassign = false, canAssign = false }: { accessToken: string; initialDealId?: number | null; onInitialDealHandled?: () => void; initialChatDealId?: number | null; onInitialChatHandled?: () => void; initialView?: ViewName | null; initialCreateSale?: boolean; onInitialViewHandled?: () => void; sessionRole?: "admin" | "gestor" | "corretor"; canReassign?: boolean; canAssign?: boolean }) {
  ...
  // null = TODOS os funis. A tela de Leads abre assim; o kanban escolhe um funil
  // concreto por conta própria (kanbanPipeId), porque um quadro precisa de etapas de um funil só.
  const [pipelineId, setPipelineId] = useState<number | null>(null);
  const [stageConfigOpen, setStageConfigOpen] = useState(false);
  // Momento pode ser atualizado direto do card, sem abrir a ficha inteira.
  const [momentoDealId, setMomentoDealId] = useState<number | null>(null);
  ...
  // O quadro do funil precisa de um funil selecionado; se o usuário está em "todos", usa o primeiro.
  const kanbanPipeId = pipelineId ?? (data?.pipelines?.[0]?.id ?? null);
  // Filtro de funil aplicado à lista: no kanban é sempre um funil, nas demais telas respeita "todos".
  const pipeFiltro = view === "pipeline" ? kanbanPipeId : pipelineId;
  const activeStages = useMemo(() => (data?.stages ?? []).filter((stage) => stage.pipeline_id === kanbanPipeId), [data, kanbanPipeId]);
  ...
  // ===== Busca de lead: nome, telefone, e-mail, instagram e #id =====
  // Nome é comparado por palavras soltas (ordem livre) e sem acento; telefone compara
  // os últimos 11 dígitos, então acha com ou sem DDI 55, DDD e qualquer formatação.
  const termos = useMemo(() => normalizarTexto(query).split(/\s+/).filter(Boolean), [query]);
  const buscaDigitos = useMemo(() => query.replace(/\D/g, ""), [query]);
  const buscando = termos.length > 0 || buscaDigitos.length >= 3;
  const casaBusca = useCallback((lead: Lead | undefined, deal: Deal) => {
    if (buscaDigitos.length >= 3) {
      const fone = soDigitos(lead?.telefone);
      if (fone && (fone.endsWith(sufixoFone(buscaDigitos)) || fone.includes(buscaDigitos))) return true;
      if (String(lead?.id ?? "") === buscaDigitos || String(deal.id) === buscaDigitos) return true;
    }
    if (!termos.length) return false;
    const alvo = normalizarTexto(`${lead?.nome ?? ""} ${lead?.email ?? ""} ${lead?.instagram ?? ""}`);
  ...
  const filteredDeals = useMemo(() => (data?.deals ?? []).filter((deal) => {
    ...
    // Buscar é procurar em TODA a base: com texto digitado, o filtro de funil sai do caminho.
    // Sem busca, respeita o funil escolhido (null = todos).
    if (!buscando && pipeFiltro !== null && deal.pipeline_id !== pipeFiltro) return false;
    ...
    const buscaOk = !buscando || casaBusca(lead, deal);
  ...
  }), [data, leadById, pipeFiltro, buscando, casaBusca, stageId, brokerId, origin, tag, group, overdueOnly, slaByDeal, dateFrom, dateTo, productFilter]);
  const overdueCount = useMemo(() => (data?.deals ?? []).filter((deal) => { const sla = slaByDeal.get(deal.id); return (pipeFiltro === null || deal.pipeline_id === pipeFiltro) && Boolean(sla && (sla.alarme_ativo || sla.cor_ativa === "vermelho")); }).length, [data, pipelineId, slaByDeal]);
  ...
  async function reorderStage(stageId: number, direction: number) {
    ...
    setMessage(null);
    const { error: rpcError } = await getBrowserSupabaseClient().rpc("crm_etapa_reordenar", { p_pipeline_id: kanbanPipeId as number, p_ids: ids });
  ...
  return <div className="crm-v2">
    ...
    {view !== "sales" && <section className="crm-toolbar-v2">
      {view !== "pipeline" && <select aria-label="Funil" value={pipelineId ?? ""} onChange={(e) => { setPipelineId(e.target.value === "" ? null : Number(e.target.value)); setStageId(null); setGroup(null); }}><option value="">Todos os funis</option>{(data?.pipelines ?? []).map((p) => <option value={p.id} key={p.id}>{p.nome}</option>)}</select>}
    ...
    {momentoDealId !== null && data && (() => {
      const d = data.deals.find((x) => x.id === momentoDealId);
      const l = d ? leadById.get(d.lead_id) : undefined;
      if (!d || !l) return null;
      return <MomentoModal lead={l} dealId={d.id} catalogo={data.momentoCatalogo ?? []} onClose={() => setMomentoDealId(null)} onSalvar={mutate} onMessage={setMessage} />;
    })()}
...
function LeadsViewEnhanced({ onMomento, momentoCatalogo, deals, leadById, stages, brokerById, slaByDeal, canReassign, onReassign, onOpen, onChat, onMutate, setMessage, canMoveDeals, onPickStage }: { onMomento?: (dealId: number) => void; momentoCatalogo?: MomentoCat[]; deals: Deal[]; leadById: Map<number, Lead>; stages: Stage[]; brokerById: Map<number, Broker>; slaByDeal: Map<number, SlaInfo>; canReassign: boolean; onReassign: (dealId: number) => void; onOpen: (id: number) => void; onChat: (id: number) => void; onMutate: (body: Record<string, unknown>) => Promise<void>; setMessage: (value: string | null) => void; canMoveDeals?: boolean; onPickStage?: (dealId: number) => void }) {
  const [busyId, setBusyId] = useState<number | null>(null);
  const change = async (body: Record<string, unknown>, success: string, dealId: number) => { setBusyId(dealId); try { await onMutate(body); setMessage(success); } catch (reason) { setMessage(reason instanceof Error ? reason.message : "Não foi possível salvar."); } finally { setBusyId(null); } };
  const visibleDeals = deals.slice(0, 200);
  return <section className="crm-table-wrap">
    <table className="crm-leads-table crm-leads-table-v3">
      ...
      <tbody>{visibleDeals.map((deal, index) => {
        const lead = leadById.get(deal.lead_id) ?? ({ id: deal.lead_id, nome: `Lead #${deal.lead_id}`, telefone: null, email: null, corretor_id: deal.corretor_id, tags: null, extras: null, origem: null } as unknown as Lead);
        const sla = slaByDeal.get(deal.id);
        const broker = brokerById.get(deal.corretor_id ?? lead.corretor_id ?? -1);
        const tags = tagList(lead.tags).slice(0, 2);
    ...
    {deals.length > 200 && <div className="crm-leads-limit">Mostrando os primeiros 200 de {deals.length} leads</div>}
...
function HistoryInstanceSelect({ instances, value, onChange }: { instances: ChatInstance[]; value: string; onChange: (value: string) => void }) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  const sel = instances.find((item) => item.key === value) ?? null;
  useEffect(() => {
    if (!open) return;
    const onDoc = (event: globalThis.MouseEvent) => { if (ref.current && !ref.current.contains(event.target as Node)) setOpen(false); };
    const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") setOpen(false); };
...
export function LeadChatDrawer({ accessToken, lead, deal, corretorNome, onClose, onResponse, onOpenLead, readOnly = false }: { accessToken: string; lead: Lead; deal: Deal; corretorNome?: string; onClose: () => void; onResponse: () => Promise<void>; onOpenLead?: () => void; readOnly?: boolean }) {
  ...
  async function cancelScheduled(id: number) { try { await liveAction({ action: "cancelScheduled", scheduledId: id }); await loadScheduled(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível cancelar."); } }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { if (!readOnly) void loadScheduled(); }, [lead.id, readOnly]);
  ...
  return <div className="crm-drawer-layer chat-layer" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className={`lead-chat-drawer ${collapsed ? "is-collapsed" : ""}`}><header><div className={onOpenLead ? "chat-head-lead clickable" : "chat-head-lead"} role={onOpenLead ? "button" : undefined} tabIndex={onOpenLead ? 0 : undefined} title={onOpenLead ? "Abrir ficha do lead" : undefined} onClick={onOpenLead ? () => onOpenLead() : undefined} onKeyDown={onOpenLead ? (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onOpenLead(); } } : undefined}><span>{initials(lead.nome)}</span><div><small>{readOnly ? "HISTÓRICO DO LEAD" : "CONVERSA DO LEAD"} #{lead.id}</small><h2>{lead.nome || "Lead sem nome"}{onOpenLead ? " ›" : ""}</h2><p>{lead.telefone || "Telefone não informado"}</p></div></div><div className="chat-head-actions"><button type="button" onClick={() => setCollapsed((v) => !v)} title={collapsed ? "Expandir" : "Minimizar"} aria-label="Minimizar">{collapsed ? "▢" : "—"}</button><button type="button" onClick={onClose} aria-label="Fechar">×</button></div></header><section className="chat-instance-bar"><label>Instância do histórico<HistoryInstanceSelect instances={instances} value={selectedKey} onChange={setSelectedKey} /></label><span className={selected?.conectada ? "connected" : ""}>{selected?.conectada ? `● conectada${last4(selected?.numero) ? ` · final ${last4(selected?.numero)}` : ""}` : "○ sem conexão confirmada"}</span></section>{readOnly && <div className="chat-readonly-note">Somente leitura · responda pelo WhatsApp do celular</div>}{error && <div className="chat-error">{error}</div>}<section className="chat-messages" ref={listRef}>{loading && <div className="chat-loading">Carregando conversa real…</div>}{!loading && messages.filter((item) => !hiddenIds.has(String(item.wa_message_id || item.id))).map((item) => <article className={`${item.direcao === "enviada" ? "sent" : "received"} ${item.direcao === "enviada" && ackState(item.status) === "erro" ? "msg-failed" : ""}`} key={item.id}>{!readOnly && !String(item.id).startsWith("temp-") && (confirmDel === String(item.wa_message_id || item.id)
...
function useLeadCopiloto(accessToken: string, leadNome: string, enabled = true) {
  ...
  useEffect(() => {
    if (!leadNome || !enabled) { setState({ loading: false, data: null }); return; }
    let alive = true;
    setState((s) => ({ loading: true, data: nonce === 0 ? null : s.data }));
    fetch(`/api/agentes/copiloto-lead?lead=${encodeURIComponent(leadNome)}${nonce ? `&regenerar=1&n=${nonce}` : ""}`, { headers: { Authorization: `Bearer ${accessToken}` } })
      .then((r) => r.json())
      .then((d) => { if (alive) setState({ loading: false, data: d?.ok ? d : null }); })
      .catch(() => { if (alive) setState({ loading: false, data: null }); });
    return () => { alive = false; };
  }, [accessToken, leadNome, nonce, enabled]);
  return { ...state, refresh: () => setNonce((n) => n + 1) };
}

/* Modal de momento: usado tanto pelo card do funil quanto pela ficha do lead.
   Dois campos apenas — momento e observação — porque a ideia é ser rápido de
   preencher todo dia; se der trabalho, o corretor não faz. */
function MomentoModal({ lead, dealId, catalogo, onClose, onSalvar, onMessage }: { lead: Lead; dealId: number; catalogo: MomentoCat[]; onClose: () => void; onSalvar: (body: Record<string, unknown>) => Promise<void>; onMessage: (v: string | null) => void }) {
  const [momento, setMomento] = useState(lead.momento ?? "");
  const [observacao, setObservacao] = useState("");
  const [busy, setBusy] = useState(false);
  const grupos = [...new Set(catalogo.map((m) => m.grupo))];
  const dias = diasDesde(lead.momento_em);
  const salvar = () => {
    if (!momento) return;
    setBusy(true);
    void onSalvar({ action: "registrarMomento", leadId: lead.id, dealId, momento, observacao: observacao || undefined })
      .then(() => { onMessage("Momento atualizado."); onClose(); })
      .catch((r) => onMessage(r instanceof Error ? r.message : "Não foi possível atualizar o momento."))
      .finally(() => setBusy(false));
  };
  return <div className="crm-center-modal" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
    <form onSubmit={(e) => { e.preventDefault(); salvar(); }}>
      <header><div><span>MOMENTO DO LEAD</span><h2>Em que ponto está?</h2><p>{lead.nome || `Lead #${lead.id}`}{lead.momento_em ? ` · atualizado ${dias === 0 ? "hoje" : dias === 1 ? "ontem" : `há ${dias} dias`}` : " · nunca atualizado"}</p></div><button type="button" onClick={onClose}>×</button></header>
      <label>Momento
        <select required value={momento} disabled={busy} onChange={(e) => setMomento(e.target.value)}>
          <option value="">Selecione</option>
          {grupos.map((g) => <optgroup label={g} key={g}>{catalogo.filter((m) => m.grupo === g).map((m) => <option value={m.slug} key={m.slug}>{m.rotulo}</option>)}</optgroup>)}
        </select>
      </label>
      <label>Observação<textarea rows={3} value={observacao} disabled={busy} placeholder="O que aconteceu no último contato" onChange={(e) => setObservacao(e.target.value)} /></label>
      {lead.momento_obs && <p className="momentox-obs-atual">Última anotação: “{lead.momento_obs}”</p>}
      <footer><button type="button" onClick={onClose}>Cancelar</button><button className="crm-primary" type="submit" disabled={busy || !momento}>{busy ? "Salvando…" : "Salvar momento"}</button></footer>
    </form>
  </div>;
}
...
function LeadDrawer({ accessToken, lead, deal, data, canReassign, canMoveDeals, onClose, onMutate, onReload, setMessage }: { accessToken: string; lead: Lead; deal: Deal; data: CrmData; canReassign: boolean; canMoveDeals?: boolean; onClose: () => void; onMutate: (body: Record<string, unknown>) => Promise<void>; onReload: () => Promise<void>; setMessage: (value: string | null) => void }) {
  ...
  /* Ficha de financiamento — gera/reaproveita o link público e mostra o status */
  const [ficha, setFicha] = useState<{ status: string; preenchida_em: string | null; renda: number | null; valor_imovel: number | null; valor_entrada: number | null; valor_financiar: number | null; conjuge_nome: string | null; conjuge_renda: number | null } | null>(null);
  useEffect(() => {
    void (async () => {
      try {
        const response = await authedFetch(`/api/financiamento?leadId=${lead.id}`, { headers: { Authorization: `Bearer ${accessToken}` } });
        const result = await response.json() as { fichas?: Array<{ status: string; preenchida_em: string | null; renda: number | null; valor_imovel: number | null; valor_entrada: number | null; valor_financiar: number | null; conjuge_nome: string | null; conjuge_renda: number | null }> };
        if (response.ok && result.fichas?.length) setFicha(result.fichas[0]);
      } catch { /* sem ficha ainda */ }
    })();
  }, [accessToken, lead.id]);
  const copyFichaLink = async () => {
    ...
    try {
      ...
      try { await navigator.clipboard.writeText(url); setMessage("Link da ficha de financiamento copiado — é só colar na conversa com o cliente."); }
      catch { setMessage(`Link da ficha: ${url}`); }
      if (!ficha) setFicha({ status: "enviada", preenchida_em: null, renda: null, valor_imovel: null, valor_entrada: null, valor_financiar: null, conjuge_nome: null, conjuge_renda: null });
    } catch (reason) { setMessage(reason instanceof Error ? reason.message : "Não foi possível gerar o link."); }
    finally { setBusy(false); }
  };
  ...
  return <div className="crm-drawer-layer" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className="crm-drawer-v2">
    ...
    <section className="lead-classic-panel">
      ...
      <div className="lead-action-grid">
        ...
        <button type="button" disabled={busy} title="Copia o link da ficha de análise de financiamento para enviar ao cliente" onClick={() => void copyFichaLink()}><LeadActionIcon name="financing" /><span>{ficha?.status === "preenchida" ? "Ficha preenchida ✓" : "Ficha de financiamento"}</span></button>
    ...
    <nav className="drawer-tabs-v2 draggable-tabs">{tabOrder.map((key) => { const label = key === "resumo" ? "Resumo" : key === "historico" ? "Histórico" : key === "agenda" ? "Tarefas e visitas" : `Produtos (${links.length})`; return <button
      ...
      {quickAction && <QuickActionModal action={quickAction} lead={lead} deal={deal} brokers={data.brokers} products={data.products} gerentes={data.gerentes ?? []} accessToken={accessToken} onClose={() => setQuickAction(null)} onSave={async (payload, endpoint) => { await callExternal(payload, endpoint); setQuickAction(null); }} />}
    ...
    {momOpen && <MomentoModal lead={lead} dealId={deal.id} catalogo={data.momentoCatalogo ?? []} onClose={() => setMomOpen(false)} onSalvar={onMutate} onMessage={setMessage} />}
    </aside></div>;
}
...
function LeadSaleModal({ accessToken, deal, products, onClose, onDone }: { accessToken: string; deal: Deal; products: Product[]; onClose: () => void; onDone: () => Promise<void> }) {
  // O valor NÃO é pedido aqui: ele é definido nas Condições comerciais, na etapa de Proposta.
  const [productId, setProductId] = useState(deal.empreendimento_id || ""); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  return <div className="crm-center-modal"><form onSubmit={(event) => { event.preventDefault(); setBusy(true); setError(""); void authedFetch("/api/crm/sales", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "solicitar", dealId: deal.id, productId }) }).then(async (response) => { const result = await response.json() as { error?: string }; if (!response.ok) throw new Error(result.error); await onDone(); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Não foi possível criar a venda.")).finally(() => setBusy(false)); }}><header><div><span>GERAR VENDA</span><h2>Gerar venda</h2><p>O lead sai do funil e vai para a Esteira de vendas. O valor é definido depois, nas Condições comerciais — até lá a venda conta como negociação em aberto.</p></div><button type="button" onClick={onClose}>×</button></header>{error && <div className="modal-error">{error}</div>}<label>Produto<select required value={productId} onChange={(event) => setProductId(event.target.value)}><option value="">Selecione</option>{products.map((product) => <option value={product.id} key={product.id}>{product.nome}</option>)}</select></label><footer><button type="button" onClick={onClose}>Cancelar</button><button className="crm-primary" disabled={busy || !productId} type="submit">{busy ? "Enviando…" : "Enviar para aprovação"}</button></footer></form></div>;
}

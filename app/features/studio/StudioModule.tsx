...
import { validCommentContext } from "./studio-utils";
...
export function StudioModule({ accessToken, initialData, mutationHandler }: {
  ...
  return <div className="studio-shell">
    ...
    {tab === "visao" && <Overview data={data} onCreate={() => setCreating(true)} onOpen={(id, piece) => { setCampaignId(id); if (piece) setPieceId(piece); setTab("workspace"); window.history.replaceState(null, "", `/studio?tab=workspace&campaign=${id}${piece ? `&piece=${piece}` : ""}`); }} />}
    ...
    {tab === "calendario" && <StudioCalendarNew data={data} busy={busy} mutate={mutate} />}
...
function Overview({ data, onCreate, onOpen }: { data: StudioData; onCreate: () => void; onOpen: (id: string, pieceId?: string) => void }) {
...
function StudioManagerBoard({ data, onOpen }: { data: StudioData; onOpen: (id: string) => void }) {
  const [status, setStatus] = useState("todos"); const [format, setFormat] = useState("todos"); const [campaign, setCampaign] = useState("todos"); const [owner, setOwner] = useState("todos"); const tasks = data.tasks ?? [];
  const filtered = data.pieces.filter((piece) => (status === "todos" || piece.status === status) && (format === "todos" || piece.formato === format) && (campaign === "todos" || piece.campaign_id === campaign) && (owner === "todos" || tasks.find((task) => task.piece_id === piece.id)?.responsavel_id === owner));
  return <section className="studio-section studio-manager-board"><header><div><span className="studio-eyebrow">Visão do gestor</span><h2>Board operacional</h2><p>Peças, prazos e bloqueios navegáveis.</p></div></header><div className="studio-board-filters"><select value={campaign} onChange={(event) => setCampaign(event.target.value)}><option value="todos">Todas as campanhas</option>{data.campaigns.map((item) => <option value={item.id} key={item.id}>{item.nome}</option>)}</select><select value={format} onChange={(event) => setFormat(event.target.value)}><option value="todos">Todos os formatos</option>{(["feed","carousel","story","reel"] as const).map((item) => <option value={item} key={item}>{FORMAT_META[item].label}</option>)}</select><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="todos">Todos os status</option>{["rascunho","em_revisao","aprovada","agendada","publicada"].map((item) => <option value={item} key={item}>{statusLabel[item]}</option>)}</select><select value={owner} onChange={(event) => setOwner(event.target.value)}><option value="todos">Todos os responsáveis</option>{(data.members ?? []).map((item) => <option value={item.id} key={item.id}>{item.nome} · {item.role ?? "membro"}</option>)}</select></div><div className="studio-board-list">{filtered.map((piece) => { const task = tasks.find((item) => item.piece_id === piece.id); return <button type="button" key={piece.id} onClick={() => { onOpen(piece.campaign_id); window.history.replaceState(null, "", `/studio?tab=workspace&campaign=${piece.campaign_id}&piece=${piece.id}`); }}><strong>{piece.titulo}</strong><span>{FORMAT_META[piece.formato].label}</span><span>{task?.prazo_em ? new Date(task.prazo_em).toLocaleString("pt-BR") : "Sem prazo"}</span><span>{task?.pendencia ?? "Sem bloqueio"}</span><b>{statusLabel[piece.status] ?? piece.status}</b></button>; })}</div>{!filtered.length && <EmptyState icon="check" title="Nenhuma peça neste filtro" text="Ajuste os filtros para visualizar o trabalho."/>}</section>;
}

function StudioMetricsDashboard({ data, onOpen }: { data: StudioData; onOpen: (id: string) => void }) {
  const [campaign, setCampaign] = useState("todos"); const [format, setFormat] = useState("todos"); const metrics = (data.metrics ?? []).filter((item) => campaign === "todos" || item.campaign_id === campaign).filter((item) => format === "todos" || data.pieces.find((piece) => piece.id === item.piece_id)?.formato === format); const sum = (key: "alcance" | "impressoes" | "cliques") => metrics.reduce((total, item) => total + Number(item[key] ?? 0), 0);
  return <section className="studio-section studio-metrics-dashboard"><header><div><span className="studio-eyebrow">Métricas</span><h2>Desempenho por campanha, imóvel e formato</h2><p>Snapshots filtráveis por período e template, sem números inventados.</p></div></header><div className="studio-board-filters"><select value={campaign} onChange={(event) => setCampaign(event.target.value)}><option value="todos">Todas as campanhas</option>{data.campaigns.map((item) => <option value={item.id} key={item.id}>{item.nome}</option>)}</select><select value={format} onChange={(event) => setFormat(event.target.value)}><option value="todos">Todos os formatos</option>{(["feed","carousel","story","reel"] as const).map((item) => <option value={item} key={item}>{FORMAT_META[item].label}</option>)}</select></div>{metrics.length ? <div className="studio-kpis"><Kpi label="Alcance" value={sum("alcance")}/><Kpi label="Impressões" value={sum("impressoes")}/><Kpi label="Cliques" value={sum("cliques")}/></div> : <EmptyState icon="grid" title="Métricas ainda não disponíveis" text="Meta não conectada ou nenhum snapshot importado para este filtro."/>}<div className="studio-metrics-rows">{metrics.map((item) => <button type="button" key={item.id} onClick={() => item.campaign_id && onOpen(item.campaign_id)}><span>{item.periodo_inicio} — {item.periodo_fim}</span><strong>{item.fonte}</strong><span>{item.observacao ?? "Snapshot operacional"}</span></button>)}</div></section>;
}
...
function Builder({ data, campaign, campaignId, piece, version, snapshot, pieces, onCampaign, onPiece, busy, mutate }: {
  ...
  return <main className="studio-builder">
    ...
    <section className="studio-canvas-area">
      ...
      <div className="studio-brief-strip"><span><strong>Estratégia</strong> {String(brief?.conteudo?.angulo_editorial ?? snapshot?.fatos.bairro ?? "bairro")}</span><span><strong>Pilares</strong> {Array.isArray(brief?.conteudo?.pilares) ? brief.conteudo.pilares.join(" · ") : "localização · diferenciais · estilo de vida"}</span><span><strong>Status</strong> {sandboxGenerated ? "Demonstração pronta para revisão" : "Aguardando geração"}</span></div>
...
function BriefingEditor({ campaignId, brief, busy, mutate }: { campaignId: string; brief: StudioData["briefs"][number] | null; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult> }) {
  ...
  return <details className="studio-brief-editor" open><summary><strong>Briefing e estratégia</strong><span>{brief ? `versão ${brief.versao}` : "não salvo"}</span></summary><div className="studio-brief-form"><label className="studio-field"><span>Público</span><input aria-label="Público-alvo" value={audience} onChange={(event) => setAudience(event.target.value)}/></label><label className="studio-field"><span>Tom de voz</span><input aria-label="Tom de voz" value={tone} onChange={(event) => setTone(event.target.value)}/></label><label className="studio-field"><span>Ângulo editorial</span><input aria-label="Ângulo editorial" value={angle} onChange={(event) => setAngle(event.target.value)}/></label><label className="studio-field"><span>Pilares</span><input aria-label="Pilares editoriais" value={pillars} onChange={(event) => setPillars(event.target.value)}/><small>Separe os pilares por · para orientar copy e formatos.</small></label><button type="button" className="studio-primary" disabled={busy} onClick={() => void mutate({ action: "saveBrief", campaignId, publico: { segmento: audience }, tom: tone, canais: ["instagram"], restricoesFactuais: [], conteudo: { angulo_editorial: angle, pilares: pillars.split("·").map((item) => item.trim()).filter(Boolean) } }, "Briefing e estratégia salvos como nova versão.")}><Icon name="check"/> Salvar briefing como nova versão</button></div></details>;
}
...
function DemoInspector({ piece, demo, variant, onVariant, mutate }: { piece: StudioPiece | null; demo: Record<string, unknown> | null; variant: number; onVariant: () => void; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult> }) {
  const safeDemo = demo ?? {};
  const [fields, setFields] = useState({ headline: String(safeDemo.headline ?? ""), legenda: String(safeDemo.legenda ?? ""), cta: String(safeDemo.cta ?? ""), estrutura: JSON.stringify({ ...(safeDemo.slides ? { slides: safeDemo.slides } : {}), ...(safeDemo.stories ? { stories: safeDemo.stories } : {}), ...(safeDemo.cenas ? { cenas: safeDemo.cenas } : {}) }, null, 2) });
  ...
  return <div className="studio-demo-inspector"><div className="studio-demo-badge">SANDBOX · SEM CUSTO · NÃO PUBLICÁVEL</div><p className="studio-safe-note">Prévia editável. Os fatos do snapshot permanecem protegidos; salvar cria uma nova versão em revisão.</p><label className="studio-field"><span>Headline</span><textarea maxLength={120} value={fields.headline} onChange={(event) => setFields({ ...fields, headline: event.target.value })}/></label><label className="studio-field"><span>Legenda</span><textarea maxLength={2200} value={fields.legenda} onChange={(event) => setFields({ ...fields, legenda: event.target.value })}/></label><label className="studio-field"><span>CTA</span><input maxLength={80} value={fields.cta} onChange={(event) => setFields({ ...fields, cta: event.target.value })}/></label><label className="studio-field"><span>Estrutura de {piece.formato === "carousel" ? "slides" : piece.formato === "story" ? "telas" : piece.formato === "reel" ? "cenas" : "conteúdo"}</span><textarea value={fields.estrutura} onChange={(event) => setFields({ ...fields, estrutura: event.target.value })}/><small>JSON editável; a versão anterior permanece intacta.</small></label><div className="studio-variant-list"><strong>Variação {variant + 1}</strong>{rows.map((row, index) => <span key={`${String(row)}-${index}`}>{String(row)}</span>)}</div><div className="studio-command-actions"><button type="button" className="studio-secondary" onClick={onVariant}><Icon name="refresh"/> Gerar outra variação</button><button type="button" className="studio-primary" onClick={() => { let estrutura: Record<string, unknown> = {}; try { estrutura = JSON.parse(fields.estrutura) as Record<string, unknown>; } catch { window.alert("Estrutura inválida. Use JSON válido."); return; } void mutate({ action: "createVariant", pieceId: piece.id, conteudo: { ...demo, headline: fields.headline, legenda: fields.legenda, cta: fields.cta, ...estrutura }, changeScope: piece.formato === "carousel" || piece.formato === "reel" ? "cena" : "copy" }, "Variação editada persistida no histórico da peça.") }}><Icon name="check"/> Salvar no histórico</button></div><small>Sandbox sem custo. A versão persistida permanece em revisão e não é publicada automaticamente.</small></div>;
}
...
function CollaborationPanel({ piece, version, tasks, comments, members, busy, mutate }: { piece: StudioPiece; version: StudioPieceVersion; tasks: NonNullable<StudioData["tasks"]>; comments: NonNullable<StudioData["comments"]>; members: NonNullable<StudioData["members"]>; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult> }) {
  const [comment, setComment] = useState(""); const [context, setContext] = useState("geral"); const task = tasks.find((item) => item.piece_id === piece.id);
  const contextPayload = context === "geral" ? {} : context.startsWith("slide") ? { slideIndex: Number(context.slice(5)) } : { cenaIndex: Number(context.slice(4)) };
  const slides = Array.isArray(version.conteudo.slides) ? version.conteudo.slides : Array.isArray(version.conteudo.stories) ? version.conteudo.stories : [];
  const cenas = Array.isArray(version.conteudo.cenas) ? version.conteudo.cenas : [];
  const contexts = [{ value: "geral", label: "Peça inteira" }, ...slides.map((_, index) => ({ value: `slide${index}`, label: `Slide ${index + 1}` })), ...cenas.map((_, index) => ({ value: `cena${index}`, label: `Cena ${index + 1}` }))];
  return <details className="studio-collaboration"><summary><strong>Colaboração e governança</strong><span>{task?.status ?? "sem responsável"}</span></summary><div className="studio-collab-form"><label className="studio-field"><span>Responsável</span><select defaultValue={task?.responsavel_id ?? ""}><option value="">Sem responsável</option>{members.map((member) => <option value={member.id} key={member.id}>{member.nome} · {member.role ?? "membro"}</option>)}</select></label><label className="studio-field"><span>Revisor</span><select defaultValue={task?.revisor_id ?? ""}><option value="">Sem revisor</option>{members.map((member) => <option value={member.id} key={member.id}>{member.nome} · {member.role ?? "membro"}</option>)}</select></label><label className="studio-field"><span>Prazo</span><input type="datetime-local" defaultValue={task?.prazo_em ? task.prazo_em.slice(0, 16) : ""}/></label><button type="button" className="studio-secondary" disabled={busy} onClick={(event) => { const form = (event.currentTarget.parentElement as HTMLElement); const selects = [...form.querySelectorAll("select")]; const input = form.querySelector("input"); void mutate({ action: "saveTask", pieceId: piece.id, responsavelId: (selects[0] as HTMLSelectElement).value, revisorId: (selects[1] as HTMLSelectElement).value, prazoEm: (input as HTMLInputElement).value ? new Date((input as HTMLInputElement).value).toISOString() : null, status: "em_andamento", pendencia: null }, "Responsável, revisor e prazo salvos."); }}>Salvar governança</button><label className="studio-field"><span>Contexto do comentário</span><select value={contexts.some((item) => item.value === context) ? context : "geral"} onChange={(event) => setContext(event.target.value)}>{contexts.map((item) => <option value={item.value} key={item.value}>{item.label}</option>)}</select></label><label className="studio-field"><span>Comentário da versão v{version.versao}</span><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Descreva o ajuste pendente"/></label><button type="button" className="studio-primary" disabled={busy || !comment.trim() || !validCommentContext(version, context === "geral" ? "geral" : context.startsWith("slide") ? "slide" : "cena", context === "geral" ? null : Number(context.replace(/\D/g, "")))} onClick={() => void mutate({ action: "addComment", pieceId: piece.id, versionId: version.id, comentario: comment, ...contextPayload }, "Comentário contextual adicionado à timeline.").then(() => setComment(""))}>Adicionar comentário</button>{comments.filter((item) => item.piece_id === piece.id).slice(0, 8).map((item) => <div className="studio-comment" key={item.id}><small>{new Date(item.criado_em).toLocaleString("pt-BR")} · {item.slide_index !== null ? `Slide ${item.slide_index + 1}` : item.cena_index !== null ? `Cena ${item.cena_index + 1}` : "Peça"}</small><span>{item.comentario}</span><button type="button" className="studio-secondary" onClick={() => void mutate({ action: "setCommentResolved", commentId: item.id, resolved: !item.resolvido_em }, item.resolvido_em ? "Pendência reaberta." : "Pendência resolvida.")}>{item.resolvido_em ? "Reabrir" : "Resolver"}</button></div>)}</div></details>;
}

export function CalendarView({ data, busy, mutate }: { data: StudioData; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult> }) {
  ...
  return <main className="studio-content"><section className="studio-section"><header><div><span className="studio-eyebrow">Agenda editorial</span><h2>Calendário de conteúdo</h2><p>Datas reais em {STUDIO_TIMEZONE}, com a versão exata de cada peça.</p></div><div className="studio-calendar-modes">{([['mes','Mês'],['semana','Semana'],['lista','Lista']] as const).map(([id,label]) => <button type="button" className={viewMode === id ? "active" : ""} onClick={() => setViewMode(id)} key={id}>{label}</button>)}</div></header>
    {grouped.length ? <div className={`studio-calendar-list ${viewMode}`}>{grouped.map(([date, rows]) => <section key={date}><header><strong>{fullDate.format(new Date(`${date}T12:00:00Z`))}</strong><small>{rows.length} publicaç{rows.length === 1 ? "ão" : "ões"}</small></header>{rows.map((schedule) => { const version = data.versions.find((item) => item.id === schedule.piece_version_id); const piece = version ? data.pieces.find((item) => item.id === version.piece_id) : null; const campaign = piece ? data.campaigns.find((item) => item.id === piece.campaign_id) : null; return <article key={schedule.id} draggable onDragEnd={() => { const next = window.prompt("Novo horário ISO para esta publicação", schedule.agendado_para); if (next && next !== schedule.agendado_para && window.confirm("Confirmar alteração de horário?")) void mutate({ action: "moveSchedule", scheduleId: schedule.id, agendadoPara: next }, "Horário alterado e conflito revalidado."); }}><span className="studio-calendar-time">{new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit", timeZone: STUDIO_TIMEZONE }).format(new Date(schedule.agendado_para))}</span><span className="studio-calendar-icon"><Icon name={piece ? FORMAT_META[piece.formato].icon : "calendar"}/></span><div><strong>{piece?.titulo ?? "Peça não encontrada"}</strong><small>{campaign?.nome} · versão {version?.versao ?? "—"}</small></div><span className={`studio-status ${schedule.status}`}>{statusLabel[schedule.status] ?? schedule.status}</span>{schedule.status === "agendado" && <button type="button" disabled={busy} onClick={() => { if (window.confirm("Confirmar o envio desta versão aprovada para a fila do Instagram?")) void mutate({ action: "preparePublication", scheduleId: schedule.id, confirm: true }, "Publicação enviada para a fila segura."); }}><Icon name="send"/> Enviar à fila</button>}</article>; })}</section>)}</div> : <EmptyState icon="calendar" title="Calendário livre" text="Aprove uma versão e escolha data e hora no construtor."/>}</section></main>;
}
...
function Settings({ data, busy, mutate }: { data: StudioData; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult> }) {
  ...
  const [canvaPiece, setCanvaPiece] = useState(data.pieces[0]?.id ?? "");
  ...
  return <main className="studio-content studio-settings"><section className="studio-section"><header><div><span className="studio-eyebrow">Integrações</span><h2>Estado real das conexões</h2><p>Nenhuma integração é mostrada como pronta sem verificação.</p></div></header><div className="studio-integrations">{(["openai", "figma", "renderer", "instagram"] as const).map((provider) => { const entry = data.integrations.find((item) => item.provider === provider); return <article key={provider}><span className={`studio-provider-mark ${provider}`}><Icon name={provider === "openai" ? "sparkles" : provider === "figma" ? "layers" : provider === "instagram" ? "send" : "image"}/></span><div><strong>{provider === "openai" ? "IA governada" : provider === "figma" ? "Figma" : provider === "instagram" ? "Instagram" : "Renderização"}</strong><small>{entry?.status === "configurada" ? "Configurada e verificada" : entry?.status === "degradada" ? "Com falha parcial" : provider === "renderer" ? "Worker externo ainda não ativado" : "Não configurada"}</small></div><span className={`studio-connection ${entry?.status ?? "nao_configurada"}`}>{entry?.status === "configurada" ? "Ativa" : entry?.status === "degradada" ? "Parcial" : "Desativada"}</span></article>; })}</div>
    ...
    <CanvaReturnPanel data={data} busy={busy} mutate={mutate} canvaPiece={canvaPiece} setCanvaPiece={setCanvaPiece}/>
...
function CanvaReturnPanel({ data, busy, mutate, canvaPiece, setCanvaPiece }: { data: StudioData; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult>; canvaPiece: string; setCanvaPiece: (value: string) => void }) {
  const [preview, setPreview] = useState<Record<string, unknown> | null>(null);
  const selected = data.pieces.find((item) => item.id === canvaPiece); const base = selected?.current_version_id ? data.versions.find((item) => item.id === selected.current_version_id) : null;
  const exportPackage = () => { const payload = { schema_version: 1, exported_at: new Date().toISOString(), piece_id: canvaPiece, base_version_id: base?.id, dimensions: { width: selected && ["story", "reel"].includes(selected.formato) ? 1080 : 1080, height: selected && ["story", "reel"].includes(selected.formato) ? 1920 : 1350 }, conteudo: base?.conteudo ?? {}, assets: data.snapshots.flatMap((item) => item.midias.map((media, index) => ({ storage_path: media.storage_path, media_index: index, authorized: true }))) }; const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "apecerto-canva-package.json"; anchor.click(); URL.revokeObjectURL(url); };
  return <section className="studio-section studio-canva"><header><div><span className="studio-eyebrow">Canva</span><h2>Exportação e retorno versionado</h2><p>O retorno só é persistido depois da prévia e confirmação.</p></div></header><div className="studio-review-actions"><button type="button" className="studio-primary" onClick={exportPackage}>Exportar pacote para Canva</button><select value={canvaPiece} onChange={(event) => { setCanvaPiece(event.target.value); setPreview(null); }} aria-label="Peça para retorno Canva"><option value="">Escolha a peça para importar</option>{data.pieces.map((item) => <option value={item.id} key={item.id}>{item.titulo}</option>)}</select><label className="studio-secondary">Selecionar retorno JSON<input type="file" accept="application/json" hidden onChange={(event) => { const file = event.target.files?.[0]; if (!file || !canvaPiece) return; void file.text().then((raw) => setPreview(JSON.parse(raw) as Record<string, unknown>)).catch(() => window.alert("Pacote Canva inválido: prévia cancelada.")); }}/></label></div>{preview && <div className="studio-canva-preview" role="dialog" aria-label="Prévia e diff do retorno Canva"><h3>Prévia antes de importar</h3><p>Versão-base: {String(preview.base_version_id ?? "ausente")} · Dimensões: {JSON.stringify(preview.dimensions ?? {})}</p><p>Antes: {String(base?.conteudo.headline ?? "")} → Depois: {String((preview.conteudo as Record<string, unknown> | undefined)?.headline ?? "")}</p><p>Estrutura e assets: {Array.isArray(preview.assets) ? preview.assets.length : 0} assets no retorno.</p><button type="button" className="studio-secondary" onClick={() => setPreview(null)}>Cancelar</button><button type="button" className="studio-primary" disabled={busy} onClick={() => void mutate({ action: "importCanvaPackage", pieceId: canvaPiece, package: preview }, "Retorno do Canva validado e salvo como nova versão.").then(() => setPreview(null))}>Confirmar importação</button></div>}</section>;
}

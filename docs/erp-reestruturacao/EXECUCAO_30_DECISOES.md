...
| 25 | Esteira cobre proposta, documentos, contrato, assinatura e pagamento. | Transições autorizadas persistem artefatos/estado e não pulam pré-condições. | entregue | PR #261 publicada no hash `77aa25ee`: o servidor exige o papel da etapa, avanço unitário e todas as pré-condições/comprovações aprovadas; o atalho em lote foi recusado. CI integral, harness 390×844/1440×900 e produção somente de leitura passaram, sem pagamento nem mutação produtiva. |
| 26 | Captação de imóvel fica vinculada ao proprietário. | Imóvel e proprietário têm vínculo canônico, autoria e privacidade por perfil. | em prova | Criação e finalização passaram a ser transações únicas: proprietário, imóvel, unidade, autor e captador são confirmados antes do sucesso; repetição tolerante a acentos retoma o mesmo rascunho e outro perfil não lê o proprietário. A prova transacional com rollback passou sem resíduo, assim como 1.157 testes, TypeScript, lint e build. Produção ainda possui 12 unidades captadas sem proprietário completo (11 aprovadas, 10 publicadas), que exigem correção humana e não foram inferidas. Falta CI, publicação e confirmação da build. |
...
Publicar e aceitar a decisão 26 sem preencher automaticamente os 12 vínculos
históricos incompletos; depois avançar na decisão 27. A decisão 20 permanece

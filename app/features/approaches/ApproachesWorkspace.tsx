...
const isMediaPart = (part: MessagePart) => part.name?.includes("media") || part.name === "send-image-message" || part.name === "send-video-message";
const partName = (part: MessagePart) => part.name === "delay" ? "Espera" : part.name === "wait-user-input" ? "Aguardar resposta" : part.name === "send-video-message" ? "Vídeo" : part.name === "send-image-message" ? "Imagem" : part.name?.includes("media") ? "Mídia" : "Mensagem";

const sampleName = "João";
const sampleBrokerName = "Fabiano";
const fillVars = (text: string) => text
  .replace(/\{\s*(primeiro_?nome|primeiroNome|nome|first_?name)\s*\}/gi, sampleName)
  .replace(/\{\s*(corretor_primeiro_nome|primeiro_nome_corretor|corretor_nome|corretor)\s*\}/gi, sampleBrokerName);
      ...
      <div className="app-wa-body">
        ...
        {enviaveis.map((part, i) => {
          ...
          if (isMediaPart(part)) {
            ...
            const video = part.name === "send-video-message" || (part.name !== "send-image-message" && isVideoUrl(url));
            return <div className="app-wa-bubble out media" key={i}>
              <div className="app-wa-media">
                {!url ? <div className="app-wa-media-empty">🎬 mídia sem link<small>cole o link do vídeo/imagem na abordagem</small></div>
                  : video ? <video src={url} controls preload="metadata" playsInline />
...
export function ApproachesWorkspace({ accessToken }: { accessToken: string }) {
  ...
  async function mutate(body: Record<string, unknown>) {
    ...
    try {
      ...
      try {
        ...
        setNotice("A alteração foi salva, mas a biblioteca não pôde ser atualizada. Recarregue antes de repetir.");
      }
    } catch { setNotice("Não foi possível confirmar a alteração. Recarregue antes de repetir."); }
  ...
  const visibleProducts = useMemo(() => {
    const productIdsWithApproaches = new Set((data?.approaches ?? []).map((item) => item.empreendimento_id).filter(Boolean));
    return (data?.products ?? []).filter((product) => productIdsWithApproaches.has(product.id));
  }, [data]);
  ...
  useEffect(() => {
    if (selectedProduct && !visibleProducts.some((product) => product.id === selectedProduct)) {
  ...
  function addPart(name: string) { if (!editing) return; const options = name === "delay" ? { valor: 5, unidade: "minutos" } : name === "send-image-message" || name === "send-video-message" ? { url: "", caption: "" } : { text: "" }; const next = [...parts, { name, options }]; setData((current) => current ? { ...current, approaches: current.approaches.map((item) => item.id === editing.id ? { ...item, mensagens: next } : item) } : current); }
  ...
  return <div className="approaches-workspace">
    ...
    {loadStatus === "loading" ? <div className="workspace-loading">Carregando biblioteca de abordagens…</div>
      : loadStatus === "error" ? <div className="workspace-error"><strong>Não foi possível carregar a biblioteca de abordagens</strong><span>{loadError}</span><button type="button" onClick={() => void load()}>Tentar novamente</button></div>
      : <div className="approaches-main">
      <aside className="approach-products"><div><strong>Produtos</strong><small>{visibleProducts.length} empreendimento{visibleProducts.length === 1 ? "" : "s"} com abordagem</small></div><button className={selectedProduct === null ? "active" : ""} type="button" onClick={() => { setSelectedProduct(null); setSelectedApproach(null); }}><span className="folder">🗀</span><b>Modelos gerais</b><em>{data?.approaches.filter((item) => !item.empreendimento_id).length ?? 0}</em></button>{visibleProducts.map((product) => <button className={selectedProduct === product.id ? "active" : ""} type="button" onClick={() => { setSelectedProduct(product.id); setSelectedApproach(null); }} key={product.id}><span>▥</span><b>{product.nome}</b><em>{data?.approaches.filter((item) => item.empreendimento_id === product.id).length ?? 0}</em></button>)}<button className="approach-add-group" type="button" onClick={createGroup}>＋ Novo grupo</button></aside>

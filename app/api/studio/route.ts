...
async function studioData(auth: Auth): Promise<StudioData> {
  ...
  const [campaigns, snapshots, pieces, versions, schedules, jobs, integrations, budgets, briefs, templates, templateVersions, tasks, comments, metrics, members] = await Promise.all([
    ...
    rest<StudioData["budgets"]>(auth, `social_budgets?${org}&mes=eq.${hojeOperacao().slice(0, 7)}-01&select=provider,limite_usd,consumido_usd`),
...
async function generatePackage(auth: Auth, campaignId: string) {
  ...
  try {
    const [integrations, budgets, campaigns, snapshots, pieces, templates, templateVersions] = await Promise.all([
      ...
      rest<Array<{ limite_usd: number; consumido_usd: number }>>(auth, `social_budgets?organization_id=eq.${STUDIO_ORGANIZATION_ID}&provider=eq.openai&mes=eq.${hojeOperacao().slice(0, 7)}-01&select=limite_usd,consumido_usd`),
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "setBudget") {
      ...
      const month = `${hojeOperacao().slice(0, 7)}-01`;

...
Deno.serve(async (req) => {
  ...
  if (!(await usuarioLogado(req))) return json({ error: "nao_autorizado" }, 401);
  ...
  if (arquivo.size > 15 * 1024 * 1024) return json({ error: "arquivo_grande", detail: "Envie imagens de ate 15 MB." }, 413);

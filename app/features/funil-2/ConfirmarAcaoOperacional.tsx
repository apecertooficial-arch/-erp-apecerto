...
export function ConfirmarAcaoOperacional({
  ...
  return <div className={`f2-confirmar-acao ${modo}`}>
    {sucesso && !aberto ? <div className="f2-confirmar-acao-sucesso" role="status"><strong>{sucesso}</strong><span>A leitura anterior permanece identificada até o processamento terminar.</span></div> : null}
    {!aberto && !sucesso ? <button type="button" className="f2-confirmar-acao-abrir" onClick={() => { setSucesso(""); setAberto(true); }}>
      Registrar ação realizada
    </button> : aberto ? <div className="f2-confirmar-acao-confirmacao" role="group" aria-label="Confirmar ação realizada">
      ...
      <label>Observação opcional<textarea value={observacao} onChange={(evento) => setObservacao(evento.target.value)} maxLength={500} placeholder="Resultado ou contexto útil para a próxima leitura da Sara" /></label>
      {erro && <em role="alert">{erro}</em>}
      <div>
        <button type="button" onClick={() => { setAberto(false); setErro(""); }} disabled={salvando}>Cancelar</button>
        <button type="button" className="primario" onClick={() => void confirmar()} disabled={salvando}>{salvando ? "Confirmando…" : "Confirmar execução"}</button>
      </div>
    </div> : null}
  </div>;
}

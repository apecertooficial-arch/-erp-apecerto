...
function bodyHtml(n){
 ...
 if(n.type==='distribution'){if(!n.opts)n.opts={};const d=n.opts.distribuicao=n.opts.distribuicao||{items:[],onlineOnly:false,tambemNegocio:false};const its=d.items=d.items||[];
  ...
  return `<div style="font-size:11px;color:var(--ink-faint);padding:2px 0 6px;line-height:1.4">Distribui o lead entre os corretores por <b>peso</b> (roleta). Quem tem peso maior recebe mais leads.</div>`+
   ...
   `<div style="height:1px;background:var(--line-soft);margin:11px 0 6px"></div><div class="ne-lb" style="margin-top:0">Proteção do dono do lead</div><div style="font-size:11px;color:var(--ink-faint);margin:2px 0 4px;line-height:1.45">Regra comercial fixa: mantém o dono somente durante <b>visita ativa</b> ou <b>negociação ativa</b>. Histórico encerrado não bloqueia uma redistribuição autorizada.</div>`+
 ...
 if(n.type==='distribution-simple'){if(!n.opts)n.opts={};
  ...
  return `<div style="font-size:11px;color:var(--ink-faint);padding:2px 0 6px;line-height:1.45">Distribui o lead em <b>sequencia proporcional aos pesos publicados</b> e segue o fluxo. <b>Nao envia abordagem</b> e <b>nao retem o lead</b> — quem manda a mensagem e o corretor, pelo celular, e o D-API confirma.</div>`+
   ...
   `<div style="font-size:11px;color:var(--ink-faint);margin:2px 0 4px;line-height:1.45">Regra comercial fixa: mantém o dono somente durante <b>visita ativa</b> ou <b>negociação ativa</b>. Histórico encerrado não bloqueia uma redistribuição autorizada.</div>`+
...
function setPaletteOpen(open){const palette=document.getElementById('palette'),toggle=document.getElementById('paletteToggle'),expanded=!!open;if(!palette||!toggle)return;palette.style.display=expanded?'block':'none';toggle.style.display=expanded?'none':'flex';toggle.setAttribute('aria-expanded',String(expanded));}
...
setupImportButton();
const paletteToggle=document.getElementById('paletteToggle');paletteToggle.title='Abrir biblioteca de módulos';paletteToggle.setAttribute('aria-controls','palette');
const paletteHeading=document.querySelector('.palette-h h3');if(paletteHeading)paletteHeading.textContent='Biblioteca de módulos';
renderPalette();

...
export function ProductDetail({ productId, accessToken, onClose, onChanged }: { productId: string; accessToken: string; onClose: () => void; onChanged: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      const response = await fetch("/api/product", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ id: productId, product: payload, owner: product?.origem === "terceiro" ? owner : null, units, condominium, origin: product?.origem }) });
      const result = await response.json();
  ...
  async function upload(files: FileList | null, forcedCategory?: string) {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Falha no upload."); } finally { setBusy(false); }
  }
        ...
        <div className="detail-scroll">
          ...
          {editing ? <div className="detail-form">
            ...
            <section className="detail-section media-library">
              <div className="section-row"><div><h3>Galeria e materiais</h3><small>{photos.length} fotos · {videos.length} vídeos · {presentations.length} apresentações</small></div></div>

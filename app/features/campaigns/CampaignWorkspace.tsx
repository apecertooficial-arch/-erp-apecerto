...
type Instance = { id: number; nome: string | null; conectada: boolean | null };
type CampaignData = { leads: Lead[]; deals: Deal[]; stages: Stage[]; approaches: Approach[]; products: Product[]; recent: Recent[]; instances: Instance[] };
...
export function CampaignWorkspace({ accessToken }: { accessToken: string }) {
  const [data, setData] = useState<CampaignData>({ leads: [], deals: [], stages: [], approaches: [], products: [], recent: [], instances: [] });
  ...
  async function load() {
    ...
    if (!response.ok) setError(body.error ?? "Não foi possível carregar os disparos."); else { setData(body); setSelectedInstances((prev) => prev.length ? prev : (body.instances ?? []).map((item) => item.id)); }
    setLoading(false);
  }
  ...
  function toggleInstance(id: number) { setSelectedInstances((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]); }
  ...
  return <div className="campaign-workspace">
    ...
    {loading ? <div className="workspace-loading">Carregando campanhas...</div> : error ? <div className="workspace-error">{error}<button type="button" onClick={() => void load()}>Tentar novamente</button></div> : <main className="campaign-main">
      ...
      </div><aside className="campaign-card cadence-card"><h2>Cadência</h2><div className="campaign-instances"><details className="instance-picker"><summary>Instâncias de envio <b>{selectedInstances.length ? `${selectedInstances.length} de ${data.instances.length}` : "nenhuma"}</b></summary><div><div className="instance-picker-tools"><button type="button" onClick={() => setSelectedInstances(data.instances.filter((inst) => inst.conectada).map((inst) => inst.id))}>✓ Selecionar todas conectadas</button><button type="button" onClick={() => setSelectedInstances(data.instances.map((inst) => inst.id))}>Todas</button><button type="button" onClick={() => setSelectedInstances([])}>Limpar</button></div>{data.instances.map((inst) => <label key={inst.id}><input type="checkbox" checked={selectedInstances.includes(inst.id)} onChange={() => toggleInstance(inst.id)} /><span><strong>{inst.nome || `Instância ${inst.id}`}</strong><small>{inst.telefone ?? ""}</small></span><i className={inst.conectada ? "connected" : ""}>{inst.conectada ? "Conectada" : "Offline"}</i></label>)}{!data.instances.length && <span className="approach-multiselect-empty">Nenhuma instância vinculada a você.</span>}</div></details><div className="selected-instance-chips">{selectedInstances.map((id) => { const inst = data.instances.find((item) => item.id === id); return inst ? <button type="button" onClick={() => toggleInstance(id)} key={id}>✓ {inst.nome || `#${id}`} ×</button> : null; })}</div>{selectedInstances.length > 0 && valid.length > 0 && <small>{valid.length} leads · ~{Math.ceil(valid.length / selectedInstances.length)} por instância · {Number(rate) * selectedInstances.length}/h no total</small>}</div><label>Velocidade<select value={rate} onChange={(event) => setRate(event.target.value)}><option value="10">Lento · 10/h</option><option value="20">Normal · 20/h</option><option value="50">Rápido · 50/h</option><option value="60">Super rápido · 60/h</option></select></label><div><label>Início<input type="time" value={startTime} onChange={(event) => setStartTime(event.target.value)} /></label><label>Fim<input type="time" value={endTime} onChange={(event) => setEndTime(event.target.value)} /></label></div><label>Data inicial<input type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></label><label>Período<select value={period} onChange={(event) => setPeriod(event.target.value)}><option value="1">1 dia</option><option value="3">3 dias</option><option value="7">7 dias</option><option value="14">14 dias</option></select></label><label>Dias<select value={days} onChange={(event) => setDays(event.target.value)}><option value="weekdays">Seg a Sex</option><option value="all">Todos os dias</option><option value="saturday">Seg a Sáb</option></select></label><button className="schedule-button" type="button" disabled={busy || origin === "excel" || !valid.length || !variants.length || !stage || !destinationStage || !selectedInstances.length} onClick={() => setReviewOpen(true)}>➤ Revisar e agendar</button>{origin === "excel" && <p>Revise e cadastre a base nova no CRM antes de agendar o envio.</p>}{notice && <div className="campaign-notice">{notice}</div>}</aside></div>
      ...
      {reviewOpen && <div className="crm-center-modal" onClick={() => setReviewOpen(false)}><form className="campaign-review" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); void schedule(); }}><header><div><span>REVISÃO OBRIGATÓRIA</span><h2>Confira antes de agendar</h2><p>Público → mensagem → instâncias/cadência → agendamento.</p></div><button type="button" onClick={() => setReviewOpen(false)}>×</button></header>
        <div className="review-grid">
          ...
          <article><small>3 · INSTÂNCIAS & CADÊNCIA</small><strong>{selectedInstances.length} instância{selectedInstances.length === 1 ? "" : "s"} · {Number(rate) * Math.max(1, selectedInstances.length)}/h no total</strong><p>{data.instances.filter((inst) => selectedInstances.includes(inst.id)).map((inst) => inst.nome || `#${inst.id}`).join(", ") || "—"}{data.instances.some((inst) => selectedInstances.includes(inst.id) && !inst.conectada) ? " · ⚠ há instância desconectada" : ""}</p></article>

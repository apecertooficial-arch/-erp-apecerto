...
test("Visão CEO usa dados reais e não mantém o objeto demo", () => {
  assert.match(ceo, /useDadosInteligencia<VisaoCeoPayload>\("empresa", accessToken, recorte\)/);
  assert.match(ceo, /leads_operacionais/);
  assert.match(ceo, /leads_carga_historica/);
  assert.doesNotMatch(ceo, /const demo/);
  assert.doesNotMatch(shell, /DEMONSTRAÇÃO|números ilustrativos/);
  assert.match(shell, /Dados reais/);
});

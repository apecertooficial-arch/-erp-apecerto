  ...
  return (
    <div className="ape-agenda">
      ...
      {!erroPendenciasResultado && (pendenciasResultadoVisiveis.length > 0 || corretorEmFoco) && <section className="ape-agenda-resultados">
        <header><div><small>RESULTADOS PENDENTES</small><h2>{pendenciasResultadoVisiveis.length} visitas {corretorEmFoco && pendenciasResultadoVisiveis.length === 0 ? "aguardam o corretor" : gerenciandoPendencias ? corretorEmFoco ? "aguardam o corretor" : "aguardam os corretores" : "precisam da sua resposta"}</h2></div><strong>{rotuloTotalResultados(`${resumoResultados.justificadas ?? 0} concluídas`, Boolean(corretorEmFoco))}</strong></header>
        ...
        {pendenciasResultadoVisiveis.length > 0 && <p>{gerenciandoPendencias ? "Cobre o responsável. A pendência sai da fila quando o corretor registra um desfecho válido." : "Informe o que aconteceu. A visita continuará aqui até receber desfecho e justificativa."}</p>}

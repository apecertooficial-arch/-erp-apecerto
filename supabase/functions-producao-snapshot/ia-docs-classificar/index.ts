  ...
  try {
    ...
    try {
      ...
      if (part) {
        ...
        if (payload?.sub) {
          atorId = payload.sub;
          const { data: u } = await supabase.from("usuarios").select("nome").eq("id", payload.sub).maybeSingle();
          atorNome = u?.nome ?? payload.email ?? null;
        }
      }
    ...
    if (anexErr) return json({ ok: false, reason: "erro_consulta", detalhe: anexErr.message }, 502);
    ...
    return new Response(JSON.stringify({ ok: false, reason: "excecao", detalhe: String(e) }), {

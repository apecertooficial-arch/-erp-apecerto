...
type ResultadoPendente = { id: string; data: string; hora: string; cliente: string; produto: string | null; status: string; corretor: string; corretor_id: number | null; meu: boolean };
type ResumoResultados = { total?: number; pendentes?: number; passadas_sem_desfecho?: number; realizadas_sem_feedback?: number; canceladas_sem_motivo?: number; nao_compareceu_sem_motivo?: number; justificadas?: number; futuras?: number };
...
type CrmData = { brokers: Broker[]; leads: Lead[]; deals: Deal[]; products: Product[]; visits: Visit[]; tasks: Task[]; role?: string; gerentes?: Gerente[]; pendencias_resultado?: ResultadoPendente[]; resumo_resultados?: ResumoResultados; pendencias_resultado_erro?: string | null; performance_feedback?: PerformanceFeedback };
...
export function CalendarWorkspace({ accessToken, corretorIdInicial = null }: { accessToken: string; corretorIdInicial?: string | null }) {
  ...
  const pendenciaSelecionada = selected?.type === "visit"
    ? (data.pendencias_resultado ?? []).find((item) => item.id === selected.id)
    : null;
  ...
  const leadFullById = useMemo(() => new Map(data.leads.map((item) => [item.id, item])), [data.leads]);
  ...
  function mostrarAviso(texto: string, tipoAviso: "success" | "error") {
    setNoticeKind(tipoAviso);
    setNotice(texto);
  }
  async function createVisit() {
    if (creatingVisitRef.current) return;
    ...
    finally { creatingVisitRef.current = false; setCreatingVisit(false); }
  }
  ...
  async function saveEdit() {
    ...
    try {
      ...
      setEditing(null);
      mostrarAviso(body.message ?? "Visita remarcada com sucesso.", "success");
      await load();
    } catch {
      mostrarAviso("Não foi possível salvar. Verifique a conexão e tente novamente.", "error");
    } finally {
      setSavingEdit(false);
    }
  }
  async function registrarResultado(dados: { status: StatusResultadoVisita; resultadoCodigo: string; justificativa: string }) {
    if (!resultadoPendente) return;
    setSalvandoResultado(true); setErroResultado(""); setNotice("");
    try {
      const response = await fetch("/api/agenda", {
        method: "PATCH",
        headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
      ...
      setErroResultado("Não foi possível salvar. Verifique a conexão e tente novamente.");
  ...
  return <div className="calendar-workspace" onMouseUp={() => finishDrag()} onMouseLeave={() => setDrag(null)}>
    ...
    {loading ? <div className="workspace-loading">Carregando agenda...</div> : error ? <div className="workspace-error">{error}<button type="button" onClick={() => void load()}>Tentar novamente</button></div> : <section className="calendar-main" aria-label="Agenda">
      ...
      {isAdmin && <section className="calendar-feedback-performance" aria-label="Qualidade dos feedbacks de visita">
        <header><div><small>QUALIDADE DOS FEEDBACKS</small><h2>Série 0–10 por corretor</h2><p>{data.performance_feedback?.status === "ok" ? `${formatarRecorteFeedback(data.performance_feedback.inicio, data.performance_feedback.fim)} · ${data.performance_feedback.estruturados_total ?? 0} de ${data.performance_feedback.historico_total ?? 0} resultados confirmados têm formulário estruturado. O histórico legado continua preservado, sem avaliação retroativa.` : "Somente respostas estruturadas e confirmadas recebem nota; nenhuma métrica é estimada."}</p></div>{data.performance_feedback?.status === "ok" && <strong>{data.performance_feedback.estruturados_total ?? 0}/{data.performance_feedback.historico_total ?? 0} avaliados</strong>}</header>
        {data.performance_feedback?.status === "ok" && (data.performance_feedback.estruturados_total ?? 0) > 0
          ? <div className="calendar-feedback-performance-lista">{(data.performance_feedback.itens ?? []).map((item) => <article key={item.corretor_id ?? item.corretor}><strong>{item.corretor}</strong><span><b>{item.nota_media.toFixed(1)}</b>/10 média</span><small>{item.feedbacks} feedback{item.feedbacks === 1 ? "" : "s"} · {item.dentro_prazo_total}/{item.feedbacks} no prazo ({item.dentro_prazo_percentual.toFixed(0)}%) até {data.performance_feedback?.feedback_visita_min ?? "—"} min · resposta média {item.resposta_media_min} min</small></article>)}</div>
          : <div className="calendar-feedback-baseline"><strong>Baseline ainda indisponível</strong><p>{data.performance_feedback?.status === "ok" ? `${data.performance_feedback.legados_total ?? 0} resultados legados foram preservados e não serão pontuados.` : "O contrato estruturado ainda não está ativo no banco. Nenhuma nota será estimada a partir de texto antigo."}</p></div>}
      </section>}
      ...
      {notice && <div className={`calendar-notice ${noticeKind}`} role={noticeKind === "error" ? "alert" : "status"} aria-live={noticeKind === "error" ? "assertive" : "polite"} aria-atomic="true"><span aria-hidden="true">{noticeKind === "error" ? "!" : "✓"}</span><strong>{notice}</strong><button type="button" aria-label="Fechar mensagem" onClick={() => setNotice("")}>×</button></div>}
    </section>}
    {selected && <div className="calendar-modal-layer" onClick={() => setSelected(null)}><aside className="calendar-detail" onClick={(event) => event.stopPropagation()}><header><div><small>{selected.type === "visit" ? "VISITA" : "TAREFA"}</small><h2>{selected.label}</h2><p>{new Date(`${selected.date}T12:00:00`).toLocaleDateString("pt-BR")} · {selected.time}{selected.endTime ? ` – ${selected.endTime}` : ""}</p></div><button type="button" onClick={() => setSelected(null)}>×</button></header><dl><div><dt>Corretor</dt><dd>{selected.brokerId ? brokerById.get(selected.brokerId) ?? "—" : "—"}</dd></div><div><dt>Produto</dt><dd>{selected.product || "— (defina em Editar)"}</dd></div>{selected.type === "visit" && <div><dt>Local</dt><dd>{(selected.raw as Visit).local || "—"}</dd></div>}<div><dt>Status</dt><dd>{selected.status}</dd></div><div><dt>Acompanhamento</dt><dd>{selected.manager ? `Com gerente${(() => { const gid = (selected.raw as Visit).gerente_id; const gn = (data.gerentes ?? []).find((g) => g.id === gid)?.nome ?? gerenteNomeGeral; return gn ? ` · ${gn}` : ""; })()}` : "Sem gerente"}</dd></div></dl>{selected.type === "visit" && (() => { const lid = (selected.raw as Visit).lead_id; const lead = lid ? leadFullById.get(lid) : null; if (!lead) return null; const tel = (lead.telefone || "").replace(/\D/g, ""); const wa = tel ? (tel.length > 11 ? tel : `55${tel}`) : ""; return <div className="cal-lead-card"><span className="cal-lead-label">Lead da visita</span><div className="cal-lead-head"><span className="cal-lead-av">{(lead.nome || "?").trim().slice(0, 1).toUpperCase()}</span><div><strong>{lead.nome || `Lead #${lead.id}`}</strong><small>{[lead.status, lead.origem].filter(Boolean).join(" · ") || "Lead"}</small></div></div><div className="cal-lead-acts">{lead.telefone && <a href={`tel:${lead.telefone}`}>☎ {lead.telefone}</a>}{wa && <a href={`https://wa.me/${wa}`} target="_blank" rel="noreferrer">WhatsApp →</a>}{lead.email && <a href={`mailto:${lead.email}`}>✉ {lead.email}</a>}{!lead.telefone && !lead.email && <span className="cal-lead-empty">Sem contato cadastrado</span>}</div></div>; })()}{selected.type === "visit" && <footer><button type="button" onClick={() => openEdit(selected.raw as Visit)}>✎ Editar</button>{pendenciaSelecionada?.meu === true && <button className="confirm" type="button" onClick={() => { setErroResultado(""); setResultadoPendente(pendenciaSelecionada); }}>Registrar resultado</button>}</footer>}</aside></div>}
    {resultadoPendente && <div className="calendar-modal-layer" onClick={() => setResultadoPendente(null)}><div onClick={(evento) => evento.stopPropagation()}><ResultadoVisitaForm cliente={resultadoPendente.cliente} dataHora={`${new Date(`${resultadoPendente.data}T12:00:00`).toLocaleDateString("pt-BR")} · ${resultadoPendente.hora}`} statusInicial={(["realizada", "cancelada", "nao_compareceu"].includes(resultadoPendente.status) ? resultadoPendente.status : "realizada") as StatusResultadoVisita} visitId={resultadoPendente.id} accessToken={accessToken} busy={salvandoResultado} erro={erroResultado} onCancelar={() => setResultadoPendente(null)} onSalvar={(dados) => void registrarResultado(dados)} /></div></div>}
    {creating && <div className="calendar-modal-layer" onClick={() => setCreating(false)}><form className="calendar-create" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); void createVisit(); }}><header><div><h2>Agendar visita</h2><p>A visita ficará vinculada ao lead e ao produto.</p></div><button type="button" onClick={() => setCreating(false)}>×</button></header><div><label>Lead<select required value={form.dealId} onChange={(event) => setForm({ ...form, dealId: event.target.value })}><option value="">Selecione</option>{data.deals.map((deal) => <option value={deal.id} key={deal.id}>{leadById.get(deal.lead_id) ?? `Lead #${deal.lead_id}`}</option>)}</select></label><label>Produto<select value={form.productId} onChange={(event) => setForm({ ...form, productId: event.target.value })}><option value="">Selecionar depois</option>{data.products.map((item) => <option value={item.id} key={item.id}>{item.nome}</option>)}</select></label><label>Data<input required type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} /></label><label>Início<input required type="time" value={form.startTime} onChange={(event) => setForm({ ...form, startTime: event.target.value })} /></label><label>Fim<input type="time" value={form.endTime} onChange={(event) => setForm({ ...form, endTime: event.target.value })} /></label><label className="wide">Local<input value={form.local} onChange={(event) => setForm({ ...form, local: event.target.value })} placeholder="Preenchido pelo endereço do produto" /></label><label className="wide">Observações<textarea value={form.observations} onChange={(event) => setForm({ ...form, observations: event.target.value })} /></label><label className="check"><input type="checkbox" checked={form.reminder} onChange={(event) => setForm({ ...form, reminder: event.target.checked })} /> Criar lembrete</label><label className="check"><input type="checkbox" checked={form.withManager} onChange={(event) => setForm({ ...form, withManager: event.target.checked, gerenteId: event.target.checked ? (form.gerenteId || String(geralGerenteId ?? "")) : "" })} /> Com gerente</label>{form.withManager && <label className="wide">Gerente que vai atender<select value={form.gerenteId || String(geralGerenteId ?? "")} onChange={(event) => setForm({ ...form, gerenteId: event.target.value })}>{(data.gerentes ?? []).map((g) => <option value={String(g.id)} key={g.id}>{g.nome}{g.geral ? " (geral)" : ""}</option>)}</select></label>}</div><footer><button type="button" onClick={() => setCreating(false)}>Cancelar</button><button className="confirm" type="submit" disabled={creatingVisit}>{creatingVisit ? "Agendando…" : "Agendar visita"}</button></footer></form></div>}
    {editing && <div className="calendar-modal-layer" onClick={() => setEditing(null)}>
      <form className="calendar-create" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); void saveEdit(); }}>
        <header><div><h2>Editar visita</h2><p>{editing.cliente_nome || "Cliente"}{editing.produto ? ` · ${editing.produto}` : ""}{isAdmin ? "" : " · você ajusta o horário"}</p></div><button type="button" onClick={() => setEditing(null)}>×</button></header>
        <div>
          <label>Data<input required type="date" value={editForm.date} onChange={(event) => setEditForm({ ...editForm, date: event.target.value })} /></label>
          <label>Início<input required type="time" value={editForm.startTime} onChange={(event) => setEditForm({ ...editForm, startTime: event.target.value })} /></label>
          <label>Fim<input type="time" value={editForm.endTime} onChange={(event) => setEditForm({ ...editForm, endTime: event.target.value })} /></label>
          <label>Produto (imóvel da visita)<select value={editForm.productId} onChange={(event) => setEditForm({ ...editForm, productId: event.target.value })}><option value="">Sem produto</option>{data.products.map((item) => <option value={item.id} key={item.id}>{item.nome}</option>)}</select></label>
          <label>Local (endereço — onde ir)<input value={editForm.local} onChange={(event) => setEditForm({ ...editForm, local: event.target.value })} placeholder="Auto-preenchido pelo endereço do produto" /></label>

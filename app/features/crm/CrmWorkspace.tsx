...
function LeadChatDrawer({ accessToken, lead, deal, corretorNome, onClose, onResponse }: { accessToken: string; lead: Lead; deal: Deal; corretorNome?: string; onClose: () => void; onResponse: () => Promise<void> }) {
  ...
  async function sendSuggestion(text: string) {
    ...
    setMessages((prev) => [...prev, { id: tempId, wa_message_id: null, direcao: "enviada", tipo: "texto", conteudo: text, media_url: null, criado_em: new Date().toISOString() }]);
    try { await request({ action: "send", telefone: lead.telefone, instanciaId: chosen.sendBig, texto: text }); await onResponse(); await loadMessages(chosen); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível enviar."); setMessages((prev) => prev.map((m) => m.id === tempId ? { ...m, conteudo: `${m.conteudo || ""} ⚠️ (falha)` } : m)); }
  }

-- Funil 2.0 — pesca pelo corretor, com dono gravado.
--
-- Dois problemas resolvidos:
--
-- 1) f2_pescar_negocio inseria corretor_id NULL. O trigger de aviso ignora
--    linha sem corretor, entao a pesca nunca notificava ninguem e o card
--    nascia orfao. Agora o pescador vira o dono no mesmo INSERT e o push sai.
--
-- 2) A pesca era exclusiva de f2_admin(). Por decisao operacional (04/08/2026)
--    qualquer corretor pesca, SEM trava de aptidao.
--
-- A trava de aptidao foi dispensada de proposito. Em troca, quando o pescador
-- esta sem D-API conectada registramos alerta NAO-BLOQUEANTE em
-- motor_execucoes: o lead entra, mas o risco fica visivel em vez de silencioso.
...
BEGIN
  ...
  SELECT l.nome, l.telefone INTO v_nome, v_tel
    FROM public.negocios n
    JOIN public.leads l ON l.id = n.lead_id
  ...
  IF v_corretor IS NOT NULL THEN
    ...
    UPDATE public.leads SET corretor_id = v_corretor
     WHERE id = (SELECT lead_id FROM public.negocios WHERE id = p_negocio_id)

      ...
      <header className="f2-ficha-cabecalho">
        <div className="f2-ficha-linha-topo">
          ...
          <div className="f2-ficha-acoes-topo">
            ...
            <span><button type="button" aria-expanded={maisAcoes} onClick={() => setMaisAcoes((valor) => !valor)}>Mais <b>⌄</b></button>{maisAcoes && <div className="f2-mais-menu" role="menu"><button type="button" onClick={() => { setMaisAcoes(false); onAgendarVisita(); }}>Agendar visita</button><button type="button" onClick={() => { setMaisAcoes(false); onGerarNegociacao(); }}>Gerar negociação</button><button type="button" onClick={() => { setMaisAcoes(false); setTagAberta(true); }}>Adicionar tag</button><hr /><button type="button" className="risco" onClick={() => { setMaisAcoes(false); onDescartar(); }}>Descartar lead</button></div>}</span>
      ...
      <div className="f2-ficha-grade">
        ...
        <div id={`${painelFichaId}-painel`} className="f2-ficha-painel" role="tabpanel" aria-labelledby={`${painelFichaId}-${abaDetalhe}-tab`}>
          {abaDetalhe === "atendimento" && <>
            <section className="f2-ficha-bloco"><h3>Classificação do atendimento</h3><div className="f2-ficha-classificacao"><label>Momento<select value={codigo} onChange={(e) => setCodigo(e.target.value)}>{momentos.map((m) => <option key={m.codigo} value={m.codigo}>{m.rotulo}</option>)}</select></label><label>Temperatura<select value={temperatura ?? "aguardando"} disabled={busy} onChange={(e) => { const valor = e.target.value; void onTemperatura(valor === "aguardando" ? null : valor as TemperaturaLead); }}>{TEMPERATURAS.map((item) => <option key={item.codigo} value={item.codigo}>{item.rotulo}</option>)}</select></label><label>Prazo da ação<input readOnly value={situacao.rotulo} /><small>Prazo padrão: {momento.prazo_rotulo}</small></label></div>{codigo === "RETORNO_PROGRAMADO" && <label>Data e hora combinadas<input type="datetime-local" value={prazo} onChange={(e) => setPrazo(e.target.value)} /></label>}<label>Observação<textarea value={obs} onChange={(e) => setObs(e.target.value)} placeholder="Por que este é o momento correto?" maxLength={500} /></label><button type="button" className="f2-secundario" disabled={busy} onClick={() => onMomento(codigo, prazo, obs)}>{busy ? "Atualizando…" : codigo === lead.momento_codigo ? "Continua neste momento · atualizar prazo" : "Salvar classificação"}</button></section>
            <section className="f2-ficha-bloco sara"><header><span>Sara</span><em>automático</em></header><p>{lead.ultima_reavaliacao_resumo ?? "Ainda não existe uma leitura resumida."}</p>{lead.qualidade_atendimento_resumo && <small>{lead.qualidade_atendimento_resumo}</small>}<div><button type="button" className="f2-secundario" disabled={busy} onClick={() => onMomento(codigo, prazo, obs)}>Aplicar momento sugerido</button><button type="button" onClick={() => setAbaDetalhe("historico")}>Ver evidências</button></div></section>
            <section className="f2-ficha-bloco"><header><h3>Conversa</h3><small>Começa no instante da entrada no funil</small><button type="button" className="f2-secundario" onClick={() => setChatAberto(true)}>Abrir conversa completa</button></header><div className="f2-ficha-conversa-vazia"><p>As mensagens permanecem no histórico canônico da conversa.</p><small>O envio e a conclusão dependem da confirmação real do D-API.</small></div></section>
            <section className="f2-ficha-bloco"><header><h3>Comentários e notas</h3><button type="button" className="f2-secundario" onClick={() => setAbaDetalhe("historico")}>+ Comentário</button></header>{notas.slice(0, 2).map((item) => <article className="f2-ficha-nota" key={item.id}><span>{item.texto}</span><small>{item.autor_nome ?? "Equipe"} · {dataCurta(item.criado_em)} · nota interna</small></article>)}{notas.length === 0 && <p>Nenhuma nota escrita ainda.</p>}</section>
          </>}

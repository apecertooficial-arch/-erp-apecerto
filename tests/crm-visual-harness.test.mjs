...
test("harness reproduz item malformado na Agenda móvel", () => {
  assert.match(harness, /itemAgendaInvalido/);
...
test("harness reproduz chave pública inválida do push", () => {
  assert.match(harness, /chavePushInvalida/);
...
test("harness reproduz tag malformada ao abrir a ação da ficha", () => {
...
test("harness reproduz item malformado na carteira antiga", () => {
  assert.match(harness, /carteiraItemInvalido/);

...
export async function POST(request: Request) {
  try {
    ...
    if (action === "importCanvaPackage") { requirePermission(auth, "editar"); const pkg = jsonObject(body.package) ? body.package : {}; const pieceId = clean(body.pieceId, 80); const content = jsonObject(pkg.conteudo) ? pkg.conteudo : null; if (!pieceId || !content || !clean(content.headline, 120) || !clean(content.legenda, 2200) || !clean(content.cta, 80)) throw new StudioError("Pacote Canva inválido: conteúdo obrigatório ausente.", 422, "invalid_canva_package"); return Response.json(await createPersistedVariant(auth, { pieceId, conteudo: content, changeScope: "canva_import" })); }
    ...
    if (action === "setCommentResolved") { requirePermission(auth, "revisar"); const id = clean(body.commentId, 80); if (!id) throw new StudioError("Comentário obrigatório.", 422, "invalid_comment"); await rest(auth, `social_piece_comments?id=eq.${encodeURIComponent(id)}`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ resolvido_em: body.resolved === true ? new Date().toISOString() : null }) }); return Response.json({ ok: true, resolved: body.resolved === true }); }

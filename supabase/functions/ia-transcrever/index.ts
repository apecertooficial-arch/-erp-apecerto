...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fase 0 (15/09/2026): so chamada de maquina (cron) com token interno do Vault.
    // A chave anon passava no gateway e expunha trechos de audio de clientes.
    const interno = req.headers.get("x-envio-interno") ?? "";
    if (!interno) return json({ ok:false, reason:"nao_autorizado" }, 401);
    const { data: tokenOk } = await supabase.rpc("ncrm_envio_token_valido", { p_token: interno });
    if (tokenOk !== true) return json({ ok:false, reason:"nao_autorizado" }, 401);
    const b = await req.json().catch(()=>({}));
    const limite = Math.min(Math.max(Number(b.limite ?? 6) || 6, 1), 25);
    const modelo = "gpt-4o-mini-transcribe"; // fixo: nao aceita modelo vindo do corpo
    ...
    const leadBusca = typeof b.lead === "string" ? b.lead.replace(/[^\p{L}\p{N} ]/gu, "").trim().slice(0, 60) : "";
    if (leadBusca) {
      const { data: l } = await supabase.from("leads").select("id").or(`nome.ilike.%${leadBusca}%,telefone.ilike.%${leadBusca}%`).order("atualizado_em",{ascending:false,nullsFirst:false}).limit(1).maybeSingle();
    ...
    for (const a of audios) {
      try {
        ...
        ok++; itens.push({id:a.id, formato: ext, caracteres: texto.length});

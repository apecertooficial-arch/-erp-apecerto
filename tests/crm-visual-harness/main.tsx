...
window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (url.pathname === "/api/funil2") {
    ...
    return json(colecaoCrmMalformada === "etapas" ? { ...payloadNormal, etapas: [null] } : colecaoCrmMalformada === "momentos" ? { ...payloadNormal, momentos: [null] } : tarefaMalformada ? { ...payloadNormal, leads: [null] } : payloadMobileInvalido ? { ...payloadNormal, momentos: undefined } : estado === "vazio" ? payloadVazio : payloadTarefas ?? (relogioNoLimite ? payloadRelogioNoLimite : saraPendente ? payloadSaraPendente : payloadNormal));
  }

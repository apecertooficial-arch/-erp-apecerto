...
export function ApproachesWorkspace({ accessToken }: { accessToken: string }) {
  ...
  const preview = (approach: Approach) => { if (!Array.isArray(approach.mensagens)) return ""; const first = (approach.mensagens as MessagePart[]).find((part) => part.options?.text); return first?.options?.text?.slice(0, 90) ?? ""; };
  ...
  return <div className="approaches-workspace">
    ...
    <main>
      <aside className="approach-products"><div><strong>Produtos</strong><small>{data?.products.length ?? 0} empreendimentos</small></div><button className={selectedProduct === null ? "active" : ""} type="button" onClick={() => { setSelectedProduct(null); setSelectedApproach(null); }}><span>◇</span><b>Modelos gerais</b><em>{data?.approaches.filter((item) => !item.empreendimento_id).length ?? 0}</em></button>{data?.products.map((product) => <button className={selectedProduct === product.id ? "active" : ""} type="button" onClick={() => { setSelectedProduct(product.id); setSelectedApproach(null); }} key={product.id}><span>▥</span><b>{product.nome}</b><em>{data.approaches.filter((item) => item.empreendimento_id === product.id).length}</em></button>)}</aside>
      ...
      {groups.map(([groupName, items]) => <div className="approach-group" key={groupName}><div className="approach-group-head"><strong>▤ {groupName}</strong><span>{items.length} abordagem{items.length === 1 ? "" : "s"}</span><button type="button" onClick={() => createApproach(groupName)}>＋ neste grupo</button><button type="button" title="Renomear grupo" onClick={() => renameGroup(groupName)}>✎</button>{groupName !== "Sem grupo" && <button type="button" title="Dissolver grupo (abordagens vão para Sem grupo)" onClick={() => dissolveGroup(groupName)}>⌦</button>}</div>{items.map((approach) => { const count = Array.isArray(approach.mensagens) ? approach.mensagens.length : 0; const snippet = preview(approach); return <article className={!approach.ativo ? "inactive" : ""} key={approach.id}><span>✉</span><div><strong>{approach.nome}</strong><small>{count} parte{count === 1 ? "" : "s"} · {approach.ativo ? "Ativa" : "Arquivada"}</small>{snippet && <em className="approach-preview">"{snippet}…"</em>}</div><button type="button" onClick={() => setSelectedApproach(approach.id)}>Editar mensagens</button><button type="button" title="Duplicar" aria-label="Duplicar" disabled={busy} onClick={() => duplicateApproach(approach)}>⧉</button><button type="button" aria-label={approach.ativo ? "Arquivar" : "Ativar"} title={approach.ativo ? "Arquivar" : "Ativar"} onClick={() => void mutate({ action: "toggleApproach", id: approach.id, active: !approach.ativo })}>{approach.ativo ? "○" : "●"}</button><button className="danger" type="button" aria-label="Excluir" onClick={() => window.confirm("Excluir esta abordagem?") && void mutate({ action: "deleteApproach", id: approach.id })}>⌫</button></article>; })}</div>)}{!approaches.length && <div className="approach-empty"><span>✉</span><strong>Nenhuma abordagem aqui</strong><p>Crie a primeira abordagem para este produto — escolha um grupo (ex.: Padrão, Unidade promocional).</p></div>}</section>
    </main>

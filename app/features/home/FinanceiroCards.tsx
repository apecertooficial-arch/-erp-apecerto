...
type Financeiro = {
  ...
  negociacao_vgv: number; negociacao_count: number;
  meta_mes: number;
...
export function FinanceiroCards({ accessToken, onNavigate }: { accessToken: string; onNavigate?: (module: string) => void }) {
  ...
  const cards = useMemo<CardSpec[]>(() => {
    ...
    return [
      {
        key: "vgv", icon: "↗", tone: "orange", label: "VGV do mês", value: compact.format(data.month_vgv), foot: `meta ${compact.format(data.meta_mes)} · ${compact.format(data.negociacao_vgv ?? 0)} em negociação`,
        micro: <Sparkline data={data.by_month.map((x) => Math.round(x.vgv))} color={TONE.orange.c} tint={TONE.orange.t} />,
        title: "VGV vendido", subtitle: "Vendas concluídas — últimos 6 meses", legend: `Só entra aqui a venda concluída, contada no mês em que fechou. Hoje há ${compact.format(data.negociacao_vgv ?? 0)} em ${data.negociacao_count ?? 0} venda(s) em negociação na esteira.`,

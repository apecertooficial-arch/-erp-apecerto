...
type MediaItem = { id: string; file: File; kind: "foto" | "video"; category: string; altText: string; cover: boolean; preview: string };
...
const emptyProperty = { name: "", title: "", slogan: "", description: "", purpose: "venda", amenities: "", differentiators: "", developer: "", status: "pronto" as const, price: "", condominiumFee: "", propertyTax: "", otherCosts: "", area: "", bedrooms: "", suites: "", bathrooms: "", parking: "", alreadyRented: false };
...
export function CaptureWizard({ onClose, onSaved, initialStandalone = false }: CaptureWizardProps) {
  ...
  function validateCurrentStep() {
    ...
    if (step === 5 && media.some((item) => item.kind === "foto" && item.altText.trim().length > 0 && item.altText.trim().length < 3)) return "A descrição acessível de cada foto deve ter pelo menos 3 caracteres ou ficar vazia.";
    return "";
  }
  ...
  function addFiles(files: FileList | File[] | null, kind: "foto" | "video") {
    ...
    setMedia((current) => {
      ...
      const additions = list.map((file, index) => ({
        id: crypto.randomUUID(), file, kind, category: kind === "video" ? "Tour" : "Sala", altText: "", cover: kind === "foto" && !hasCover && index === 0, preview: URL.createObjectURL(file),
      }));
      ...
      for (let index = 0; index < media.length; index += 1) {
        ...
        try {
          ...
          const { error: mediaError } = await supabase.from("midias").upsert({
            ...
            categoria: item.category.toLowerCase(), alt_text: item.altText.trim() || null, ordem: index, is_capa: item.id === coverPhotoId,

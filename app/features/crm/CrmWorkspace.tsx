...
type LeadTrajetoria = { entrou_em: string | null; dias_desde_entrada: number | null; corretor: string | null; etapa_atual: string | null; dias_na_etapa_atual: number | null; total_movimentacoes: number | null; trajetoria_detalhada: Array<{ de: string | null; para: string | null; em: string }> };
...
function LeadDrawer({ accessToken, lead, deal, data, canReassign, canMoveDeals, onClose, onMutate, onReload, setMessage }: { accessToken: string; lead: Lead; deal: Deal; data: CrmData; canReassign: boolean; canMoveDeals?: boolean; onClose: () => void; onMutate: (body: Record<string, unknown>) => Promise<void>; onReload: () => Promise<void>; setMessage: (value: string | null) => void }) {
  ...
  const [traj, setTraj] = useState<LeadTrajetoria | null>(null);
  useEffect(() => {
    let alive = true;
    void (async () => {
      try {
        const client = getBrowserSupabaseClient() as unknown as { rpc: (fn: string, args: Record<string, unknown>) => Promise<{ data: unknown }> };
        const { data: result } = await client.rpc("lead_trajetoria", { p_lead: lead.id });
        if (alive) setTraj((result as LeadTrajetoria) ?? null);
      } catch { /* trajetória é opcional; nunca quebra o drawer */ }
    })();
    return () => { alive = false; };
  }, [lead.id]);

...
type Media = { id: string; tipo: "foto" | "video" | "pdf" | "apresentacao"; storage_path: string; categoria: string | null; nome: string | null; is_capa: boolean; url: string | null; unidade_id?: string | null; ordem: number; alt_text: string | null };
type Unit = { id: string; codigo?: string | null; numero: string | null; tipologia: string | null; area_m2: number | null; vagas: number | null; valor_tabela: number | null; valor_promo: number | null; condominio_valor?: number | null; iptu?: number | null; outros_custos?: number | null; compre_ja_alugado?: boolean; disponivel: boolean; publicado?: boolean; de_terceiros?: boolean; captador_nome?: string | null; proprietario_nome?: string | null; proprietario_contato?: string | null; acesso_tipo?: string | null; acesso_codigo?: string | null; acesso_instrucoes?: string | null; titulo_comercial?: string | null; descricao_comercial?: string | null; seo_titulo?: string | null; seo_descricao?: string | null; aprovacao?: string | null; reprovacao_motivo?: string | null; mine?: boolean; pode_editar?: boolean; pode_ver_proprietario?: boolean; owner_complete?: boolean };
...
type ProductDetailData = {
  ...
  seo_titulo?: string | null; seo_descricao?: string | null;
...
export function ProductDetail({ productId, accessToken, sessionRole = "corretor", initialUnitId, initialUnitAction = "view", initialEditing = false, captadorScore = null, onClose, onChanged }: { productId: string; accessToken: string; sessionRole?: string; initialUnitId?: string | null; initialUnitAction?: UnitOpenAction; initialEditing?: boolean; captadorScore?: number | null; onClose: () => void; onChanged: () => void }) {
  ...
  const focusedUnitRecommendations = useMemo<UnitRecommendation[]>(() => {
    ...
    if (!focusedUnitChecks["Descrição comercial"]) items.push({ key: "descricao", severity: "Bloqueador", title: "Completar a descrição comercial", evidence: "A descrição do imóvel precisa ter pelo menos 80 caracteres para comunicar seus diferenciais.", action: "Editar texto", target: "dados" });
  ...
  async function mediaAction(action: "updateMedia" | "deleteMedia", mediaId: string, categoryValue?: string, altTextValue?: string) {
    setBusy(true); setMessage("");
    try {
      const response = await fetch("/api/product", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ id: productId, action, mediaId, category: categoryValue, ...(altTextValue !== undefined ? { altText: altTextValue } : {}) }) });
  ...
  function mediaPeers(item: Media) {
    return (product?.midias ?? []).filter((candidate) => candidate.unidade_id === item.unidade_id && candidate.tipo === item.tipo);
  }
  ...
  async function reorderMedia(items: Media[], mediaId: string, direction: -1 | 1) {
    ...
    setBusy(true); setMessage("");
    try {
      const response = await fetch("/api/product", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ id: productId, action: "reorderMedia", mediaIds: reordered.map((item) => item.id) }) });
      await productResponse(response, "Não foi possível reordenar a galeria.");
      await load(); onChanged(); setMessage("Ordem da galeria atualizada.");
    } catch (error) { setMessage(productFailureMessage(error, "Erro ao reordenar a galeria.")); } finally { setBusy(false); }
  }
    ...
    try {
      ...
      for (const [fileIndex, originalFile] of pendingFiles.entries()) {
        try {
          ...
          arquivosComFalha.push(`${fileIndex + 1}. ${originalFile.name}: ${productFailureMessage(reason, "falha no envio ou registro")}`);
        }
      }
      await load(); onChanged();
      if (arquivosComFalha.length) throw new ProductClientError(`${enviadaComSucesso} arquivo(s) enviado(s). Não chegaram: ${arquivosComFalha.join("; ")}`);
  ...
  const mediaLibrary = product && <section className="detail-section media-library fv2-media">
    ...
    {visibleMedia.length ? <div className="detail-gallery">{visibleMedia.map((item) => <article key={item.id}>
      ...
      {item.tipo === "foto" && item.url ? <button className="gallery-open watermarked-preview" type="button" onClick={() => setLightboxIndex(photos.findIndex((photo) => photo.id === item.id))}><img src={item.url} alt={item.alt_text || item.categoria || item.nome || "Foto do imóvel"} onError={retryMediaImage} /></button> : item.tipo === "video" && item.url ? <div className="watermarked-preview"><video src={item.url} controls preload="metadata" /></div> : item.url ? <button className="file-tile watermarked-preview" type="button" onClick={() => setDocumentPreview(item)}>Abrir {item.tipo === "pdf" ? "PDF" : "apresentação"}</button> : <div className="file-tile watermarked-preview">{item.tipo.toUpperCase()}</div>}
      {editImages ? <div><select aria-label={`Classificação de ${item.nome ?? "material"}`} value={item.categoria ?? "Outros"} onChange={(event) => void mediaAction("updateMedia", item.id, event.target.value, item.alt_text ?? "")}>{mediaCategories.map((entry) => <option key={entry}>{entry}</option>)}</select>{item.tipo === "foto" && <input key={`${item.id}:${item.alt_text ?? ""}`} className="media-alt-input" aria-label={`Descrição acessível de ${item.nome ?? "foto"}`} maxLength={160} defaultValue={item.alt_text ?? ""} placeholder="Descrição acessível da foto" onBlur={(event) => { if (event.currentTarget.value.trim() !== (item.alt_text ?? "")) void mediaAction("updateMedia", item.id, item.categoria ?? "Outros", event.currentTarget.value); }} />}<small>{item.nome}</small><div className="media-order-actions"><button disabled={busy || mediaPeers(item)[0]?.id === item.id} type="button" aria-label={`Mover ${item.nome ?? "mídia"} para a esquerda`} onClick={() => void reorderMedia(mediaPeers(item), item.id, -1)}>←</button><button disabled={busy || mediaPeers(item).at(-1)?.id === item.id} type="button" aria-label={`Mover ${item.nome ?? "mídia"} para a direita`} onClick={() => void reorderMedia(mediaPeers(item), item.id, 1)}>→</button></div><div className="media-actions">{item.tipo === "foto" && <button disabled={busy || item.is_capa} type="button" onClick={() => void setCover(item.id)}>{item.is_capa ? "✓ Foto de capa" : "Usar como capa"}</button>}</div></div> : null}
    ...
    <section className="capture-panel fv2-unit-media-edit-panel">
      ...
      <div className="capture-body">
        ...
        {unitMediaEditorItems.length ? <div className="detail-gallery unit-media-editor-grid">{unitMediaEditorItems.map((item) => <article key={item.id}>
          ...
          {item.tipo === "foto" && item.url ? <button className="gallery-open watermarked-preview" type="button" onClick={() => setUnitLightbox({ items: unitMediaEditorPhotos.map((photo) => ({ url: photo.url ?? "", label: photo.alt_text || photo.categoria || photo.nome || "Foto da unidade" })), index: Math.max(0, unitMediaEditorPhotos.findIndex((photo) => photo.id === item.id)) })}><img src={item.url} alt={item.alt_text || item.categoria || item.nome || "Foto da unidade"} onError={retryMediaImage} /></button> : item.url ? <div className="watermarked-preview"><video src={item.url} controls preload="metadata" /></div> : <div className="file-tile">Mídia indisponível</div>}
  ...
  function renderFocusedUnitDesign(currentProduct: ProductDetailData, unit: Unit) {
    const unitTitle = unit.titulo_comercial || (focusedUnitStandalone ? currentProduct.nome : `Apartamento ${unit.numero || "s/n"}`);
    const unitDescription = unit.descricao_comercial || currentProduct.descricao;
    ...
    const photoItems = focusedUnitPhotos.map((item) => ({ url: item.url ?? "", label: item.alt_text || item.categoria || item.nome || "Foto do apartamento" }));
    const commonAreaItems = focusedUnitCommonAreaPhotos.map((item) => ({ url: item.url ?? "", label: item.alt_text || `Área comum · ${item.categoria || item.nome || "Foto do condomínio"}` }));
      ...
      <section className="pv3-detail-main">
        ...
        <div className="pv3-detail-scroll">
          ...
          {tab === "resumo" && <div className="pv3-detail-summary">
            ...
            <div className="pv3-detail-quick-actions">{canShowOnSite ? <a href={unitUrl} target="_blank" rel="noreferrer"><IcLink />Ver no site</a> : <button type="button" disabled title="O imóvel ainda não está publicado"><IcLink />Ver no site</button>}<button type="button" onClick={() => void copyListingLink(unit.id)}><IcLink />Copiar link</button><button type="button" onClick={() => void shareListing(unit.id)}><IcSend />Enviar ao cliente</button><button type="button" onClick={() => window.print()}><IcFile />Gerar book</button><a className="pv3-detail-history" href={`/auditoria?produto=${currentProduct.id}`}><IcClock />Histórico</a></div>
            ...
            <section className="pv3-detail-description"><header><h3>Sobre o imóvel</h3>{unit.pode_editar && <button type="button" onClick={() => setUnitEdit({ ...unit })}><IcEdit />Editar texto</button>}</header><p>{unitDescription || "Descrição comercial ainda não cadastrada para esta unidade."}</p></section>
          </div>}

          {tab === "site" && <div className="site-content-review pv3-detail-tab-card"><div className="site-content-head"><div><small>COMO O IMÓVEL SERÁ APRESENTADO</small><h3>{unit.titulo_comercial || currentProduct.titulo || unitTitle}</h3><p>{currentProduct.slogan || "Adicione uma chamada curta para valorizar este imóvel."}</p></div><span className={`quality-badge ${currentProduct.quality.level}`}>Nota {focusedUnitScore}</span></div><section><h4>Descrição</h4><p>{unitDescription || "Nenhuma descrição cadastrada."}</p></section><section><h4>Prévia de busca</h4><strong>{unit.seo_titulo || unit.titulo_comercial || currentProduct.seo_titulo || unitTitle}</strong><p>{unit.seo_descricao || unitDescription || currentProduct.seo_descricao || "Adicione uma descrição para os mecanismos de busca."}</p></section><div className="site-content-actions">{unit.pode_editar && <button className="fv2-btn fv2-btn-outline" type="button" onClick={() => setUnitEdit({ ...unit })}><IcEdit />Editar apartamento</button>}{canShowOnSite && <a className="fv2-btn fv2-btn-ghost" href={unitUrl} target="_blank" rel="noreferrer"><IcLink />Ver este imóvel no site</a>}</div></div>}
      ...
      <section className="pv3-detail-main">
        ...
        <div className="pv3-detail-scroll">
          ...
          {tab === "resumo" && <div className="pv3-detail-summary">
            ...
            <div className="pv3-detail-quick-actions">{published ? <a href={productUrl} target="_blank" rel="noreferrer"><IcLink />Ver no site</a> : <button type="button" disabled><IcLink />Ver no site</button>}<button type="button" onClick={() => void copyListingLink()}><IcLink />Copiar link</button><button type="button" onClick={() => void shareListing()}><IcSend />Enviar ao cliente</button><button type="button" onClick={() => window.print()}><IcFile />Gerar book</button><a className="pv3-detail-history" href={`/auditoria?produto=${currentProduct.id}`}><IcClock />Histórico</a></div>
  ...
  return <div className="modal-layer product-detail-layer">
    ...
    <aside className="product-detail-panel ficha-v2" aria-label="Ficha completa do produto">
      {!product ? <div className="detail-loading">{message || "Carregando dados reais do produto..."}</div> : focusedUnit ? renderFocusedUnitDesign(product, focusedUnit) : editing ? (
        <div className="fv2-edit">
          ...
          {mediaLibrary}<div className="detail-form">
            ...
            <div className="field-grid"><label>Título para busca<input maxLength={70} value={draft.seo_titulo ?? ""} onChange={(event) => setDraft({ ...draft, seo_titulo: event.target.value })} placeholder="Até 70 caracteres" /><small>{String(draft.seo_titulo ?? "").length}/70</small></label><label>Descrição para busca<textarea rows={3} maxLength={180} value={draft.seo_descricao ?? ""} onChange={(event) => setDraft({ ...draft, seo_descricao: event.target.value })} placeholder="Resumo do imóvel para buscadores" /><small>{String(draft.seo_descricao ?? "").length}/180</small></label></div>
    ...
    {unitEdit && <div className="modal-layer fv2-unit-edit-layer" role="dialog" aria-modal="true" aria-label="Editar unidade">
      ...
      <section className="capture-panel fv2-unit-edit-panel">
        ...
        <div className="capture-body unit-editorial-fields"><div className="form-section"><h3>Conteúdo exclusivo deste imóvel</h3><p className="form-guidance">Estes textos substituem o conteúdo geral do condomínio na página desta unidade.</p><label>Título comercial<input maxLength={120} value={unitEdit.titulo_comercial ?? ""} onChange={(event) => setUnitEdit({ ...unitEdit, titulo_comercial: event.target.value })} placeholder="Ex.: Apartamento reformado com vista livre" /><small>{String(unitEdit.titulo_comercial ?? "").length}/120</small></label><label>Descrição comercial<textarea rows={6} minLength={80} maxLength={5000} value={unitEdit.descricao_comercial ?? ""} onChange={(event) => setUnitEdit({ ...unitEdit, descricao_comercial: event.target.value })} placeholder="Descreva os ambientes, diferenciais e perfil ideal do imóvel" /><small>{String(unitEdit.descricao_comercial ?? "").trim().length}/80 caracteres mínimos para publicar</small></label><div className="field-grid"><label>Título para busca<input maxLength={70} value={unitEdit.seo_titulo ?? ""} onChange={(event) => setUnitEdit({ ...unitEdit, seo_titulo: event.target.value })} placeholder="Até 70 caracteres" /><small>{String(unitEdit.seo_titulo ?? "").length}/70</small></label><label>Descrição para busca<textarea rows={3} maxLength={180} value={unitEdit.seo_descricao ?? ""} onChange={(event) => setUnitEdit({ ...unitEdit, seo_descricao: event.target.value })} placeholder="Resumo para buscadores" /><small>{String(unitEdit.seo_descricao ?? "").length}/180</small></label></div></div></div>

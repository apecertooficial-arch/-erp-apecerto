-- No fim de semana a equipe trabalha na rua: presenca fisica e feedback de
-- visita pendente nao bloqueiam a distribuicao. Na segunda-feira ambas as
-- regras voltam automaticamente, sem cron ou alteracao de estado.
...
begin
  if position('feedback_visita_exigido' in v_def)=0
     or position('if v_fim_de_semana then' in v_def)=0
     or position('if coalesce(cfg.exigir_feedback_visita,true) then' in v_def)=0
     or position('if v_fim_de_semana then' in v_def)
        > position('if coalesce(cfg.exigir_feedback_visita,true) then' in v_def) then
    raise exception 'excecao_fim_de_semana_feedback_nao_instalada';
  end if;
end
$check$;

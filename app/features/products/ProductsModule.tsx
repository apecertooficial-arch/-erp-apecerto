...
type CatalogResponse = {
  ...
  catalog: Array<{
    ...
    leads?: number; alreadyRented?: boolean; condominiumFee?: number | null;
  }>;
  ...
  inventoryUnits?: Array<{
    id: string; productId: string; codigo: string | null; numero: string | null; productName: string;
    neighborhood: string; city: string; captador: string | null; mine: boolean; available: boolean;
    approval: string; published: boolean; inCommercialCatalog: boolean; price: number | null; area: number | null;
    parking: number | null; ownMedia: number; referenceMedia: number; coverUrl: string | null;
...
function captureStatusLabel(unit: NonNullable<CatalogResponse["myUnits"]>[number]): string {
  if (unit.photoCount === 0) return "Cadastro incompleto · envie as fotos";
...
type ProductsSection = "unidades" | "empreendimentos" | "condominios" | "estoque" | "qualidade" | "aprovacoes";
...
export function ProductsModule({ accessToken }: { accessToken: string }) {
  ...
  const [inventoryUnits, setInventoryUnits] = useState<NonNullable<CatalogResponse["inventoryUnits"]>>([]);
  ...
  const openMyCapture = useCallback((unit: NonNullable<CatalogResponse["myUnits"]>[number]) => {
    if (unit.photoCount === 0) setInitialUnitAction("media");
    else setInitialUnitAction("view");
    setInitialUnitId(unit.id);
    setSelectedProductId(unit.empreendimentoId);
  }, []);
  ...
  const loadCatalog = useCallback(async function requestCatalog(token: string, allowRefresh = true) {
    ...
    try {
      ...
      setProducts(result.catalog.map((item) => {
        const priceIsValid = item.price !== null && isPlausibleProductPrice(item.price, item.purpose);
        return ({
        ...
        price: item.price === null ? "Preço pendente" : priceIsValid ? currency.format(item.price) : "Preço inválido",
        ...
        priceM2: priceIsValid && item.price && item.area ? `${currency.format(item.price / item.area)}/m²` : "—",
        ...
        origin: item.origin, standalone: item.standalone, numericPrice: priceIsValid ? item.price : null, priceInvalid: item.price !== null && !priceIsValid, favorite: item.favorite,
        ...
        alreadyRented: item.alreadyRented === true, condominiumFee: item.condominiumFee ?? null,
      });
      }));
      ...
      setInventorySummary(result.inventorySummary ?? null);
      setInventoryUnits(result.inventoryUnits ?? []);
  ...
  const inventoryUnitsVisible = useMemo(() => {
    const queryKey = normalizedKey(query);
    return inventoryUnits.filter((unit) => !queryKey || [unit.codigo, unit.numero, unit.productName, unit.neighborhood, unit.city, unit.captador]
      .some((value) => normalizedKey(value).includes(queryKey)));
  ...
  const immediateRiskCount = qualityQueue.filter((item) => item.issues.includes("preco_invalido") || item.issues.includes("sem_foto_propria")).length;
  ...
  function openQualityItem(item: ProductQualityQueueItem, action: QualityRepairAction) {
    setInitialUnitId(item.unitId);
    setInitialUnitAction(action);
    setSelectedProductId(item.productId);
  }

  function openInventoryUnit(unit: NonNullable<CatalogResponse["inventoryUnits"]>[number], action: UnitOpenAction = "view") {
    setInitialUnitId(unit.id);
    setInitialUnitAction(action);
    setSelectedProductId(unit.productId);
  }
  ...
  if (ehCelular) return <section className="ape-produtos" aria-label="Produtos">
    ...
    {section !== "estoque" && section !== "qualidade" && <><nav className="ape-filtros ape-produto-filtros" aria-label="Filtrar produtos">
      {([
    ...
    {dataState === "live" && section !== "qualidade" && section !== "estoque" && produtosVisiveis.length === 0 && <div className="ape-estado"><div className="ape-estado-icone" aria-hidden="true">⌕</div><strong>Nenhum produto encontrado</strong><p>Ajuste a busca ou escolha outro filtro.</p></div>}

    {dataState === "live" && section === "qualidade" && <ProductQualityQueue items={qualityQueue} onOpen={openQualityItem} />}
    {dataState === "live" && section === "estoque" && <section className="ape-mobile-inventory"><header><h2>Estoque completo</h2><p>Inclui imóveis inativos, legados e fora do catálogo comercial.</p></header>{inventoryUnitsVisible.map((unit) => <article key={unit.id}><button type="button" className="ape-mobile-inventory-main" onClick={() => openInventoryUnit(unit)}>{unit.coverUrl ? <img src={unit.coverUrl} alt={`Foto de ${unit.productName}`} onError={retryProductMediaImage} /> : <span aria-hidden="true">▥</span>}<div><small>{unit.codigo || "Sem código"} · {unit.captador || "Sem captador"}</small><strong>{unit.productName} · Un. {unit.numero || "s/n"}</strong><em>{unit.inCommercialCatalog ? "No catálogo" : unit.available ? "Fora do catálogo" : "Inativo"} · {unit.ownMedia} foto(s) próprias</em></div></button><footer>{(canApprove || unit.mine) && <button type="button" onClick={() => openInventoryUnit(unit, "edit")}>Editar dados</button>}{(canApprove || unit.mine) && <button type="button" onClick={() => openInventoryUnit(unit, "media")}>Editar fotos</button>}{(canApprove || unit.mine) && <button type="button" className="danger" onClick={() => openInventoryUnit(unit, "delete")}>Excluir</button>}</footer></article>)}{inventoryUnitsVisible.length === 0 && <div className="ape-estado"><strong>Nenhuma unidade encontrada no estoque</strong><p>Revise o código AP ou o nome informado.</p></div>}</section>}

    {myUnitsOpen && myUnits.length > 0 && <section className="ape-mobile-captacoes"><h2>Minhas captações</h2>{myUnits.map((unit) => <button type="button" key={unit.id} onClick={() => openMyCapture(unit)}><strong>{unit.predio} · Un. {unit.numero ?? "s/n"}{unit.codigo ? ` · ${unit.codigo}` : ""}</strong><span>{captureStatusLabel(unit)}</span>{unit.photoCount === 0 && <em className="pv3-continue-photos">Continuar fotos</em>}</button>)}</section>}
    {canApprove && approvalFilter && pendingUnitsVisiveis.length > 0 && <section className="ape-mobile-captacoes"><h2>Unidades para aprovar</h2>{pendingUnitsVisiveis.map((unit) => <div key={unit.id}><button type="button" onClick={() => { setInitialUnitId(unit.id); setSelectedProductId(unit.empreendimentoId); }}><strong>{unit.predio} · Un. {unit.numero ?? "s/n"}</strong><span>{unit.photoCount === 0 ? "Sem fotos · correção necessária" : unit.indicador ?? "Sem captador"}</span></button><div><button type="button" disabled={unit.photoCount === 0} title={unit.photoCount === 0 ? "A unidade precisa ter fotos antes da aprovação." : undefined} onClick={() => void decideUnitFromList(unit.empreendimentoId, unit.id, true)}>✓ Aprovar</button><button type="button" onClick={() => void decideUnitFromList(unit.empreendimentoId, unit.id, false)}>✕ Reprovar</button></div></div>)}</section>}

    {dataState === "live" && section !== "qualidade" && section !== "estoque" && produtosVisiveis.length > 0 && <section className="ape-produto-lista">
      {produtosVisiveis.map((product) => {
        ...
        return <article className="ape-produto-card" key={product.unitId ?? product.id ?? product.name}>
          <button type="button" className={`ape-produto-foto${product.coverUrl ? " com-foto" : ""}`} onClick={() => { if (product.id) { setInitialUnitId(product.unitId ?? null); setSelectedProductId(product.id); } }} aria-label={`Abrir ${product.name}`}>
            {product.coverUrl ? <img src={product.coverUrl} alt={`Foto de ${product.name}`} onError={retryProductMediaImage} /> : <span aria-hidden="true">▥</span>}
            <em>{product.draft ? "Rascunho" : product.status?.replaceAll("_", " ") || "Produto"}</em>
          </button>
          <div className="ape-produto-info">
            <strong className={`ape-produto-preco${product.priceInvalid ? " invalid" : ""}`}>{product.price}</strong>
  ...
  return (
    <section className="products-v3" aria-label="Produtos">
      ...
      {section !== "aprovacoes" && inventorySummary && <section className="pv3-inventory-truth" aria-label="Conciliação do estoque"><div><strong>Estoque total: {inventorySummary.totalUnits} unidades</strong><span>A contagem completa do banco, sem esconder cadastro legado ou inativo.</span></div><dl><div><dt>Aprovadas e disponíveis</dt><dd>{inventorySummary.approvedAvailable}</dd></div><div><dt>No catálogo comercial</dt><dd>{inventorySummary.catalogUnits}</dd></div><div><dt>Fora do catálogo</dt><dd>{inventorySummary.outsideCommercialCatalog}</dd></div><div><dt>Inativas</dt><dd>{inventorySummary.unavailableUnits}</dd></div></dl><div className="pv3-inventory-actions"><button type="button" onClick={() => chooseSection("estoque")}>Abrir estoque completo →</button><button type="button" onClick={() => chooseSection("qualidade")}>Abrir {inventorySummary.qualityBlocked} correções →</button></div></section>}
      ...
      <nav className="pv3-tabs" aria-label="Áreas de produtos">
        ...
        <button type="button" className={section === "qualidade" ? "active" : ""} onClick={() => chooseSection("qualidade")}>{canApprove ? "Central de decisões" : "Qualidade"} <span>{qualityQueue.length}</span></button>
      ...
      {section !== "aprovacoes" && section !== "qualidade" && <>
        <section className="pv3-toolbar">
          <label className="pv3-search"><Icon name="search" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={section === "empreendimentos" ? "Buscar em empreendimentos..." : section === "condominios" ? "Buscar em condomínios..." : section === "estoque" ? "Buscar código AP, prédio, unidade ou captador..." : "Buscar em produtos..."} /></label>
          {section !== "estoque" && <><button className={moreFiltersOpen ? "pv3-secondary active" : "pv3-secondary"} type="button" onClick={() => setMoreFiltersOpen(!moreFiltersOpen)}><Icon name="filter" /> Filtros</button>
        ...
        {section !== "estoque" && moreFiltersOpen && <section className="pv3-filters">
      ...
      {dataState === "live" && section === "unidades" && <section className="pv3-content">
        ...
        {myUnits.length > 0 && <div className="pv3-my-captures"><button type="button" onClick={() => setMyUnitsOpen(!myUnitsOpen)}><span><Icon name="user" /><b>Minhas captações</b><em>{myUnits.length}</em></span>{myUnitsOpen ? "Recolher" : "Ver todas"}</button>{myUnitsOpen && <div>{myUnits.map((unit) => <button type="button" key={unit.id} onClick={() => openMyCapture(unit)}><strong>{unit.predio} · Unidade {unit.numero || "s/n"}</strong><span>{unit.codigo || "Sem código"} · {captureStatusLabel(unit)}</span>{unit.photoCount === 0 ? <em className="pv3-continue-photos">Continuar fotos</em> : <Icon name="arrow" />}</button>)}</div>}</div>}
      </section>}

      {dataState === "live" && section === "estoque" && <section className="pv3-inventory-list"><header><div><h2>Estoque completo</h2><p>Busca operacional em todas as unidades, inclusive inativas, legadas e fora do catálogo comercial.</p></div><strong>{inventoryUnitsVisible.length} de {inventoryUnits.length} unidades</strong></header><div>{inventoryUnitsVisible.map((unit) => <article key={unit.id}><button type="button" className="pv3-inventory-unit" onClick={() => openInventoryUnit(unit)}>{unit.coverUrl ? <img src={unit.coverUrl} alt={`Foto de ${unit.productName}`} onError={retryProductMediaImage} /> : <span className="pv3-inventory-placeholder"><Icon name="building" /></span>}<div><small>{unit.codigo || "Sem código"} · Captador: {unit.captador || "não identificado"}</small><strong>{unit.productName} · Unidade {unit.numero || "s/n"}</strong><span>{unit.neighborhood} · {unit.city} · {unit.ownMedia} foto(s) próprias</span></div><em className={unit.inCommercialCatalog ? "catalog" : unit.available ? "offline" : "inactive"}>{unit.inCommercialCatalog ? "No catálogo" : unit.available ? "Fora do catálogo" : "Inativo"}</em></button><footer>{(canApprove || unit.mine) && <button type="button" onClick={() => openInventoryUnit(unit, "edit")}>Editar dados</button>}{(canApprove || unit.mine) && <button type="button" onClick={() => openInventoryUnit(unit, "media")}>Editar fotos</button>}{(canApprove || unit.mine) && <button type="button" className="danger" onClick={() => openInventoryUnit(unit, "delete")}>Excluir imóvel</button>}</footer></article>)}</div>{inventoryUnitsVisible.length === 0 && <div className="pv3-empty"><strong>Nenhuma unidade encontrada no estoque</strong><p>Revise o código AP, nome do prédio, unidade ou captador.</p></div>}</section>}
      ...
      {dataState === "live" && section === "qualidade" && <>
        {canApprove && <section className="pv3-decision-metrics" aria-label="Resumo das decisões de Produtos"><article><b>{immediateRiskCount}</b><span>Risco comercial imediato</span></article><article><b>{qualityQueue.length}</b><span>Bloqueios de qualidade</span></article><article><b>—</b><span>Recência ainda não mensurada</span></article><article><b>{approvalTotal}</b><span>Aguardando decisão</span></article></section>}
        <ProductQualityQueue items={qualityQueue} onOpen={openQualityItem} />
      </>}

      {dataState === "live" && section === "aprovacoes" && canApprove && <section className="pv3-approval"><header><h2>Central de aprovação</h2><p>Escolha uma requisição para revisar ou decida diretamente quando a evidência já for suficiente.</p></header><div className="pv3-approval-list"><div className="pv3-approval-list-head"><strong>Requisições pendentes</strong><span>{approvalTotal} na fila · {pendingUnitsVisiveis.filter((u) => u.rejectionReason || u.photoCount === 0).length + produtosVisiveis.filter((p) => !p.quality?.readyForSite).length} com bloqueios</span></div>{pendingUnitsVisiveis.map((unit) => <article className="pv3-approval-decision" key={unit.id}><button type="button" onClick={() => { setInitialUnitId(unit.id); setSelectedProductId(unit.empreendimentoId); }}><div className="pv3-approval-thumb" style={unit.coverUrl ? { backgroundImage: `url(${unit.coverUrl})` } : undefined}>{!unit.coverUrl && <Icon name="home" />}</div><span><strong>{unit.numero ? `Unidade ${unit.numero}` : "Unidade"} · {unit.predio}</strong><small>Unidade individual · {unit.indicador || "Captador não identificado"}</small></span><em className={unit.rejectionReason || unit.photoCount === 0 ? "blocked" : "ready"}>{unit.photoCount === 0 ? "Sem fotos" : unit.rejectionReason ? "1 bloqueio" : "Revisar cadastro"}</em><Icon name="arrow" /></button><footer><button type="button" onClick={() => void decideUnitFromList(unit.empreendimentoId, unit.id, false)}>Devolver com motivo</button><button type="button" className="approve" disabled={unit.photoCount === 0} title={unit.photoCount === 0 ? "A unidade precisa ter fotos antes da aprovação." : undefined} onClick={() => void decideUnitFromList(unit.empreendimentoId, unit.id, true)}>Aprovar</button></footer></article>)}{produtosVisiveis.map((product) => <button type="button" key={product.id} onClick={() => openProduct(product)}><div className="pv3-approval-thumb" style={product.coverUrl ? { backgroundImage: `url(${product.coverUrl})` } : undefined}>{!product.coverUrl && <Icon name="building" />}</div><span><strong>{product.name}</strong><small>{/lan[cç]|obra/i.test(product.status ?? "") ? "Empreendimento" : "Condomínio"} · {product.capturedBy || product.developer || "Equipe ApêCerto"}</small></span><em className={product.quality?.readyForSite ? "ready" : "blocked"}>{product.quality?.readyForSite ? "Pronto para aprovar" : `${product.quality?.blocking.length || 1} bloqueio(s)`}</em><Icon name="arrow" /></button>)}</div>{approvalTotal === 0 && <div className="pv3-empty"><strong>Fila de aprovação vazia</strong><p>Nenhuma solicitação aguarda revisão.</p></div>}</section>}

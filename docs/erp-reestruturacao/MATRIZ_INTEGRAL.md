...
| Pós-visita | feedback e cobrança | CRM/gestão | `/api/agenda`, `f2_registrar_resultado_visita`, `f2_visitas_resultado_pendente` | `f2_visita`, `f2_lead`, eventos | corretor dono grava; gestão somente acompanha na UI; banco ainda permite admin | 22 testes de Agenda; 436/436 no gate frontend; navegador desktop/móvel sanitizado | `TRACE_VISITA_FEEDBACK_COBRANCA.md` | parcial; gravação, erro fail-closed, virada de mês, responsável/idade e separação gerente × corretor cobertos; escalonamento persistido e contrato sem expiração pendentes |

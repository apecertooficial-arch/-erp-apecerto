...
export function StudioModule({ accessToken, initialData, mutationHandler }: {
  ...
  return <div className="studio-shell">
    ...
    {data.externalActionsEnabled !== true && <div className="studio-alert" role="status"><Icon name="warning"/><span>Ações externas estão desativadas neste ambiente. IA paga, renderização, conexão Meta, orçamento e envio para publicação permanecem bloqueados.</span></div>}
...
function Overview({ data, onCreate, onOpen }: { data: StudioData; onCreate: () => void; onOpen: (id: string, pieceId?: string) => void }) {
  ...
  return <section className="studio-content studio-overview" aria-label="Visão geral do Studio">
...
function Builder({ data, campaign, campaignId, piece, version, snapshot, pieces, onCampaign, onPiece, busy, mutate }: {
  ...
  if (!campaignId || !campaign) return <section className="studio-content" aria-label="Construtor do Studio"><EmptyState icon="grid" title="Escolha uma campanha" text="O construtor mostra as peças e versões de uma campanha."/></section>;
  ...
  const externalActionsEnabled = data.externalActionsEnabled === true;
  const realGenerationBlocked = !externalActionsEnabled || data.integrations.find((item) => item.provider === "openai")?.status !== "configurada" || Number(data.budgets.find((item) => item.provider === "openai")?.limite_usd ?? 0) <= 0;
  ...
  return <section className="studio-builder" aria-label="Construtor do Studio">
    ...
    <section className="studio-canvas-area">
      ...
      {piece && version && snapshot && <div className="studio-render-actions">
        <button type="button" className="studio-primary" disabled={busy || renderActive || !snapshot.midias.length || !externalActionsEnabled} title={!externalActionsEnabled ? "A renderização externa ainda não foi ativada neste ambiente." : undefined} onClick={() => void mutate(
...
function Settings({ data, busy, mutate }: { data: StudioData; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult> }) {
  ...
  const externalActionsEnabled = data.externalActionsEnabled === true;
  return <section className="studio-content studio-settings" aria-label="Configurações do Studio"><section className="studio-section"><header><div><span className="studio-eyebrow">Integrações</span><h2>Estado real das conexões</h2><p>Nenhuma integração é mostrada como pronta sem verificação.</p></div></header><div className="studio-integrations">{(["openai", "figma", "renderer", "instagram"] as const).map((provider) => { const entry = data.integrations.find((item) => item.provider === provider); return <article key={provider}><span className={`studio-provider-mark ${provider}`}><Icon name={provider === "openai" ? "sparkles" : provider === "figma" ? "layers" : provider === "instagram" ? "send" : "image"}/></span><div><strong>{provider === "openai" ? "IA governada" : provider === "figma" ? "Figma" : provider === "instagram" ? "Instagram" : "Renderização"}</strong><small>{entry?.status === "configurada" ? "Configurada e verificada" : entry?.status === "degradada" ? "Com falha parcial" : provider === "renderer" ? "Worker externo ainda não ativado" : "Não configurada"}</small></div><span className={`studio-connection ${entry?.status ?? "nao_configurada"}`}>{entry?.status === "configurada" ? "Ativa" : entry?.status === "degradada" ? "Parcial" : "Desativada"}</span></article>; })}</div>
      <div className="studio-review-actions">{instagram?.status === "configurada"
        ? <button type="button" className="studio-secondary" disabled={busy || !externalActionsEnabled} onClick={() => { if (window.confirm("Desconectar o Instagram e revogar o token guardado no Vault?")) void mutate({ action: "metaOAuthDisconnect" }, "Instagram desconectado com segurança."); }}><Icon name="close"/> Desconectar Instagram</button>
        : <button type="button" className="studio-primary" disabled={busy || !externalActionsEnabled} onClick={() => void mutate({ action: "metaOAuthStart" }).then((result) => { if (!result.authorization_url) throw new Error("A Meta não devolveu a URL de autorização."); window.location.assign(result.authorization_url); })}><Icon name="send"/> Conectar Instagram profissional</button>}
      </div>
      {!externalActionsEnabled && <p className="studio-safe-note">Ativação externa bloqueada pelo servidor até a homologação de credenciais, custos e publicação.</p>}
    </section>
    ...
    <section className="studio-section studio-budget"><header><div><span className="studio-eyebrow">Custos</span><h2>Limites do mês</h2><p>O padrão seguro é zero. Apenas um administrador pode autorizar um novo limite.</p></div></header><div>{data.budgets.map((budget) => <BudgetEditor key={`${budget.provider}-${budget.limite_usd}`} budget={budget} busy={busy} mutate={mutate} externalActionsEnabled={externalActionsEnabled}/>)}</div></section>
  </section>;
}
...
function BudgetEditor({ budget, busy, mutate, externalActionsEnabled }: { budget: StudioData["budgets"][number]; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult>; externalActionsEnabled: boolean }) {
  ...
  return <article><span>{budget.provider}</span><strong>US$ {Number(budget.consumido_usd).toFixed(2)} consumidos</strong><div><i style={{ width: `${budget.limite_usd ? Math.min(100, budget.consumido_usd / budget.limite_usd * 100) : 0}%` }}/></div><label><span>Limite</span><input type="number" min="0" max="1000" step="0.01" value={limit} onChange={(event) => setLimit(event.target.value)} disabled={!externalActionsEnabled}/><button type="button" disabled={busy || !externalActionsEnabled || next === Number(budget.limite_usd)} onClick={save}>Salvar</button></label></article>;
}

...
function comparison(current: unknown, previous: unknown) {
  const a = num(current); const b = num(previous);
  if (!b) return a ? "Sem base no período anterior" : "Sem variação no período";
...
export function CentralComandoWorkspace({ accessToken }: { accessToken: string }) {
  ...
  const adsAvailable = adsMeta.status === "conectado" || adsGoogle.status === "conectado";
  const adsPartial = adsAvailable && !(adsMeta.status === "conectado" && adsGoogle.status === "conectado");
  ...
  const cohortVisits = flow.find((item) => item.key === "visit_done")?.value ?? 0;
  const cohortSales = flow.find((item) => item.key === "sales")?.value ?? 0;
      ...
      <div className="cc-grid partner-grid">
        <Panel eyebrow="CAMINHO COMERCIAL" title="Pessoas interessadas → visitas → vendas">
          <div className="cc-partner-funnel">
            ...
            <div><span>Com visita realizada</span><strong>{fmt(cohortVisits)}</strong><small>{summary.leads_validos ? `${fmt(num(cohortVisits) / num(summary.leads_validos) * 100, 1)} de cada 100` : "sem base"}</small></div><b>→</b>

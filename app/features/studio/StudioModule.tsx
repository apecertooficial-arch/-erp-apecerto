...
function Settings({ data, busy, mutate }: { data: StudioData; busy: boolean; mutate: (body: Record<string, unknown>, success?: string) => Promise<ApiResult> }) {
  ...
  return <main className="studio-content studio-settings"><section className="studio-section"><header><div><span className="studio-eyebrow">Integrações</span><h2>Estado real das conexões</h2><p>Nenhuma integração é mostrada como pronta sem verificação.</p></div></header><div className="studio-integrations">{(["openai", "figma", "renderer", "instagram"] as const).map((provider) => { const entry = data.integrations.find((item) => item.provider === provider); return <article key={provider}><span className={`studio-provider-mark ${provider}`}><Icon name={provider === "openai" ? "sparkles" : provider === "figma" ? "layers" : provider === "instagram" ? "send" : "image"}/></span><div><strong>{provider === "openai" ? "IA governada" : provider === "figma" ? "Figma" : provider === "instagram" ? "Instagram" : "Renderização"}</strong><small>{entry?.status === "configurada" ? "Configurada e verificada" : entry?.status === "degradada" ? "Com falha parcial" : provider === "renderer" ? "Worker externo ainda não ativado" : "Não configurada"}</small></div><span className={`studio-connection ${entry?.status ?? "nao_configurada"}`}>{entry?.status === "configurada" ? "Ativa" : entry?.status === "degradada" ? "Parcial" : "Desativada"}</span></article>; })}</div>
    ...
    <section className="studio-section studio-canva"><header><div><span className="studio-eyebrow">Canva</span><h2>Exportação e retorno versionado</h2><p>Não há sincronização conectada neste ambiente. Exporte um pacote com peças, versões, assets autorizados, dimensões e manifest; ao retornar, importe como nova versão validada.</p></div></header><div className="studio-review-actions"><button type="button" className="studio-primary" onClick={() => { const payload = { schema_version: 1, exported_at: new Date().toISOString(), templates: data.templates, pieces: data.pieces, versions: data.versions, assets: data.snapshots.flatMap((item) => item.midias.map((media, index) => ({ storage_path: media.storage_path, media_index: index, authorized: true, width: 1080, height: 1350 }))), note: "Pacote de referência do ApêCerto Studio; importe e ajuste no Canva." }; const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "apecerto-canva-package.json"; anchor.click(); URL.revokeObjectURL(url); }}>Exportar pacote para Canva</button><select value={canvaPiece} onChange={(event) => setCanvaPiece(event.target.value)} aria-label="Peça para retorno Canva"><option value="">Escolha a peça para importar</option>{data.pieces.map((item) => <option value={item.id} key={item.id}>{item.titulo}</option>)}</select><label className="studio-secondary">Importar retorno JSON<input type="file" accept="application/json" hidden onChange={(event) => { const file = event.target.files?.[0]; if (!file || !canvaPiece) return; void file.text().then((raw) => { const parsed = JSON.parse(raw) as Record<string, unknown>; return mutate({ action: "importCanvaPackage", pieceId: canvaPiece, package: parsed }, "Retorno do Canva validado e salvo como nova versão."); }).catch(() => window.alert("Pacote Canva inválido.")); }}/></label><button type="button" className="studio-secondary" disabled title="A abertura automática exige conexão Canva aprovada">Abrir no Canva (conexão não configurada)</button></div></section>

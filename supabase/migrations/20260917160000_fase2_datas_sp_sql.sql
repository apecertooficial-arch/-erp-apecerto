-- Fase 2 — datas de negócio no fuso de São Paulo (lado SQL). Continuação do PR #243.
--
-- Problema: o Postgres do Supabase roda em TimeZone=UTC. current_date, now()::date,
-- ts::date e date_trunc(..., timestamptz) usam o fuso da SESSÃO; depois das 21h
-- (São Paulo) devolvem o dia seguinte. Ex.: venda concluída às 22h de 30/09 era
-- carimbada 01/10 e caía no VGV de outubro.
--
-- Correção: public.hoje_operacao() = (now() at time zone 'America/Sao_Paulo')::date,
-- independente do TimeZone da sessão, e troca mínima nos pontos de data de negócio:
--   current_date / now()::date          -> public.hoje_operacao()
--   timestamptz::date (bucket por dia)   -> (ts at time zone 'America/Sao_Paulo')::date
--   ts >= current_date - N               -> ts >= (hoje_operacao() - N)::timestamp at time zone 'America/Sao_Paulo'
--   date_trunc('month', timestamptz)     -> date_trunc('month', ts at time zone 'America/Sao_Paulo')
-- Assinaturas, SECURITY DEFINER, search_path e grants preservados (CREATE OR REPLACE
-- mantém owner/ACL; nenhuma função tinha COMMENT). Nada mais foi alterado no corpo.
--
-- Rollback: supabase/rollbacks/20260917160000_fase2_datas_sp_sql.down.sql
...
comment on function public.hoje_operacao() is
  'Data de negócio (dia civil em America/Sao_Paulo), independente do TimeZone da sessão.';

grant execute on function public.hoje_operacao() to authenticated, service_role;
...
AS $function$
  select jsonb_build_object(
    'month_vgv',      (select coalesce(sum(vgv),0) from vendas where data_conclusao is not null and date_trunc('month',data_conclusao)=date_trunc('month',public.hoje_operacao())),
    'month_count',    (select count(*) from vendas where data_conclusao is not null and date_trunc('month',data_conclusao)=date_trunc('month',public.hoje_operacao())),
    'month_comissao', (select coalesce(sum(vgv*percentual_comissao),0) from vendas where data_conclusao is not null and date_trunc('month',data_conclusao)=date_trunc('month',public.hoje_operacao())),
    ...
    'meta_mes', (select coalesce(max(meta_vgv),0) from metas
                 where corretor_id is null and periodo_tipo='mensal'
                   and ano=extract(year from public.hoje_operacao())::int and periodo=extract(month from public.hoje_operacao())::int),
    'by_month', (
      select coalesce(jsonb_agg(jsonb_build_object(
        'm', to_char(g.m,'YYYY-MM'),
        'vgv', coalesce(v.vgv,0), 'count', coalesce(v.c,0), 'comissao', coalesce(v.com,0)) order by g.m), '[]'::jsonb)
      from generate_series(date_trunc('month',public.hoje_operacao())-interval '5 months', date_trunc('month',public.hoje_operacao()), interval '1 month') g(m)
      left join (
        select date_trunc('month',data_conclusao) mm, sum(vgv) vgv, count(*) c, sum(vgv*percentual_comissao) com
        from vendas
        where data_conclusao is not null
          and data_conclusao >= (date_trunc('month',public.hoje_operacao())-interval '5 months')::date
        group by 1
      ) v on v.mm = g.m
    ),
    'ranking', (
      ...
      from (
        ...
        where v.data_conclusao is not null
          and v.data_conclusao >= (date_trunc('month',public.hoje_operacao())-interval '5 months')::date
...
AS $function$
  ...
  select jsonb_build_object(
    ...
    'conv_por_mes', (
      ...
      from generate_series(date_trunc('month',public.hoje_operacao())-interval '5 months', date_trunc('month',public.hoje_operacao()), interval '1 month') g(m)
      left join (select date_trunc('month',criado_em at time zone 'America/Sao_Paulo') mm, count(*) n from leads where criado_em >= (date_trunc('month',public.hoje_operacao())-interval '5 months') group by 1) l on l.mm = g.m
      left join (select date_trunc('month',data_venda) mm, count(*) n from vendas where data_venda >= (date_trunc('month',public.hoje_operacao())-interval '5 months')::date group by 1) v on v.mm = g.m
    )
  );
$function$
;
...
AS $function$
  select jsonb_build_object(
    'leads_today', (select count(*) from leads where (criado_em at time zone 'America/Sao_Paulo')::date = public.hoje_operacao()),
    'leads_week',  (select count(*) from leads where criado_em >= (public.hoje_operacao() - 6)::timestamp at time zone 'America/Sao_Paulo'),
    ...
    'leads_per_day', (
      ...
      from generate_series(public.hoje_operacao() - 13, public.hoje_operacao(), interval '1 day') g(d)
      left join (select (criado_em at time zone 'America/Sao_Paulo')::date dd, count(*) n from leads where criado_em >= (public.hoje_operacao() - 13)::timestamp at time zone 'America/Sao_Paulo' group by 1) c on c.dd = g.d::date
    ),
    'leads_by_origem', (
      select coalesce(jsonb_agg(jsonb_build_object('k', k, 'n', n) order by n desc), '[]'::jsonb)
      from (select coalesce(nullif(origem,''),'Sem origem') k, count(*) n from leads where criado_em >= (public.hoje_operacao() - 6)::timestamp at time zone 'America/Sao_Paulo' group by 1) t
    ),
    ...
    'atend_hoje', (
      ...
      ) from leads where (criado_em at time zone 'America/Sao_Paulo')::date = public.hoje_operacao()
    ),
    'atend_por_dia', (
      select coalesce(jsonb_agg(jsonb_build_object('d', to_char(g.d::date,'YYYY-MM-DD'), 'pct', pct) order by g.d), '[]'::jsonb)
      from generate_series(public.hoje_operacao() - 6, public.hoje_operacao(), interval '1 day') g(d)
      left join lateral (
        select case when count(*)>0 then round(100.0*count(*) filter (where atendido_em is not null)/count(*)) else 0 end pct
        from leads where (criado_em at time zone 'America/Sao_Paulo')::date = g.d::date
      ) c on true
    ),
    'tempo_resp_min', (
      select coalesce(round(avg(extract(epoch from (atendido_em - criado_em))/60.0)),0)
      from leads where atendido_em is not null and criado_em >= (public.hoje_operacao() - 6)::timestamp at time zone 'America/Sao_Paulo' and atendido_em >= criado_em
    ),
    'aguardando_total', (select count(*) from leads where atendido_em is null and criado_em >= (public.hoje_operacao() - 30)::timestamp at time zone 'America/Sao_Paulo'),
    'aguardando_por_corretor', (
      select coalesce(jsonb_agg(jsonb_build_object('k', k, 'n', n) order by n desc), '[]'::jsonb)
      from (
        select coalesce(c.nome,'Sem corretor') k, count(*) n
        from leads l left join corretores c on c.id = l.corretor_id
        where l.atendido_em is null and l.criado_em >= (public.hoje_operacao() - 30)::timestamp at time zone 'America/Sao_Paulo'
        group by 1 order by count(*) desc limit 8
      ) t
    )
  );
...
begin
  ...
  values (public.hoje_operacao(), v_prod.id, v_prod.nome, s.vgv, s.forma_pgto, 'pendente', s.obs, s.corretor_id)
  ...
  with scope as (
    ...
    select
      ...
      (select count(*) from public.negocios n, scope s where n.status='ganho' and (s.all_access or n.corretor_id=s.broker_id)
         and date_trunc('month',coalesce(n.ultima_movimentacao,n.criado_em) at time zone 'America/Sao_Paulo')=date_trunc('month',public.hoje_operacao()::timestamp)) as ganhos_mes,
...
AS $function$
  ...
  left join lateral (
    ...
    where na.usuario_id = c.usuario_id
      and na.dia >= public.hoje_operacao() - 1
  ) a on true
  where c.ativo = true
  order by c.nome;
$function$
...
begin
  for r in
    ...
    where t.prazo < public.hoje_operacao() and not t.concluida and not t.arquivada
      and not exists (
        select 1 from projeto_atividades a
        where a.tarefa_id = t.id and a.acao = 'tarefa_atrasada' and (a.criado_em at time zone 'America/Sao_Paulo')::date = public.hoje_operacao()
      )
  loop
  ...
  if v_conclui_novo and not v_conclui_velho then
    ...
    update vendas
       set status = case when v_status in ('pago','distrato') then status else 'concluido'::status_venda end,
           data_conclusao = coalesce(data_conclusao, public.hoje_operacao()),
...
-- Default de coluna: dia de negócio da nota de atendimento da IA.
alter table public.ia_notas_atendimento alter column dia set default public.hoje_operacao();

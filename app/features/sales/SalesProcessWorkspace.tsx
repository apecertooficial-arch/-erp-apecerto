...
export function SalesProcessView({ accessToken, initialCreate = false, sessionRole = "corretor" }: { accessToken: string; initialCreate?: boolean; sessionRole?: string }) {
  ...
  const decideSolic = async (id: string, aprovar: boolean, motivo?: string) => { const key = JSON.stringify([id, aprovar, motivo || ""]); const requestId = decisionRequests.current.get(key) ?? crypto.randomUUID(); decisionRequests.current.set(key, requestId); setBusy(true); setError(null); try { const response = await authedFetch("/api/crm/sales", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: aprovar ? "aprovarSolicitacao" : "recusarSolicitacao", id, motivo: motivo || "", requestId }) }); const result = await response.json().catch(() => ({})) as { success?: boolean; error?: string }; if (!response.ok || result.success !== true) throw new Error(result.error || "Não foi possível decidir a solicitação."); decisionRequests.current.delete(key); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Erro ao decidir a solicitação."); } finally { setBusy(false); } };
  ...
  const move = async (processId: string, stage: string) => { setBusy(true); setError(null); try { const response = await authedFetch("/api/crm/sales", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "move", processId, stage }) }); const result = await response.json().catch(() => ({})) as { success?: boolean; error?: string }; if (!response.ok || result.success !== true) throw new Error(result.error || "A Esteira não confirmou a movimentação da venda."); await load(); return true; } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível mover a venda."); return false; } finally { setBusy(false); } };
  const mutateStages = async (payload: Record<string, unknown>, success?: string) => { setBusy(true); setError(null); try { const response = await authedFetch("/api/crm/sales", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json() as { success?: boolean; error?: string }; if (!response.ok || result.success !== true) throw new Error(result.error || "Não foi possível salvar a etapa."); await load(); if (success) setError(null); return true; } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível salvar a etapa."); return false; } finally { setBusy(false); } };
  const stageCreateRequests = useRef(new Map<string, string>());
  const createStage = async () => { const nome = newStageName.trim(); if (!nome) return; const key = nome.toLocaleLowerCase("pt-BR"); const requestId = stageCreateRequests.current.get(key) ?? crypto.randomUUID(); stageCreateRequests.current.set(key, requestId); if (await mutateStages({ action: "createStage", nome, requestId })) { stageCreateRequests.current.delete(key); setAddingStage(false); setNewStageName(""); } };
  const stageOrderRequests = useRef(new Map<string, string>());
  const reorderStage = (index: number, direction: number) => { const ordered = stageList.filter((s) => s.dbId); const target = index + direction; if (target < 0 || target >= ordered.length) return; const ids = ordered.map((s) => s.dbId as string); [ids[index], ids[target]] = [ids[target], ids[index]]; const key = JSON.stringify(ids); const requestId = stageOrderRequests.current.get(key) ?? crypto.randomUUID(); stageOrderRequests.current.set(key, requestId); void mutateStages({ action: "reorderStages", ids, requestId }).then((ok) => { if (ok) stageOrderRequests.current.delete(key); }); };
  const stageDeleteRequests = useRef(new Map<string, string>());
  const deleteStage = async (stageId: string) => { const requestId = stageDeleteRequests.current.get(stageId) ?? crypto.randomUUID(); stageDeleteRequests.current.set(stageId, requestId); if (await mutateStages({ action: "deleteStage", stageId, requestId })) { stageDeleteRequests.current.delete(stageId); setMenuStage(null); } };
  const stageUpdateRequests = useRef(new Map<string, string>());
  const updateStage = (stageId: string, patch: Record<string, unknown>) => { const key = JSON.stringify([stageId, patch]); const requestId = stageUpdateRequests.current.get(key) ?? crypto.randomUUID(); stageUpdateRequests.current.set(key, requestId); void mutateStages({ action: "updateStage", stageId, requestId, ...patch }).then((ok) => { if (ok) stageUpdateRequests.current.delete(key); }); };
    ...
    <div className="sales-kanban">{stageList.map((stage) => {
      ...
      return <article className="sales-stage" style={{ "--sale-stage": stage.color } as CSSProperties} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const id = event.dataTransfer.getData("text/process-id"); const item = visible.find((row) => row.id === id); if (!item || busy) return; if (!opcoesDeMovimento(stageList, item).some((alvo) => alvo.id === stage.id)) { setError("Avance uma etapa por vez; retornos para etapas anteriores continuam disponíveis."); return; } void move(id, stage.id); }} key={stage.id}>
        ...
        {configurable && menuStage === stage.id && <div className="crm-stage-menu">
          <div className="crm-stage-menu-row"><label className="crm-stage-color">Cor da etapa<input type="color" value={stage.color} onChange={(event) => { if (stage.dbId) updateStage(stage.dbId, { cor: event.target.value }); }} /></label></div>
          ...
          <div className="crm-stage-menu-row crm-stage-reorder"><button type="button" disabled={busy} onClick={() => { const nome = window.prompt("Novo nome da etapa:", stage.name); if (stage.dbId && nome && nome.trim() && nome.trim() !== stage.name) updateStage(stage.dbId, { nome: nome.trim() }); }}>✎ Renomear</button><button type="button" className="crm-stage-danger" disabled={busy || items.length > 0} title={items.length > 0 ? "Mova as vendas antes de excluir" : "Excluir etapa"} onClick={() => { if (stage.dbId && window.confirm(`Excluir a etapa "${stage.name}"?`)) void deleteStage(stage.dbId); }}>🗑 Excluir</button></div>
        </div>}

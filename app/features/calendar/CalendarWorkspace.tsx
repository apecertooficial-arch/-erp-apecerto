"use client";
/* Calendário: Dia/Semana/Mês/Lista e cores por tipo. Novas visitas nascem na
   ficha do lead; uma visita existente pode ser reagendada por aqui. */
...
type CrmData = { brokers: Broker[]; leads: Lead[]; deals: Deal[]; products: Product[]; visits: Visit[]; tasks: Task[]; role?: string; scope?: "equipe" | "propria"; gerentes?: Gerente[] };
...
export function CalendarWorkspace({ accessToken }: { accessToken: string }) {
  ...
  const [selected, setSelected] = useState<CalendarItem | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [notice, setNotice] = useState("");
  const [editing, setEditing] = useState<Visit | null>(null); const [novoHorario, setNovoHorario] = useState(""); const [savingEdit, setSavingEdit] = useState(false); const [editError, setEditError] = useState("");
  const isGestao = data.scope === "equipe";
  ...
  async function updateVisitStatus(visitId: string, nextStatus: string) { setNotice(""); const response = await fetch("/api/agenda", { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "updateVisitStatus", visitId, status: nextStatus }) }); const body = await response.json() as { error?: string }; if (!response.ok) setNotice(body.error ?? "Não foi possível atualizar."); else { setNotice("Visita atualizada."); setSelected(null); await load(); } }
  function openEdit(visit: Visit) { setSelected(null); setEditing(visit); setNovoHorario(""); setEditError(""); }
  async function saveEdit() {
    if (!editing || !novoHorario) return;
    setSavingEdit(true); setEditError("");
    const response = await fetch("/api/agenda", {
      method: "PATCH",
      headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
      body: JSON.stringify({ action: "updateVisit", visitId: editing.id, date: novoHorario.slice(0, 10), startTime: novoHorario.slice(11, 16) }),
    });
    const body = await response.json().catch(() => null) as { error?: string } | null;
    if (!response.ok) setEditError(body?.error ?? "Não foi possível reagendar a visita.");
    else { setEditing(null); setNotice("Visita remarcada."); await load(); }
    setSavingEdit(false);
  }
  ...
  const eventChip = (item: CalendarItem, compact = false) => <button className={`cal-event type-${item.type} ${item.manager ? "with-manager" : ""} ${/eliz/i.test(gerenteNomeDe(item)) ? "eliz" : ""} ${item.status}`} type="button" onClick={() => setSelected(item)} key={item.id}>
    ...
    {!compact && isGestao && <small>{item.brokerId ? brokerById.get(item.brokerId) ?? "Sem corretor" : item.type === "task" ? "Tarefa" : "Sem corretor"}</small>}
  </button>;
  ...
  return <div className="calendar-workspace">
    <header className="workspace-top"><div><h1>Calendário</h1><p>{isGestao ? "Visitas, tarefas e compromissos da equipe" : "Suas visitas, tarefas e compromissos"}</p></div></header>
    ...
    {selected && <div className="calendar-modal-layer" onClick={() => setSelected(null)}><aside className="calendar-detail" onClick={(event) => event.stopPropagation()}><header><div><small>{selected.type === "visit" ? "VISITA" : "TAREFA"}</small><h2>{selected.label}</h2><p>{new Date(`${selected.date}T12:00:00`).toLocaleDateString("pt-BR")} · {selected.time}{selected.endTime ? ` – ${selected.endTime}` : ""}</p></div><button type="button" onClick={() => setSelected(null)}>×</button></header><dl><div><dt>Corretor</dt><dd>{selected.brokerId ? brokerById.get(selected.brokerId) ?? "—" : "—"}</dd></div><div><dt>Produto</dt><dd>{selected.product || "—"}</dd></div>{selected.type === "visit" && <div><dt>Local</dt><dd>{(selected.raw as Visit).local || "—"}</dd></div>}<div><dt>Status</dt><dd>{selected.status}</dd></div><div><dt>Acompanhamento</dt><dd>{selected.manager ? `Com gerente${(() => { const gid = (selected.raw as Visit).gerente_id; const gn = (data.gerentes ?? []).find((g) => g.id === gid)?.nome ?? gerenteNomeGeral; return gn ? ` · ${gn}` : ""; })()}` : "Sem gerente"}</dd></div></dl>{selected.type === "visit" && (() => { const lid = (selected.raw as Visit).lead_id; const lead = lid ? leadFullById.get(lid) : null; if (!lead) return null; const tel = (lead.telefone || "").replace(/\D/g, ""); const wa = tel ? (tel.length > 11 ? tel : `55${tel}`) : ""; return <div className="cal-lead-card"><span className="cal-lead-label">Lead da visita</span><div className="cal-lead-head"><span className="cal-lead-av">{(lead.nome || "?").trim().slice(0, 1).toUpperCase()}</span><div><strong>{lead.nome || `Lead #${lead.id}`}</strong><small>{[lead.status, lead.origem].filter(Boolean).join(" · ") || "Lead"}</small></div></div><div className="cal-lead-acts">{lead.telefone && <a href={`tel:${lead.telefone}`}>☎ {lead.telefone}</a>}{wa && <a href={`https://wa.me/${wa}`} target="_blank" rel="noreferrer">WhatsApp →</a>}{lead.email && <a href={`mailto:${lead.email}`}>✉ {lead.email}</a>}{!lead.telefone && !lead.email && <span className="cal-lead-empty">Sem contato cadastrado</span>}</div></div>; })()}{selected.type === "visit" && <footer><button type="button" onClick={() => openEdit(selected.raw as Visit)}>✎ Editar horário</button><button type="button" onClick={() => void updateVisitStatus(String((selected.raw as Visit).id), "cancelada")}>Cancelar visita</button><button className="confirm" type="button" onClick={() => void updateVisitStatus(String((selected.raw as Visit).id), "realizada")}>✓ Marcar realizada</button></footer>}</aside></div>}
    {editing && <div className="calendar-modal-layer" onClick={() => setEditing(null)}><form className="calendar-create" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); void saveEdit(); }}><header><div><h2>Editar visita</h2><p>{editing.cliente_nome || "Cliente"} · escolha um novo horário disponível</p></div><button type="button" onClick={() => setEditing(null)}>×</button></header><div><div className="wide">{editError && <p className="f2-horarios-erro" role="alert">{editError}</p>}<HorariosVisita accessToken={accessToken} visitId={editing.id} comGerente={editing.com_gerente} gerenteId={editing.gerente_id} value={novoHorario} initialDate={editing.data} onChange={setNovoHorario} disabled={savingEdit} /></div></div><footer><button type="button" onClick={() => setEditing(null)}>Cancelar</button><button className="confirm" type="submit" disabled={savingEdit || !novoHorario}>{savingEdit ? "Salvando…" : "Remarcar visita"}</button></footer></form></div>}
  </div>;
}

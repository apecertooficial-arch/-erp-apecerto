...
function computeIssues(){
 ...
 list.forEach(n=>{
  ...
  if(n.type==='field-operation'){const fs=n.opts.fieldOperations||[];if(!fs.length)add('erro',n.id,'Operações de campos sem nenhum mapeamento.');fs.forEach(f=>{const o=f.options||{};if(!['set-field-operation','parse-phone-field-operation'].includes(f.name))add('erro',n.id,'Operação de campo não implementada: '+(f.name||'vazia')+'.');if(f.name==='set-field-operation'&&!o.parameter)add('erro',n.id,'Mapeamento de campo sem destino.');});}

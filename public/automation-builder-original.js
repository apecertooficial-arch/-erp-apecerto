...
const ACTIONS=[['create-lead-action','Criar lead','lead'],['create-business-action','Criar negócio','business'],['f2-add-action','Adicionar ao Funil 2.0','business'],['move-business-action','Mover negócio','business'],['add-attendant-on-business-action','Atribuir corretor','business'],['clean-attendant-on-business-action','Remover corretor','business'],['add-tag-action','Adicionar tag','lead'],['set-lead-momento-action','Definir momento do lead','lead'],['start-another-automation-action','Iniciar outra automação','system']];
...
/* Etapas do Funil 2.0 (f2_etapa_config). Lista fixa: sao 4 e mudam por migration,
   nao por tela — nao vale um round-trip a mais no carregamento do construtor. */
const F2_ETAPAS=[['novo','Novo'],['tentando_contato','Tentando contato'],['em_atendimento','Em atendimento'],['pos_visita','Pós-visita']];
function f2EtapaOpts(v){const cur=v||'novo';return F2_ETAPAS.map(([k,l])=>`<option value="${k}" ${k===cur?'selected':''}>${esc(l)}</option>`).join('');}
...
function actRow(a,i){const m=acaoMeta(a.name),cfg=m[3],o=a.options||{};let f='';
 ...
 else if(cfg==='f2_etapa')f=`<div class="ne-lb" style="margin-top:0">Etapa no Funil 2.0</div><select class="ne-sel" data-af="f2etapa">${f2EtapaOpts(o.f2etapa)}</select>`+
  `<div class="ne-lb">Onde abrir o negócio, se o lead ainda não tiver um</div><select class="ne-sel" data-af="pipeline">${pipeOpts(o.pipeline_id)}</select><select class="ne-sel" data-af="etapa">${stageOpts(o.etapa_id,o.pipeline_id)}</select>`+
  `<div style="font-size:10.5px;color:var(--ink-faint);margin-top:4px;line-height:1.45">Não precisa de um <b>Criar negócio</b> antes: se o lead já tiver negócio aberto, este passo reaproveita; se não tiver, cria aqui. O card do Funil 2.0 se apoia no negócio para abrir a conversa e o WhatsApp.</div>`;

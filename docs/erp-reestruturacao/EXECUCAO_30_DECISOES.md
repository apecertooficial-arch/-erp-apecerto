...
| 27 | Gestor aprova a captação antes da publicação no site. | Reprovação não publica; aprovação publica uma vez e registra decisão. | em prova | A API usa uma única RPC transacional para aprovar ou reprovar, exige motivo na reprovação, bloqueia decisões diretas e torna a repetição idempotente. Ensaio produtivo com `ROLLBACK` comprovou publicação única, reprovação fora do site e uma auditoria canônica por decisão; contadores e dados permaneceram intactos. |
...
Publicar e aceitar em produção a decisão 27, aprovação gerencial única e auditável
antes da publicação, sem preencher automaticamente os 12 vínculos históricos
incompletos. A decisão 20 permanece

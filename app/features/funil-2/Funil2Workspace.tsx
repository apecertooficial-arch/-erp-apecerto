...
import { dataCurta, ETAPAS_FUNIL2, situacaoPrazo, type EventoFunil2, type LeadFunil2, type MomentoFunil2 } from "./modelo";
  ...
  return (
    <div className="f2-root">
            ...
            return <div key={etapa.codigo} className={`f2-coluna etapa-${etapa.codigo}`}>
              ...
              <div className="f2-lista">
                {daEtapa.map((item) => {
                  ...
                  const prazo = situacaoPrazo(item.proxima_acao_em);
                  return <button key={item.id} type="button" className={`f2-card ${selecionado === item.id ? "selecionado" : ""}`} onClick={() => setSelecionado(item.id)}>
                    ...
                    <div className="f2-card-acao"><span>FAÇA AGORA</span><b>{item.acao_rotulo}</b></div>
      ...
      {!carregando && aba === "dia" && <main className="f2-dia">
        <div className="f2-dia-cab"><div><span className="f2-eyebrow">PLANO DE TRABALHO</span><h2>Hoje está claro o que fazer e até quando.</h2><p>Ao confirmar a ação, o laboratório registra a evidência e recalcula o próximo prazo.</p></div><b>{leads.length} ações</b></div>
        <div className="f2-dia-lista">
          {[...leads].sort((a, b) => +new Date(a.proxima_acao_em) - +new Date(b.proxima_acao_em)).map((item, index) => {
            const momento = momentos.find((m) => m.codigo === item.momento_codigo);
            const prazo = situacaoPrazo(item.proxima_acao_em);
            return <button key={item.id} type="button" className="f2-dia-item" onClick={() => setSelecionado(item.id)}>
              <span className="f2-dia-ordem">{index + 1}</span><div><strong>{item.nome}</strong><small>{momento?.rotulo}</small></div><div><span>FAÇA AGORA</span><b>{item.acao_rotulo}</b></div><em className={prazo.classe}>{prazo.rotulo}</em><i>Abrir</i>
            </button>;
          })}
        </div>
      </main>}
      ...
      {lead && momentoAtual && <Detalhe key={`${lead.id}:${lead.versao}`}
        lead={lead} momento={momentoAtual} momentos={momentos} eventos={eventosLead} busy={busy}
...
function Detalhe({ lead, momento, momentos, eventos, busy, onFechar, onMomento, onConfirmar }: {
  ...
  return <div className="f2-overlay" onClick={onFechar}>
    <aside className="f2-detalhe" aria-label={`Detalhe de ${lead.nome}`} onClick={(e) => e.stopPropagation()}>
      ...
      <section className="f2-agora">
        <div className="f2-agora-cab"><span>ORDEM ATUAL</span><em className={situacao.classe}>{situacao.rotulo}</em></div>
        ...
        <span>FAÇA AGORA</span><h3>{lead.acao_rotulo}</h3><p>{momento.descricao}</p>
        ...
        <button type="button" className="f2-principal" disabled={busy} onClick={() => onConfirmar(momento.exige_dapi ? "dapi" : "registro_operacional", obs)}>
          {busy ? "Atualizando…" : momento.exige_dapi ? "Simular confirmação do D-API" : "Registrar conclusão operacional"}
        </button>
        <small className="f2-nota">Neste laboratório o botão simula a evidência. Na migração real, mensagens só serão concluídas pelo webhook do D-API.</small>
      </section>

      <section className="f2-atualizar">
        <span className="f2-eyebrow">ATUALIZAR A CLASSIFICAÇÃO</span>
        ...
        <button type="button" className="f2-secundario" disabled={busy || codigo === lead.momento_codigo} onClick={() => onMomento(codigo, prazo, obs)}>Aplicar momento e recalcular prazo</button>
      </section>

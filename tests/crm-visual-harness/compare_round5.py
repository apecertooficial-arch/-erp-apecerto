"""Gera comparações da quinta rodada contra a quarta e o Claude Design congelado."""
...
BEFORE = ROOT / "deliverables/crm-correcao-6-2-para-10/visual-round-4-after"
AFTER = ROOT / "deliverables/crm-correcao-6-2-para-10/visual-round-5-after"
...
def round4_round5(width: int, height: int):
    ...
    return artefatos("round4-round5", before, after, width, height)


def claude_round5(width: int, height: int):
    ...
    return artefatos("claude-round5", reference, actual, width, height)


if __name__ == "__main__":
    AFTER.mkdir(parents=True, exist_ok=True)
    results = [round4_round5(*viewport) for viewport in VIEWPORTS]
    results.extend(claude_round5(*viewport) for viewport in ((1440, 900), (1280, 800), (390, 843)))
    (AFTER / "visual-diff-round5.json").write_text(

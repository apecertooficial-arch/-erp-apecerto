...
test("navegador nunca pode forjar a origem D-API", () => {
  ...
  assert.match(componente, /fonte: "registro_operacional"/);
  assert.doesNotMatch(componente, /fonte: "dapi"/);
});
...
test("confirmação exige gesto explícito, trata erro e atualiza a ficha", () => {
  ...
  assert.match(componente, /onConfirmada\(json\.resultado \?\? null\)/);
  assert.match(componente, /Somente o corretor responsável/);
  assert.match(componente, /confirmada automaticamente pelo D-API/);
  assert.match(componente, /Ação registrada\. A Sara está analisando/);
});

test("ação manual gera evento idempotente para a Sara sem fingir reavaliação", () => {
  ...
  assert.match(draft, /returning id into v_evento_id/);
  assert.match(draft, /'sara_em_fila', true/);
  assert.doesNotMatch(draft, /ultima_reavaliacao_sara_em\s*=\s*now\(\)/);
  assert.doesNotMatch(draft, /'sara_reavaliou'/);
});

test("draft fecha o bypass direto da RPC sem apagar histórico", () => {
...
test("API traduz recusas da confirmação mesmo se o estado mudar dentro da transação", () => {
  assert.match(rota, /confirmacao_dapi_obrigatoria:/);
  assert.match(rota, /confirmacao_dapi_pelo_webhook:/);
  assert.match(rota, /momento_invalido:/);
});
